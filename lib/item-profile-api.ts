...
export async function fetchGameItemProfiles(gameId: string, name?: string): Promise<ItemProfile[]> {
    ...
    if (!res.ok)
        throw new Error("Failed to fetch item profiles");
    const data = await res.json();
    // Transform the response to match ItemProfile interface
    return data.items || [];
}
export async function createItemProfile(gameId: string, profileData: {
    name: string;
    code_name?: string;
    type?: string;
    level_start?: number;
...
export async function fetchItemProfile(profileId: string): Promise<ItemProfile> {
    ...
    if (!res.ok)
        throw new Error("Failed to fetch item profile");
...
export async function updateItemProfileStatus(itemProfileId: string, status: string): Promise<ItemProfile> {
    ...
    if (!res.ok)
        throw new Error("Failed to update item profile status");
    const data: ApiResponse<ItemProfile> = await res.json();
    return data.data;
}
...
export async function fetchFixedLootboxItems(lootboxProfileId: string): Promise<LootboxItemDetail[]> {
    ...
    if (!res.ok)
        throw new Error("Failed to fetch loot box items");
