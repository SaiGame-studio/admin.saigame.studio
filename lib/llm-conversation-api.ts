...
import type { Conversation, ListConversationsResponse, CreateConversationRequest, UpdateConversationRequest, SubmitRequestBody, SubmitRequestResponse, CreateRecordsResponse, CreateLoreRecordsResponse, RequestType, ConversationContentLink, } from '@/types/llm-conversation';
...
function extractErrorCodeAndMessage(data: unknown, status: number): string {
    if (data && typeof data === 'object' && !Array.isArray(data)) {
        const record = data as Record<string, unknown>;
        const errorCode = typeof record.error === 'string' && record.error.trim() ? record.error.trim() : '';
        if (errorCode)
            return errorCode;
        const errorMessage = typeof record.message === 'string' && record.message.trim() ? record.message.trim() : '';
        if (errorMessage)
            return errorMessage;
    }
    return `HTTP ${status}`;
...
export function extractTokenUsage(payload: Record<string, unknown> | null | undefined): TokenUsageInfo | null {
    if (!payload)
        return null;
    ...
    const usageRecord = usage && typeof usage === 'object' && !Array.isArray(usage)
        ...
        : null;
    const inputTokens = toFiniteNumber(usageRecord?.input_tokens ?? usageRecord?.inputTokens ?? payload.input_tokens ?? payload.inputTokens);
    const outputTokens = toFiniteNumber(usageRecord?.output_tokens ?? usageRecord?.outputTokens ?? payload.output_tokens ?? payload.outputTokens);
    let totalTokens = toFiniteNumber(usageRecord?.total_tokens ?? usageRecord?.totalTokens ?? payload.total_tokens ?? payload.totalTokens ?? payload.tokens);
...
export async function listConversations(gameId: string, params?: {
...
export async function createConversation(gameId: string, body: CreateConversationRequest): Promise<Conversation> {
    return api.post(base(gameId), body);
}
export async function getConversation(gameId: string, conversationId: string): Promise<Conversation> {
    return api.get(`${base(gameId)}/${conversationId}`);
}
export async function updateConversation(gameId: string, conversationId: string, body: UpdateConversationRequest): Promise<Conversation> {
    return api.patch(`${base(gameId)}/${conversationId}`, body);
}
export async function submitRequest(gameId: string, conversationId: string, body: SubmitRequestBody): Promise<SubmitRequestResponse> {
    return api.post(`${base(gameId)}/${conversationId}/requests`, body);
}
export async function archiveConversation(gameId: string, conversationId: string): Promise<Conversation> {
    return api.post(`${base(gameId)}/${conversationId}/archive`);
}
export async function unarchiveConversation(gameId: string, conversationId: string): Promise<Conversation> {
    return api.post(`${base(gameId)}/${conversationId}/unarchive`);
}
export async function deleteConversation(gameId: string, conversationId: string): Promise<void> {
    return api.delete(`${base(gameId)}/${conversationId}`);
}
export async function createRecordsFromConversation(gameId: string, conversationId: string, loreEntryIds?: string[]): Promise<CreateRecordsResponse> {
    return api.post(`${base(gameId)}/${conversationId}/create-records`, {
        ...(loreEntryIds && loreEntryIds.length > 0 ? { lore_entry_ids: loreEntryIds } : {}),
    });
}
export async function createLoreRecordsFromConversation(gameId: string, conversationId: string): Promise<CreateLoreRecordsResponse> {
    return api.post(`${base(gameId)}/${conversationId}/create-lore-records`);
}
export async function linkConversationContent(gameId: string, conversationId: string, contentType: string, contentId: string): Promise<void> {
    return api.post(`${base(gameId)}/${conversationId}/content`, {
        content_type: contentType,
        content_id: contentId,
    });
}
export async function listConversationContent(gameId: string, conversationId: string): Promise<ConversationContentLink[]> {
    const res = await api.get(`${base(gameId)}/${conversationId}/content`);
    return (res?.items ?? []) as ConversationContentLink[];
}
export async function unlinkConversationContent(gameId: string, conversationId: string, contentType: string, contentId: string): Promise<void> {
...
function normalizeDetectedIntents(payload: Record<string, unknown>): DetectedIntentResult {
    ...
    if (detectedRequestType) {
        ...
        intents = (payload.detected_intents as Array<{
            type?: string;
            entity_type?: string;
            goal?: string;
            goals?: string[];
...
export async function streamDetectIntent(gameId: string, conversationId: string, userPrompt: string, history: DetectIntentHistoryEntry[], contextIds: ConversationContextIds, onChunk: (text: string) => void, onDone: (result: DetectedIntentResult, usage?: TokenUsageInfo) => void, onError: (message: string) => void): Promise<void> {
    ...
    const res = await fetch(`${apiUrl}${base(gameId)}/${conversationId}/detect-intent`, {
        method: 'POST',
        headers: {
    ...
    if (!res.ok) {
        ...
        throw new Error(extractErrorCodeAndMessage(data, res.status));
    }
    const contentType = res.headers.get('content-type') ?? '';
    if (!contentType.includes('text/event-stream')) {
        const payload = await res.json() as Record<string, unknown>;
        onDone(normalizeDetectedIntents(payload), extractTokenUsage(payload) ?? undefined);
        return;
    }
    ...
    while (true) {
        ...
        for (const part of parts) {
            ...
            if (evt.type === 'chunk') {
                ...
                onError((typeof evt.error === 'string' && evt.error) ||
                    (typeof evt.code === 'string' && evt.code) ||
    ...
    if (leftover) {
        try {
            ...
            if (evt.detected_request_type || evt.detected_intents) {
                onDone(normalizeDetectedIntents(evt), extractTokenUsage(evt) ?? undefined);
            }
        }
        catch {
...
export async function requestContainerCreatingPlanning(gameId: string, conversationId: string, userPrompt: string, contextIds: ConversationContextIds, options?: {
    entityType?: string;
    goals?: string[];
    history?: DetectIntentHistoryEntry[];
    ...
    if (options?.goals?.length)
        body.goals = options.goals;
    if (options?.history?.length)
        body.history = options.history;
    return api.post(`${base(gameId)}/${conversationId}/requests/container-creating-planning`, body, { suppressToast: true });
}
export async function requestCraftingRecipeCreatingPlanning(gameId: string, conversationId: string, userPrompt: string, contextIds: ConversationContextIds, options?: {
    language?: string;
    entityType?: string;
    goals?: string[];
    history?: DetectIntentHistoryEntry[];
    ...
    if (options?.history?.length)
        body.history = options.history;
    return api.post(`${base(gameId)}/${conversationId}/requests/crafting-recipe-creating-planning`, body, { suppressToast: true });
}
export async function requestGachaPackCreatingPlanning(gameId: string, conversationId: string, userPrompt: string, contextIds: ConversationContextIds, options?: {
    language?: string;
    entityType?: string;
    goals?: string[];
    history?: DetectIntentHistoryEntry[];
    ...
    if (options?.history?.length)
        body.history = options.history;
    return api.post(`${base(gameId)}/${conversationId}/requests/gacha-pack-creating-planning`, body, { suppressToast: true });
}
export async function requestGeneratorItemCreatingPlanning(gameId: string, conversationId: string, userPrompt: string, contextIds: ConversationContextIds, options?: {
    language?: string;
    entityType?: string;
    goals?: string[];
    history?: DetectIntentHistoryEntry[];
    ...
    if (options?.history?.length)
        body.history = options.history;
    return api.post(`${base(gameId)}/${conversationId}/requests/generator-item-creating-planning`, body, { suppressToast: true });
}
export async function requestEntityPoolCreatingPlanning(gameId: string, conversationId: string, userPrompt: string, contextIds: ConversationContextIds, options?: {
    language?: string;
    entityType?: string;
    goals?: string[];
    history?: DetectIntentHistoryEntry[];
    ...
    if (options?.history?.length)
        body.history = options.history;
    return api.post(`${base(gameId)}/${conversationId}/requests/entity-pool-creating-planning`, body, { suppressToast: true });
}
export async function requestQuestDefinitionGenerationPlanning(gameId: string, conversationId: string, userPrompt: string, contextIds: ConversationContextIds, options?: {
    language?: string;
    entityType?: string;
    goals?: string[];
    history?: DetectIntentHistoryEntry[];
    ...
    if (options?.history?.length)
        body.history = options.history;
    if (options?.requestHistory?.length)
        body.request_history = options.requestHistory;
    return api.post(`${base(gameId)}/${conversationId}/requests/quest-definition-generation-planning`, body, { suppressToast: true });
}
export async function requestQuestDefinitionGeneration(gameId: string, conversationId: string, userPrompt: string, contextIds: ConversationContextIds, options?: QuestDefinitionGenerationRequestOptions): Promise<QuestDefinitionGenerationResponse> {
    const body: Record<string, unknown> = {
        user_prompt: userPrompt,
        lore_entry_ids: contextIds.lore_entry_ids,
        item_definition_ids: contextIds.item_definition_ids,
    ...
    if (options?.requestHistory?.length)
        body.request_history = options.requestHistory;
    if (options?.generatedItems?.length)
        body.generated_items = options.generatedItems;
    return api.post(`${base(gameId)}/${conversationId}/requests/quest-definition-generation`, body, { suppressToast: true });
}
function requestPathForType(requestType: string): string {
    return requestType.replace(/_/g, '-');
}
export async function streamRequest(gameId: string, conversationId: string, requestType: string, userPrompt: string, onChunk: (text: string) => void, onDone: (requestId: string, usage?: TokenUsageInfo) => void, onError: (message: string) => void, requestHistory?: Array<{
    request_type: string;
    response_text: string;
}>, contextIds?: ConversationContextIds, entityType?: string, goals?: string[], generatedItems?: unknown[]): Promise<void> {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL;
    if (!apiUrl)
        throw new Error('API URL is not configured.');
    const token = getValidToken();
    ...
    const res = await fetch(`${apiUrl}${base(gameId)}/${conversationId}/requests/${urlType}`, {
        method: 'POST',
        headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
    ...
    while (true) {
        ...
        for (const part of parts) {
            ...
            if (evt.type === 'chunk') {
                ...
                onDone(evt.request_id ?? '', extractTokenUsage(evt as Record<string, unknown>) ?? undefined);
                return;
            }
            else if (evt.type === 'error') {
                onError((typeof evt.error === 'string' && evt.error) ||
