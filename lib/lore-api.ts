...
): Promise<ListLoreEntriesResponse> {
...
export async function getLoreTypes(): Promise<string[]> {
  const res: { data: string[] } = await api.get('/api/v1/lore/types')
  return res.data ?? []
}
