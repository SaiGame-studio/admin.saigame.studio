...
function maskEmail(email?: string): string {
    if (!email || !email.includes("@"))
        return "***@saigame.studio";
    const [localPart, domain] = email.split("@");
    if (!localPart)
        return `***@${domain || "saigame.studio"}`;
...
export async function getPlayerIdentityMapByUserIds(userIds: string[], seed?: Array<{
    user_id: string;
    user_display_name?: string;
    user_email?: string;
}>, gameId?: string): Promise<Record<string, PlayerIdentity>> {
    ...
    if (missingUserIds.length > 0) {
        try {
            ...
            if (gameId) {
                ...
                userMap = new Map(progress.map((p) => [p.user_id, { display_name: p.user_display_name, email: p.user_email }]));
            }
            else {
...
export async function getGameProgressList(gameId: string, params?: {
    display_name?: string;
...
export async function getProgressItems(progressId: string, params?: {
    limit?: number;
    offset?: number;
    name?: string;
    category?: string;
    ...
    if (params?.container_id)
        qs.set("container_id", params.container_id);
...
export async function getProgressContainers(progressId: string, params?: {
    limit?: number;
    offset?: number;
    type?: string;
...
export async function getProgressContainer(progressId: string, containerId: string): Promise<PlayerContainer> {
...
export async function getContainerItems(progressId: string, containerId: string, params?: {
    limit?: number;
    offset?: number;
...
export async function getPlayerPresets(gameId: string, userId: string): Promise<PlayerPresetsResult> {
...
export async function getPlayerPresetDetail(gameId: string, userId: string, presetId: string): Promise<PlayerPresetDetail> {
...
export async function getGachaTransactions(progressId: string, params?: {
    limit?: number;
    offset?: number;
...
export async function getBattleSessions(gameId: string, playerId: string, params?: {
    limit?: number;
    offset?: number;
...
export async function getJourneySessions(gameId: string, userId: string, params?: {
    limit?: number;
    offset?: number;
    from?: string;
    to?: string;
...
export async function getJourneySessionEvents(gameId: string, sessionId: string, params?: {
    limit?: number;
    offset?: number;
...
export async function getPlayerEquipped(gameId: string, progressId: string): Promise<PlayerEquippedResult> {
