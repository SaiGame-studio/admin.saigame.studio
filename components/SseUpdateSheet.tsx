...
function FieldRow({
  ...
  return (
    <div id={`sse-update-field-row-${label}`} className="grid grid-cols-3 gap-2 items-start text-xs py-1.5 border-b last:border-0">
      ...
      <span
        id={`sse-update-field-proposed-${label}`}
        className={same ? 'text-muted-foreground truncate' : 'font-semibold text-foreground truncate'}
      >
...
export function SseUpdateSheet({
  open,
  onClose,
  onApplied,
  item,
  ...
  async function handleApply() {
    ...
    try {
      ...
      if (sseData.name?.trim()) patch.name = sseData.name.trim()
      if (sseData.item_code !== undefined) patch.item_code = sseData.item_code?.trim() || undefined
      if (sseData.category) patch.category = sseData.category
      if (sseData.rarity) patch.rarity = sseData.rarity
      if (sseData.is_stackable !== undefined) patch.is_stackable = sseData.is_stackable
      if (sseData.max_stack_size != null && sseData.max_stack_size !== '') {
        patch.max_stack_size = Number(sseData.max_stack_size) || null
      }
      if (sseData.grid_width != null) patch.grid_width = Number(sseData.grid_width) || 1
      if (sseData.grid_height != null) patch.grid_height = Number(sseData.grid_height) || 1
      if (sseData.client_writable !== undefined) patch.client_writable = sseData.client_writable
      if (sseData.allow_client_update_qty !== undefined) patch.allow_client_update_qty = sseData.allow_client_update_qty
    ...
    <Sheet open={open} onOpenChange={(v) => { if (!v) onClose() }}>
      <SheetContent id="sse-update-sheet-root" side="right" className="w-full sm:max-w-md overflow-y-auto flex flex-col">
        ...
        <div id="sse-update-sheet-body" className="flex-1 overflow-y-auto py-3 space-y-1">
          ...
          {sseData.name !== undefined && (
            <FieldRow label={t('items.name')} current={item.name} proposed={sseData.name} />
          )}
          {sseData.item_code !== undefined && (
            <FieldRow label={t('items.itemCode')} current={item.item_code ?? '—'} proposed={sseData.item_code ?? '—'} />
          )}
          {sseData.category !== undefined && (
            <FieldRow label={t('items.category')} current={item.category} proposed={sseData.category} />
          )}
          {sseData.rarity !== undefined && (
            <FieldRow
              ...
              proposed={
                rarityColor ? (
                  <Badge
                    id="sse-update-rarity-badge"
                    variant="outline"
                    className={`text-[10px] px-1 py-0 ${rarityColor.text} ${rarityColor.border} ${rarityColor.bg}`}
                  >
                    {sseData.rarity}
                  </Badge>
                ) : sseData.rarity
          ...
          {sseData.grid_width !== undefined && (
            <FieldRow
              label={`${t('items.gridWidth')} × ${t('items.gridHeight')}`}
              current={`${item.grid_width} × ${item.grid_height}`}
              proposed={`${sseData.grid_width} × ${sseData.grid_height}`}
            />
          )}
          {sseData.is_stackable !== undefined && (
            <FieldRow
              label={t('items.stackable')}
              current={item.is_stackable ? '✓' : '✗'}
              proposed={sseData.is_stackable ? '✓' : '✗'}
            />
          )}
          {sseData.max_stack_size !== undefined && (
            <FieldRow label={t('items.maxStackSize') || 'Max Stack'} current={item.max_stack_size ?? '∞'} proposed={sseData.max_stack_size || '∞'} />
          )}
          {sseData.description !== undefined && sseData.description !== '' && (
            <FieldRow
              label={t('items.description')}
              current={(item.metadata?.description as string | undefined) ?? '—'}
              proposed={sseData.description}
            />
          )}
          {sseData.stats && sseData.stats.length > 0 && (
            <FieldRow
              label={t('items.baseStats')}
              current={
          ...
          {sseData.client_writable !== undefined && (
            <FieldRow
              label={t('items.allowClientWriteProps')}
              current={item.client_writable ? '✓' : '✗'}
              proposed={sseData.client_writable ? '✓' : '✗'}
            />
          )}
          {sseData.allow_client_update_qty !== undefined && (
            <FieldRow
              label={t('items.allowClientUpdateQty')}
              current={item.allow_client_update_qty ? '✓' : '✗'}
              proposed={sseData.allow_client_update_qty ? '✓' : '✗'}
            />
          )}
        </div>
