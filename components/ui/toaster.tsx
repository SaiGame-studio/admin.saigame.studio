...
function ToastCopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded text-foreground/60 hover:text-foreground hover:bg-foreground/10 transition-colors group-[.destructive]:text-red-200 group-[.destructive]:hover:text-red-50 group-[.destructive]:hover:bg-red-500/20"
    >
    ...
    <ToastProvider swipeDirection="left">
      {toasts.map(function ({ id, title, description, action, ...props }) {
        ...
        return (
          <Toast key={id} {...props}>
            <div
              className="grid gap-1 flex-1 min-w-0 select-text"
              onPointerDown={(e) => e.stopPropagation()}
              onMouseDown={(e) => e.stopPropagation()}
            >
              {title && (
