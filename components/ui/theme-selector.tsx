...
export function ThemeSelector() {
    ...
    return (<Select value={theme} onValueChange={handleThemeChange}>
      <SelectTrigger className="w-[240px]">
        <SelectValue placeholder="Select theme">
          {currentTheme && (<div className="flex items-center gap-3">
      ...
      <SelectContent className="w-[280px]">
        ...
        {getThemesByCategory('light').map((option) => (<SelectItem key={option.value} value={option.value} className="cursor-pointer">
        ...
        {getThemesByCategory('dark').map((option) => (<SelectItem key={option.value} value={option.value} className="cursor-pointer">
            <div className="flex items-center gap-3 w-full py-1">
              <option.icon className="h-4 w-4 flex-shrink-0"/>
              <div className="flex flex-col min-w-0 flex-1">
                <span className="text-sm font-medium">{t(option.labelKey)}</span>
        ...
        {getThemesByCategory('auto').map((option) => (<SelectItem key={option.value} value={option.value} className="cursor-pointer">
            <div className="flex items-center gap-3 w-full py-1">
              <option.icon className="h-4 w-4 flex-shrink-0"/>
              <div className="flex flex-col min-w-0 flex-1">
                <span className="text-sm font-medium">{t(option.labelKey)}</span>
