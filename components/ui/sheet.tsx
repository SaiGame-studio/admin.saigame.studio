...
const SheetContent = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Content>, SheetContentProps>(({ side = "right", className, overlayClassName, children, ...props }, ref) => (<SheetPortal>
...
// Strip custom `id` — Radix uses its own generated titleId for aria-labelledby;
// overriding it breaks the accessible link and triggers the DialogTitle warning.
>(({ className, id: _id, ...props }, ref) => (<SheetPrimitive.Title ref={ref} className={cn("text-lg font-semibold text-foreground", className)} {...props}/>));
...
// Strip custom `id` — same reason as SheetTitle; Radix owns the descriptionId.
>(({ className, id: _id, ...props }, ref) => (<SheetPrimitive.Description ref={ref} className={cn("text-sm text-muted-foreground", className)} {...props}/>));
