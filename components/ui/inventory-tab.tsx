...
const InventoryIndicator = ({ inventory, gameId, t }: InventoryIndicatorProps) => (<TooltipProvider>
    <Tooltip>
      <TooltipTrigger asChild>
        <Link href={getInventoryTabUrl(gameId, inventory.id)} className="flex items-center justify-center w-5 h-5 cursor-pointer hover:scale-110 transition-transform">
...
export function InventoryTab({ itemProfile, gameId }: InventoryTabProps) {
    ...
    const [itemToMove, setItemToMove] = useState<{
        itemId: string;
        fromInventory: ItemProfile | null;
    }>({ itemId: "", fromInventory: null });
    ...
    const loadData = async () => {
        try {
            ...
            const filteredItems = allItems.filter(item => item.type !== 'inventory' &&
    ...
    const filteredAvailableItems = availableItems.filter(item => item.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-4">
              ...
              <div className="relative">
                ...
                <Input placeholder={t('inventory.searchItems')} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 w-64"/>
              </div>
            </div>
            ...
            return (<SelectItem key={item.id} value={item.id}>
                        <div className="flex items-center justify-between w-full">
                          ...
                          {belongsToInventory && (<InventoryIndicator inventory={belongsToInventory} gameId={gameId} t={t}/>)}
                        </div>
                      </SelectItem>);
        })}
                </SelectContent>
              </Select>
              <Button onClick={handleAddItem} disabled={!selectedItemId || isAddingItem} className="whitespace-nowrap">
                <Plus className="w-4 h-4 mr-2"/>
                {isAddingItem ? t('common.adding') : t('inventory.addItem')}
              </Button>
            </div>
          </div>
          {inventoryItems.length === 0 ? (<p className="text-center text-muted-foreground py-8">
              {t('inventory.emptyInventory')}
            </p>) : (<div className="space-y-2">
              {inventoryItems.map((item) => (<div key={item.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <Link href={`/games/${gameId}/item-profiles/${item.id}`} className="font-medium inline-flex items-center gap-1 hover:text-primary">
                        {item.name}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {itemToMove.fromInventory && (<div className="mt-2 p-2 bg-muted rounded">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleConfirmMove} className="bg-orange-600 hover:bg-orange-700">
