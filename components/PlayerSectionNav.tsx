...
export function PlayerSectionNav({ gameId, progressId, activeTab, onTabChange, counts = {}, }: PlayerSectionNavProps) {
    const isActive = (value: PlayerNavTab) => value === activeTab || (value === "containers" && activeTab === "containers-detail");
    const badge = (value: PlayerNavTab) => {
        let n: number | undefined;
        let plus = false;
        if (value === "items" && counts.items != null)
            n = counts.items;
        if (value === "containers" && counts.containers != null) {
            n = counts.containers;
            plus = !!counts.containersHasMore;
        }
        if (value === "presets" && counts.presets != null)
            n = counts.presets;
        if (value === "generators" && counts.generators != null)
            n = counts.generators;
        if (value === "quests" && counts.quests != null)
            n = counts.quests;
        if (value === "journey" && counts.journey != null && activeTab === "journey")
            n = counts.journey;
        if (value === "transactions" && counts.transactions != null && activeTab === "transactions")
            n = counts.transactions;
        if (!n)
            return null;
        return (<span className="ml-1.5 rounded-full bg-muted px-1.5 py-0.5 text-xs leading-none">
        {n}{plus ? "+" : ""}
      </span>);
    };
    return (<div className="border-b mb-4">
      <div className="flex items-center">
        {TABS.map((tab) => {
            ...
            return (<a key={tab.value} href={href} onClick={onTabChange
                    ? (e) => {
                    ...
                    : undefined} className={`inline-flex items-center whitespace-nowrap px-3 py-2 text-sm font-medium border-b-2 -mb-px transition-colors ${active
        ...
        <a href={`/games/${gameId}/mailbox?userId=${progressId}`} className="inline-flex items-center gap-1.5 whitespace-nowrap px-3 py-2 text-sm font-medium border-b-2 -mb-px border-transparent text-muted-foreground hover:text-foreground hover:border-muted-foreground/40 transition-colors">
