...
export default function TeamNameEditable({ team, teamId, onNameUpdate }: TeamNameEditableProps) {
    ...
    return (<div className="group flex items-center gap-2">
      {editing ? (<>
          <Input value={name} onChange={e => setName(e.target.value)} className="w-64 h-9 px-2 text-3xl font-bold" disabled={loading}/>
...
export function TeamDescriptionEditable({ team, teamId, onDescriptionUpdate }: TeamDescriptionEditableProps) {
    ...
    return (<div className="group flex items-center gap-2 mt-1">
      {editing ? (<>
          <Input value={description} onChange={e => setDescription(e.target.value)} placeholder={t('team.addDescPlaceholder')} className="w-96 h-8 px-2 text-sm" disabled={loading}/>
          <Button size="icon" variant="ghost" onClick={handleSave} disabled={loading}>
            <Save className="w-4 h-4"/>
          </Button>
          <Button size="icon" variant="ghost" onClick={() => {
