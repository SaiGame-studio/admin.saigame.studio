...
export function AddTeamToGameDialog({ gameId, studioId, existingTeamIds, onTeamsAdded }: AddTeamToGameDialogProps) {
    ...
    function toggleTeam(teamId: string) {
        setSelectedTeamIds(prev => prev.includes(teamId)
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            await Promise.all(selectedTeamIds.map(teamId => assignTeamToGame(teamId, gameId)));
            toast({
        ...
        <SheetContent side="right" className="w-full sm:max-w-lg flex flex-col">
          <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
            ...
            <div className="py-4 flex-1 overflow-y-auto">
              {teamsLoading ? (<div className="flex items-center justify-center py-8">
                  <Loader2 className="h-6 w-6 animate-spin text-muted-foreground"/>
                </div>) : availableTeams.length > 0 ? (<div className="space-y-3">
                  {availableTeams.map((team) => (<div key={team.id} className="flex items-start space-x-3 p-3 border rounded-md hover:bg-accent">
                      <Checkbox id={`team-${team.id}`} checked={selectedTeamIds.includes(team.id)} onCheckedChange={() => toggleTeam(team.id)} disabled={loading}/>
                      <div className="flex-1">
                        ...
                        <div className="flex gap-2 mt-1">
                          {team.slug && (<span className="text-xs text-muted-foreground font-mono">
                              {team.slug}
                            </span>)}
            ...
            <SheetFooter className="pt-4">
              ...
              <Button type="submit" disabled={loading || selectedTeamIds.length === 0}>
