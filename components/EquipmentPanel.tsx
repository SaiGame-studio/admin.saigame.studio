...
export function EquipmentPanel({ gameId, catalog, gamePlugins }: EquipmentPanelProps) {
    ...
    return (<div className="space-y-2">
      <div className="flex items-center gap-2 mb-1">
        ...
        <Link href={`/games/${gameId}/plugins`} className="text-muted-foreground hover:text-foreground">
      ...
      <div className="flex flex-col gap-2">
        {visiblePlugins.map((plugin, idx) => {
            ...
            const subs = gamePlugins?.subscriptions.filter((s) => s.plugin.id === plugin.id && !s.subscription.is_revoked) ?? [];
            ...
            return (<div key={plugin.id} className="flex items-center gap-2">
              ...
              <div className="flex gap-1">
                {Array.from({ length: plugin.max_stacks }).map((_, si) => (<span key={si} className="relative inline-flex w-5 h-5">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={tier.image} alt={tier.label} className="w-full h-full rounded-full" style={si < activeOwned
                    ...
                    {si >= activeOwned && si < owned && (<span className="absolute inset-0 rounded-full border-2 border-orange-400/70"/>)}
                  </span>))}
              </div>
