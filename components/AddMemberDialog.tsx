      ...
      <SheetContent side="right" className="w-full sm:max-w-lg flex flex-col">
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          <SheetHeader>
            ...
            {studioMembersLimit != null && studioMembersUsage != null && (<div className="mt-2 space-y-1.5 rounded-md border border-border bg-muted/40 px-3 py-2.5">
                <div className="flex items-center justify-between text-xs">
                  <span className="flex items-center gap-1.5 font-medium text-muted-foreground">
                    <Users className="h-3.5 w-3.5 shrink-0"/>
                    {t('team.studioMemberSlots')}
                  </span>
                  <span className={`font-semibold ${studioMembersUsage >= studioMembersLimit
                ? 'text-destructive'
                : 'text-foreground'}`}>
                    {studioMembersUsage} / {studioMembersLimit}
                    {studioMembersUsage >= studioMembersLimit && (<span className="ml-1 font-normal">{t('team.limitReached')}</span>)}
                  </span>
                </div>
                <Progress value={Math.min((studioMembersUsage / studioMembersLimit) * 100, 100)} className={`h-1.5 ${studioMembersUsage >= studioMembersLimit
                ? '[&>div]:bg-destructive'
                : ''}`}/>
          ...
          <div className="flex flex-col gap-4 py-4 flex-1 overflow-y-auto">
            <div className="grid gap-2">
              ...
              <Input id="user-id" placeholder={t('team.userIdPlaceholder')} value={userId} onChange={(e) => setUserId(e.target.value)} disabled={loading}/>
            </div>
            <div className="grid gap-2">
