...
export function ConversationLinkedContent({ gameId, linkedContent, isLoadingLinkedContent, unlinkingId, loreEntryTitles, itemDefinitionNames, entityDefinitionNames, containerDefinitionNames, presetDefinitionNames, gachaPackNames, craftingRecipeNames, entityPoolNames, entityPoolKeys, questDefinitionNames, onUnlink, t, }: ConversationLinkedContentProps) {
    ...
    function renderBadge(link: ConversationContentLink, refNum: string) {
        ...
        return (<span key={link.id} id={`conv-panel-linked-item-${link.id}`} className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] min-w-0 overflow-hidden ${badgeClass}`} title={`${refNum} · ${displayName}`}>
        ...
        <button id={`conv-panel-linked-item-name-${link.id}`} type="button" className="font-medium hover:underline hover:text-foreground transition-colors truncate flex-1 min-w-0 text-left" onClick={(e) => {
                e.stopPropagation();
                if (isItem) {
        ...
        <button id={`conv-panel-linked-item-unlink-${link.id}`} type="button" className="opacity-50 hover:opacity-100 hover:text-destructive transition-opacity shrink-0" disabled={unlinkingId === link.id} onClick={(e) => { e.stopPropagation(); onUnlink(link.id, link.content_type, link.content_id); }}>
          {unlinkingId === link.id
    ...
    return (<div id="conv-panel-linked-content" className="shrink-0 border-t px-2 pt-1.5 pb-1">
      ...
      <div id="conv-panel-linked-content-groups" className="flex flex-col gap-1.5">
        {itemLinks.length > 0 && (<div id="conv-panel-linked-content-items-group">
        ...
        {entityLinks.length > 0 && (<div id="conv-panel-linked-content-entity-defs-group">
        ...
        {loreLinks.length > 0 && (<div id="conv-panel-linked-content-lore-group">
        ...
        {containerLinks.length > 0 && (<div id="conv-panel-linked-content-containers-group">
        ...
        {presetLinks.length > 0 && (<div id="conv-panel-linked-content-preset-defs-group">
        ...
        {gachaPackLinks.length > 0 && (<div id="conv-panel-linked-content-gacha-packs-group">
        ...
        {entityPoolLinks.length > 0 && (<div id="conv-panel-linked-content-entity-pools-group">
        ...
        {craftingRecipeLinks.length > 0 && (<div id="conv-panel-linked-content-crafting-recipes-group">
        ...
        {questDefinitionLinks.length > 0 && (<div id="conv-panel-linked-content-quest-defs-group">
