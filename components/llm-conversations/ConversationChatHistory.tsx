...
function TagSuggestionResult({ responseText, turnId, responseIdx, isDone, isStreaming, gameId, appliedTags, createdItemTags, onApplyGameTag, onRemoveGameTag, onCreateItemTag, onDeleteItemTag, t, }: TagSuggestionResultProps) {
    if (!isDone) {
        return (<Loader2 id={`conv-panel-tag-suggestion-loader-${turnId}-${responseIdx}`} className="h-3 w-3 animate-spin text-muted-foreground"/>);
    }
    ...
    if (parsedTags.length === 0) {
        return (<p id={`conv-panel-tag-suggestion-empty-${turnId}-${responseIdx}`} className="text-xs text-muted-foreground italic">
        {t('llmConversation.tagSuggestionEmpty')}
      </p>);
    }
    return (<div id={`conv-panel-tag-suggestion-result-${turnId}-${responseIdx}`} className="flex flex-col gap-3">
      <div id={`conv-panel-tag-suggestion-tags-${turnId}-${responseIdx}`} className="flex flex-wrap gap-2">
        {parsedTags.map((tag) => {
            ...
            return (<div key={tag} id={`conv-panel-tag-chip-${turnId}-${responseIdx}-${tag}`} className="inline-flex items-center rounded-full border bg-muted/30 text-[11px] overflow-hidden">
              {/* Tag label */}
              <span id={`conv-panel-tag-chip-label-${turnId}-${responseIdx}-${tag}`} className="flex items-center gap-1 px-2.5 py-0.5 text-muted-foreground border-r">
                <Tag className="h-2.5 w-2.5 shrink-0"/>
                {tag}
              </span>
              {/* Apply / Remove game tag (toggle) */}
              <button id={`conv-panel-tag-chip-game-btn-${turnId}-${responseIdx}-${tag}`} type="button" onClick={() => isGameApplied
                    ? onRemoveGameTag(tag, turnId, responseIdx)
                    : onApplyGameTag(tag, turnId, responseIdx)} disabled={isStreaming} title={isGameApplied ? t('llmConversation.tagSuggestionRemoveFromGame') : t('llmConversation.tagSuggestionApply')} className={[
              ...
              <button id={`conv-panel-tag-chip-item-btn-${turnId}-${responseIdx}-${tag}`} type="button" onClick={() => isItemCreated
                    ? onDeleteItemTag(tag, turnId, responseIdx)
                    : onCreateItemTag(tag, turnId, responseIdx)} disabled={isStreaming} title={isItemCreated ? t('llmConversation.tagSuggestionDeleteItemTag') : t('llmConversation.tagSuggestionCreateItemTag')} className={[
      ...
      {reasoning && (<div id={`conv-panel-tag-suggestion-reasoning-${turnId}-${responseIdx}`} className="flex flex-col gap-0.5">
          <span id={`conv-panel-tag-suggestion-reasoning-label-${turnId}-${responseIdx}`} className="text-[10px] font-medium text-muted-foreground uppercase tracking-wide">
            {t('llmConversation.tagSuggestionReasoning')}
          </span>
          <p id={`conv-panel-tag-suggestion-reasoning-text-${turnId}-${responseIdx}`} className="text-xs text-muted-foreground italic">
            {reasoning}
          </p>
        </div>)}
      ...
      <div id={`conv-panel-tag-suggestion-links-${turnId}-${responseIdx}`} className="flex items-center gap-3">
        <Link id={`conv-panel-tag-suggestion-link-game-${turnId}-${responseIdx}`} href={`/games/${gameId}`} className="flex items-center gap-1 rounded-full border border-sky-500/20 bg-sky-500/10 px-2 py-1 text-[11px] text-sky-700 transition-colors hover:bg-sky-500/20 hover:text-sky-800">
          <Gamepad2 className="h-3 w-3 text-sky-600"/>
          {t('llmConversation.tagSuggestionViewGameDetail')}
        </Link>
        <Link id={`conv-panel-tag-suggestion-link-item-tags-${turnId}-${responseIdx}`} href={`/games/${gameId}/items?tab=tags`} className="flex items-center gap-1 rounded-full border border-amber-500/20 bg-amber-500/10 px-2 py-1 text-[11px] text-amber-700 transition-colors hover:bg-amber-500/20 hover:text-amber-800">
...
const MARKDOWN_COMPONENTS = {
    pre: ({ children, ...props }: React.HTMLAttributes<HTMLPreElement>) => (<div id="conv-panel-md-pre-scroll-wrap" className="overflow-x-auto w-full my-2 rounded">
...
export function ConversationChatHistory({ chatHistory, isStreaming, gameId, activeConvId, savedLoreIds, loreEntryTitles, savedItemDefinitionIds, savedEntityDefinitionIds, savedPresetDefinitionIds, savedContainerDefinitionIds, savedGachaPackIds, savedEquipmentSlotIds, savedCraftingRecipeIds, savedEntityPoolIds, savedQuestDefinitionIds, craftingRecipeNames, entityPoolNames, questDefinitionNames, contentFontSize, premiumTokensRemaining, onRetry, onRetryResponse, onOpenLoreReview, onSaveItemDefinition, onSaveEntityDefinition, onSavePresetDefinition, onSaveContainerDefinition, onSaveGachaPack, onSaveEquipmentSlot, onSaveCraftingRecipe, onSaveEntityPool, onSaveQuestDefinition, onBuyTokens, onApplyTagSuggestion, onRemoveGameTag, onCreateItemTagFromSuggestion, onDeleteItemTagFromSuggestion, appliedTagsPerResponse, createdItemTagsPerResponse, t, }: ConversationChatHistoryProps) {
    ...
    function resolveErrorMessage(message: string | null): string {
        ...
        // Older persisted turns can store the translation key itself.
        // Re-translate it here so the UI stays localized even when the data is stale.
        if (message.startsWith('llmConversation.')) {
    ...
    function isTokenQuotaError(message: string): boolean {
        ...
        return resolvedMessage === t('llmConversation.errorTokenQuotaExceeded')
            || /token_quota_exceeded|lm token quota exceeded|token quota exceeded|quota exceeded|payment required|http 402|402\b/i.test(message);
    }
    ...
    function renderEntityDefinitionResponse(turn: ChatTurn, response: NonNullable<ChatTurn['responses']>[number], idx: number) {
        ...
        {segments.map((seg, segIdx) => {
                ...
                if (seg.type === 'entityDefinition') {
                    ...
                    return (<div key={segIdx} id={`conv-panel-entity-def-segment-${turn.id}-${idx}-${seg.entityDefinitionIdx}`} className="flex flex-col gap-1">
                ...
                {savedEntityId ? (<Link id={`conv-panel-entity-link-${turn.id}-${idx}-${seg.entityDefinitionIdx}`} href={`/games/${gameId}/entities?expanded=${savedEntityId}`} className="self-start inline-flex items-center gap-1 rounded-full border border-sky-500/30 bg-sky-500/10 text-sky-400 hover:bg-sky-500/20 transition-colors px-2 py-0.5 text-[10px] max-w-[240px]" title={entityName}>
                  ...
                  </Link>) : (<button id={`conv-panel-save-entity-btn-${turn.id}-${idx}-${seg.entityDefinitionIdx}`} onClick={() => onSaveEntityDefinition(seg.entityDefinition, turn.id, idx, seg.entityDefinitionIdx)} className="self-start inline-flex items-center gap-1 rounded border px-2 py-0.5 text-[11px] text-muted-foreground hover:text-foreground hover:bg-accent transition-colors my-[10px]">
                      ...
                      {response.error ? (<div id={`conv-panel-ai-response-error-wrap-${turn.id}-${idx}`} className="flex items-center gap-2 flex-wrap">
                          ...
                          {isTokenQuotaError(response.error) ? (<>
                            ...
                            {!response.done && (<Loader2 id={`conv-panel-ai-response-cursor-${turn.id}-${idx}`} className="h-3 w-3 animate-spin text-muted-foreground"/>)}
                                ...
                                return savedPresetId ? (<Link id={`conv-panel-preset-link-${turn.id}-${idx}-${seg.presetIdx}`} href={`/games/${gameId}/items?tab=preset&q=${savedPresetId}`} className="self-start inline-flex items-center gap-1 rounded-full border border-amber-500/30 bg-amber-500/10 text-amber-400 hover:bg-amber-500/20 transition-colors px-2 py-0.5 text-[10px] max-w-[240px]" title={presetName}>
                                      ...
                                      </Link>) : (<button id={`conv-panel-save-preset-btn-${turn.id}-${idx}-${seg.presetIdx}`} onClick={() => onSavePresetDefinition(seg.preset, turn.id, idx, seg.presetIdx)} className="self-start inline-flex items-center gap-1 rounded border px-2 py-0.5 text-[11px] text-muted-foreground hover:text-foreground hover:bg-accent transition-colors my-[10px]">
                            ...
                            {!response.done && (<Loader2 id={`conv-panel-ai-response-cursor-${turn.id}-${idx}`} className="h-3 w-3 animate-spin text-muted-foreground"/>)}
                          </div>) : response.intentType === 'container_creating' ? (<div id={`conv-panel-ai-response-containers-${turn.id}-${idx}`} className="flex flex-col gap-1">
                            {splitContainerResponseSegments(response.responseText).map((seg, segIdx) => seg.type === 'text' ? (<div key={segIdx} id={`conv-panel-container-segment-text-${turn.id}-${idx}-${segIdx}`} className={`prose prose-sm max-w-none text-xs break-words [&>*:first-child]:mt-0 [&>*:last-child]:mb-0 [&_h1]:text-sm [&_h2]:text-sm [&_h3]:text-xs [&_h4]:text-xs [&_h5]:text-xs [&_h6]:text-xs [&_p]:text-xs [&_p]:break-words [&_li]:text-xs [&_li]:break-words [&_a]:break-all${resolvedTheme?.includes('dark') ? ' prose-invert' : ''}`}>
                                  <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw]} components={MARKDOWN_COMPONENTS}>
                                    {seg.text}
                                  </ReactMarkdown>
                                </div>) : seg.type === 'container' ? (<div key={segIdx} id={`conv-panel-container-segment-${turn.id}-${idx}-${seg.containerIdx}`} className="flex flex-col gap-1">
                                ...
                                return savedContainerId ? (<Link id={`conv-panel-container-link-${turn.id}-${idx}-${seg.containerIdx}`} href={`/games/${gameId}/items?tab=containers&q=${savedContainerId}`} className="self-start inline-flex items-center gap-1 rounded-full border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 transition-colors px-2 py-0.5 text-[10px] max-w-[240px]" title={containerLabel}>
                                      ...
                                      </Link>) : (<button id={`conv-panel-save-container-btn-${turn.id}-${idx}-${seg.containerIdx}`} onClick={() => onSaveContainerDefinition(seg.container, turn.id, idx, seg.containerIdx)} className="self-start inline-flex items-center gap-1 rounded border px-2 py-0.5 text-[11px] text-muted-foreground hover:text-foreground hover:bg-accent transition-colors my-[10px]">
                            ...
                            {!response.done && (<Loader2 id={`conv-panel-ai-response-cursor-${turn.id}-${idx}`} className="h-3 w-3 animate-spin text-muted-foreground"/>)}
                          </div>) : response.intentType === 'gacha_pack_creating' ? (<div id={`conv-panel-ai-response-gacha-packs-${turn.id}-${idx}`} className="flex flex-col gap-1">
                            {splitGachaPackResponseSegments(response.responseText).map((seg, segIdx) => seg.type === 'text' ? (<div key={segIdx} id={`conv-panel-gacha-pack-segment-text-${turn.id}-${idx}-${segIdx}`} className={`prose prose-sm max-w-none text-xs break-words [&>*:first-child]:mt-0 [&>*:last-child]:mb-0 [&_h1]:text-sm [&_h2]:text-sm [&_h3]:text-xs [&_h4]:text-xs [&_h5]:text-xs [&_h6]:text-xs [&_p]:text-xs [&_p]:break-words [&_li]:text-xs [&_li]:break-words [&_a]:break-all${resolvedTheme?.includes('dark') ? ' prose-invert' : ''}`}>
                                  <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw]} components={MARKDOWN_COMPONENTS}>
                                    {seg.text}
                                  </ReactMarkdown>
                                </div>) : seg.type === 'gachaPack' ? (<div key={segIdx} id={`conv-panel-gacha-pack-segment-${turn.id}-${idx}-${seg.gachaPackIdx}`} className="flex flex-col gap-1">
                                ...
                                return savedPackId ? (<Link id={`conv-panel-gacha-pack-link-${turn.id}-${idx}-${seg.gachaPackIdx}`} href={`/games/${gameId}/items?tab=gacha&q=${savedPackId}`} className="self-start inline-flex items-center gap-1 rounded-full border border-violet-500/30 bg-violet-500/10 text-violet-400 hover:bg-violet-500/20 transition-colors px-2 py-0.5 text-[10px] max-w-[240px]" title={packName}>
                                      ...
                                      </Link>) : (<button id={`conv-panel-save-gacha-pack-btn-${turn.id}-${idx}-${seg.gachaPackIdx}`} onClick={() => onSaveGachaPack(seg.gachaPack, turn.id, idx, seg.gachaPackIdx)} className="self-start inline-flex items-center gap-1 rounded border px-2 py-0.5 text-[11px] text-muted-foreground hover:text-foreground hover:bg-accent transition-colors my-[10px]">
                            ...
                            {!response.done && (<Loader2 id={`conv-panel-ai-response-cursor-gacha-${turn.id}-${idx}`} className="h-3 w-3 animate-spin text-muted-foreground"/>)}
                          </div>) : response.intentType === 'equipment_slot_generation' ? (<div id={`conv-panel-ai-response-equipment-slots-${turn.id}-${idx}`} className="flex flex-col gap-1">
                            {splitEquipmentSlotResponseSegments(response.responseText).map((seg, segIdx) => seg.type === 'text' ? (<div key={segIdx} id={`conv-panel-equipment-slot-segment-text-${turn.id}-${idx}-${segIdx}`} className={`prose prose-sm max-w-none text-xs break-words [&>*:first-child]:mt-0 [&>*:last-child]:mb-0 [&_h1]:text-sm [&_h2]:text-sm [&_h3]:text-xs [&_h4]:text-xs [&_h5]:text-xs [&_h6]:text-xs [&_p]:text-xs [&_p]:break-words [&_li]:text-xs [&_li]:break-words [&_a]:break-all${resolvedTheme?.includes('dark') ? ' prose-invert' : ''}`}>
                                  <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw]} components={MARKDOWN_COMPONENTS}>
                                    {seg.text}
                                  </ReactMarkdown>
                                </div>) : seg.type === 'equipmentSlot' ? (<div key={segIdx} id={`conv-panel-equipment-slot-segment-${turn.id}-${idx}-${seg.equipmentSlotIdx}`} className="flex flex-col gap-1">
                                ...
                                return savedSlotId ? (<Link id={`conv-panel-equipment-slot-link-${turn.id}-${idx}-${seg.equipmentSlotIdx}`} href={`/games/${gameId}/items?tab=equipments&noconvpanel=1&q=${encodeURIComponent(displayKey)}&subtab=list`} className="self-start inline-flex items-center gap-1 rounded-full border border-orange-500/30 bg-orange-500/10 text-orange-400 hover:bg-orange-500/20 transition-colors px-2 py-0.5 text-[10px] max-w-[240px]" title={displayKey}>
                                      ...
                                      </Link>) : (<button id={`conv-panel-save-equipment-slot-btn-${turn.id}-${idx}-${seg.equipmentSlotIdx}`} onClick={() => onSaveEquipmentSlot(seg.equipmentSlot, turn.id, idx, seg.equipmentSlotIdx)} className="self-start inline-flex items-center gap-1 rounded border px-2 py-0.5 text-[11px] text-muted-foreground hover:text-foreground hover:bg-accent transition-colors my-[10px]">
                            ...
                            {!response.done && (<Loader2 id={`conv-panel-ai-response-cursor-equipment-slot-${turn.id}-${idx}`} className="h-3 w-3 animate-spin text-muted-foreground"/>)}
                          </div>) : response.intentType === 'crafting_recipe_creating' ? (<div id={`conv-panel-ai-response-crafting-recipes-${turn.id}-${idx}`} className="flex flex-col gap-1">
                            {splitCraftingRecipeResponseSegments(response.responseText).map((seg, segIdx) => seg.type === 'text' ? (<div key={segIdx} id={`conv-panel-crafting-recipe-segment-text-${turn.id}-${idx}-${segIdx}`} className={`prose prose-sm max-w-none text-xs break-words [&>*:first-child]:mt-0 [&>*:last-child]:mb-0 [&_h1]:text-sm [&_h2]:text-sm [&_h3]:text-xs [&_h4]:text-xs [&_h5]:text-xs [&_h6]:text-xs [&_p]:text-xs [&_p]:break-words [&_li]:text-xs [&_li]:break-words [&_a]:break-all${resolvedTheme?.includes('dark') ? ' prose-invert' : ''}`}>
                                  <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw]} components={MARKDOWN_COMPONENTS}>
                                    {seg.text}
                                  </ReactMarkdown>
                                </div>) : seg.type === 'craftingRecipe' ? (<div key={segIdx} id={`conv-panel-crafting-recipe-segment-${turn.id}-${idx}-${seg.craftingRecipeIdx}`} className="flex flex-col gap-1">
                                ...
                                return savedRecipeId ? (<Link id={`conv-panel-crafting-recipe-link-${turn.id}-${idx}-${seg.craftingRecipeIdx}`} href={`/games/${gameId}/items?tab=crafting&expanded=${savedRecipeId}`} className="self-start inline-flex items-center gap-1 rounded-full border border-cyan-500/30 bg-cyan-500/10 text-cyan-500 hover:bg-cyan-500/20 transition-colors px-2 py-0.5 text-[10px] max-w-[240px]" title={linkedName}>
                                      ...
                                      </Link>) : (<button id={`conv-panel-save-crafting-recipe-btn-${turn.id}-${idx}-${seg.craftingRecipeIdx}`} onClick={() => onSaveCraftingRecipe(seg.craftingRecipe, turn.id, idx, seg.craftingRecipeIdx)} className="self-start inline-flex items-center gap-1 rounded border px-2 py-0.5 text-[11px] text-muted-foreground hover:text-foreground hover:bg-accent transition-colors my-[10px] disabled:opacity-40 disabled:cursor-not-allowed">
                            ...
                            {!response.done && (<Loader2 id={`conv-panel-ai-response-cursor-crafting-recipe-${turn.id}-${idx}`} className="h-3 w-3 animate-spin text-muted-foreground"/>)}
                          </div>) : response.intentType === 'entity_pool_creating' ? (<div id={`conv-panel-ai-response-entity-pools-${turn.id}-${idx}`} className="flex flex-col gap-1">
                            {splitEntityPoolResponseSegments(response.responseText).map((seg, segIdx) => seg.type === 'text' ? (<div key={segIdx} id={`conv-panel-entity-pool-segment-text-${turn.id}-${idx}-${segIdx}`} className={`prose prose-sm max-w-none text-xs break-words [&>*:first-child]:mt-0 [&>*:last-child]:mb-0 [&_h1]:text-sm [&_h2]:text-sm [&_h3]:text-xs [&_h4]:text-xs [&_h5]:text-xs [&_h6]:text-xs [&_p]:text-xs [&_p]:break-words [&_li]:text-xs [&_li]:break-words [&_a]:break-all${resolvedTheme?.includes('dark') ? ' prose-invert' : ''}`}>
                                  <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw]} components={MARKDOWN_COMPONENTS}>
                                    {seg.text}
                                  </ReactMarkdown>
                                </div>) : seg.type === 'entityPool' ? (<div key={segIdx} id={`conv-panel-entity-pool-segment-${turn.id}-${idx}-${seg.entityPoolIdx}`} className="flex flex-col gap-1">
                                ...
                                return savedPoolId ? (<Link id={`conv-panel-entity-pool-link-${turn.id}-${idx}-${seg.entityPoolIdx}`} href={`/games/${gameId}/entities?tab=pools&poolExpanded=${savedPoolId}${poolKeyValue ? `&poolKey=${encodeURIComponent(poolKeyValue)}` : ''}`} className="self-start inline-flex items-center gap-1 rounded-full border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 transition-colors px-2 py-0.5 text-[10px] max-w-[240px]" title={linkedName}>
                                        <Layers className="h-3 w-3 shrink-0"/>
                                      ...
                                      </Link>) : (<button id={`conv-panel-save-entity-pool-btn-${turn.id}-${idx}-${seg.entityPoolIdx}`} onClick={() => onSaveEntityPool(seg.entityPool, turn.id, idx, seg.entityPoolIdx)} disabled={!canSave} title={!canSave ? t('llmConversation.errorSaveEntityPool') : undefined} className="self-start inline-flex items-center gap-1 rounded border px-2 py-0.5 text-[11px] text-muted-foreground hover:text-foreground hover:bg-accent transition-colors my-[10px]">
                                        <Layers className="h-3 w-3"/>
                          ...
                          </>)) : !response.done ? (<Loader2 id={`conv-panel-ai-response-spinner-${turn.id}-${idx}`} className="h-3 w-3 animate-spin text-muted-foreground"/>) : null}

                      {!response.error && response.responseText && response.intentType === 'lore_creating' && response.done && (<div id={`conv-panel-response-actions-${turn.id}-${idx}`} className="flex flex-wrap gap-1 mt-1">
                          {(() => {
                            const linkKey = `${turn.id}:${idx}`;
                            const savedLoreId = savedLoreIds[linkKey];
                            return savedLoreId ? (<Link id={`conv-panel-lore-link-${turn.id}-${idx}`} href={`/games/${gameId}/lore?lore_id=${savedLoreId}`} className="inline-flex items-center gap-1 rounded border px-2 py-0.5 text-[11px] text-primary hover:bg-accent transition-colors max-w-[240px]" title={loreEntryTitles[savedLoreId] ?? t('llmConversation.viewLore')}>
                                <BookOpen className="h-3 w-3 shrink-0"/>
                                <span className="truncate">{loreEntryTitles[savedLoreId] ?? t('llmConversation.viewLore')}</span>
                              </Link>) : (<button id={`conv-panel-save-lore-btn-${turn.id}-${idx}`} onClick={() => onOpenLoreReview(turn, idx, response.responseText, response.entityType)} className="inline-flex items-center gap-1 rounded border px-2 py-0.5 text-[11px] text-muted-foreground hover:text-foreground hover:bg-accent transition-colors">
