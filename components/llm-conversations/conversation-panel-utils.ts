...
// ---------------------------------------------------------------------------
export function parseLoreResponse(text: string): {
    title: string;
    summary: string;
    content: string;
} {
...
export function parseGeneratedItemsResponse(text: string): unknown[] {
    ...
    if (!trimmed)
        return [];
    const looksLikeItem = (obj: Record<string, unknown>) => typeof obj.name === 'string' && (obj.category !== undefined || obj.rarity !== undefined || obj.item_code !== undefined);
    const tryParse = (input: string): unknown[] | null => {
        try {
            ...
            if (parsed && typeof parsed === 'object') {
                ...
                if (Array.isArray(record.generated_items))
                    return record.generated_items;
                if (Array.isArray(record.items))
                    return record.items;
                // Single item object — wrap in array
                if (looksLikeItem(record))
                    return [record];
            }
        }
        catch {
    ...
    for (const block of fencedBlocks) {
        ...
        if (parsed)
            fencedItems.push(...parsed);
    }
    if (fencedItems.length > 0)
        return fencedItems;
...
export type ResponseSegment = {
    ...
    type: 'gachaPack';
    text: string;
    gachaPack: Record<string, unknown>;
    gachaPackIdx: number;
} | {
    type: 'equipmentSlot';
    text: string;
    equipmentSlot: Record<string, unknown>;
    equipmentSlotIdx: number;
} | {
    type: 'craftingRecipe';
    text: string;
    craftingRecipe: Record<string, unknown>;
                ...
                if (depth === 0 && start !== -1) {
                    try {
                        ...
                        if (looksLikeItem(obj))
                            boundaries.push({ start, end: i + 1, item: obj });
                    }
                    catch { /* ignore */ }
                    start = -1;
                }
...
export function splitEntityDefinitionResponseSegments(text: string): ResponseSegment[] {
    const looksLikeEntityDefinition = (obj: Record<string, unknown>) => typeof obj.entity_key === 'string' && typeof obj.entity_type === 'string' && typeof obj.name === 'string';
    const boundaries: Array<{
        start: number;
        end: number;
        entityDefinition: Record<string, unknown>;
        ...
        for (let i = 0; i < text.length; i++) {
            ...
            if (ch === '{') {
                ...
                if (depth === 0 && start !== -1) {
                    try {
                        const obj = JSON.parse(text.slice(start, i + 1)) as Record<string, unknown>;
                        if (looksLikeEntityDefinition(obj))
                            boundaries.push({ start, end: i + 1, entityDefinition: obj });
                    }
                    catch { /* ignore */ }
                    start = -1;
                }
...
export function splitEntityPoolResponseSegments(text: string): ResponseSegment[] {
    const looksLikeEntityPool = (obj: Record<string, unknown>) => typeof obj.pool_key === 'string' &&
        ...
        for (let i = 0; i < text.length; i++) {
            ...
            if (ch === '{') {
                ...
                if (depth === 0 && start !== -1) {
                    try {
                        const obj = JSON.parse(text.slice(start, i + 1)) as Record<string, unknown>;
                        if (looksLikeEntityPool(obj))
                            boundaries.push({ start, end: i + 1, entityPool: obj });
                    }
                    catch { /* ignore */ }
                    start = -1;
                }
...
export function splitQuestDefinitionResponseSegments(text: string): ResponseSegment[] {
    const looksLikeQuestDefinition = (obj: Record<string, unknown>) => typeof obj.name === 'string' &&
        ...
        for (let i = 0; i < text.length; i++) {
            ...
            if (ch === '{') {
                ...
                if (depth === 0 && start !== -1) {
                    try {
                        const obj = JSON.parse(text.slice(start, i + 1)) as Record<string, unknown>;
                        if (looksLikeQuestDefinition(obj))
                            boundaries.push({ start, end: i + 1, questDefinition: obj });
                    }
                    catch { /* ignore */ }
                    start = -1;
                }
...
export function parseGeneratedPresetsResponse(text: string): unknown[] {
    ...
    if (!trimmed)
        return [];
    const looksLikePreset = (obj: Record<string, unknown>) => typeof obj.code_name === 'string' && typeof obj.preset_type === 'string';
    const tryParse = (input: string): unknown[] | null => {
        try {
            const parsed: unknown = JSON.parse(input);
            if (Array.isArray(parsed)) {
            ...
            if (parsed && typeof parsed === 'object') {
                const record = parsed as Record<string, unknown>;
                if (Array.isArray(record.generated_presets))
                    return record.generated_presets;
                if (Array.isArray(record.presets))
                    return record.presets;
                if (looksLikePreset(record))
                    return [record];
            }
        }
        catch { /* ignore */ }
    ...
    for (const block of fencedBlocks) {
        const parsed = tryParse(block);
        if (parsed)
            fencedPresets.push(...parsed);
    }
    if (fencedPresets.length > 0)
        return fencedPresets;
    const extracted = extractTopLevelJsonObjects(trimmed);
    if (extracted.length > 0) {
        ...
        for (let i = 0; i < text.length; i++) {
            ...
            if (ch === '{') {
                ...
                if (depth === 0 && start !== -1) {
                    try {
                        const obj = JSON.parse(text.slice(start, i + 1)) as Record<string, unknown>;
                        if (looksLikePreset(obj))
                            boundaries.push({ start, end: i + 1, preset: obj });
                    }
                    catch { /* ignore */ }
                    start = -1;
                }
...
export function parseGeneratedContainersResponse(text: string): unknown[] {
    ...
    if (!trimmed)
        return [];
    const looksLikeContainer = (obj: Record<string, unknown>) => typeof obj.name === 'string' && typeof obj.code_name === 'string' && typeof obj.container_type === 'string' &&
    ...
    const tryParse = (input: string): unknown[] | null => {
        try {
            ...
            if (parsed && typeof parsed === 'object') {
                const record = parsed as Record<string, unknown>;
                if (Array.isArray(record.generated_containers))
                    return record.generated_containers;
                if (Array.isArray(record.containers))
                    return record.containers;
                if (looksLikeContainer(record))
                    return [record];
            }
        }
        catch { /* ignore */ }
    ...
    for (const block of fencedBlocks) {
        const parsed = tryParse(block);
        if (parsed)
            fencedContainers.push(...parsed);
    }
    if (fencedContainers.length > 0)
        return fencedContainers;
    const extracted = extractTopLevelJsonObjects(trimmed);
    if (extracted.length > 0) {
        ...
        for (let i = 0; i < text.length; i++) {
            ...
            if (ch === '{') {
                ...
                if (depth === 0 && start !== -1) {
                    try {
                        const obj = JSON.parse(text.slice(start, i + 1)) as Record<string, unknown>;
                        if (looksLikeContainer(obj))
                            boundaries.push({ start, end: i + 1, container: obj });
                    }
                    catch { /* ignore */ }
                    start = -1;
                }
...
export function parseGeneratedEntityPoolsResponse(text: string): unknown[] {
        ...
        try {
            ...
            if (parsed && typeof parsed === 'object') {
                ...
                if (Array.isArray(record.pools))
                    return record.pools;
                if (Array.isArray(record.generated_pools))
                    return record.generated_pools;
                if (looksLikeEntityPool(record))
                    return [record];
            }
        }
        catch {
    ...
    for (const block of fencedBlocks) {
        const parsed = tryParse(block);
        if (parsed)
            fencedPools.push(...parsed);
    }
    if (fencedPools.length > 0)
        return fencedPools;
    const arrayCandidate = trimmed.match(/\[[\s\S]*\]/);
    if (arrayCandidate) {
        const parsed = tryParse(arrayCandidate[0]);
...
export function parseGeneratedCraftingRecipesResponse(text: string): unknown[] {
    ...
    if (!trimmed)
        return [];
    const looksLikeCraftingRecipe = (obj: Record<string, unknown>) => typeof obj.recipe_key === 'string' &&
        typeof obj.name === 'string' &&
    ...
    const tryParse = (input: string): unknown[] | null => {
        try {
            ...
            if (parsed && typeof parsed === 'object') {
                const record = parsed as Record<string, unknown>;
                if (Array.isArray(record.generated_recipes))
                    return record.generated_recipes;
                if (Array.isArray(record.recipes))
                    return record.recipes;
                if (looksLikeCraftingRecipe(record))
                    return [record];
            }
        }
        catch { /* ignore */ }
    ...
    for (const block of fencedBlocks) {
        const parsed = tryParse(block);
        if (parsed)
            fencedRecipes.push(...parsed);
    }
    if (fencedRecipes.length > 0)
        return fencedRecipes;
    const extracted = extractTopLevelJsonObjects(trimmed);
    if (extracted.length > 0) {
...
export function parseGeneratedEntityDefinitionsResponse(text: string): unknown[] {
    ...
    const tryParse = (input: string): unknown[] | null => {
        try {
            ...
            if (parsed && typeof parsed === 'object') {
                const record = parsed as Record<string, unknown>;
                if (Array.isArray(record.entity_definitions))
                    return record.entity_definitions;
                if (looksLikeEntityDefinition(record))
                    return [record];
            }
        }
        catch {
    ...
    for (const block of fencedBlocks) {
        const parsed = tryParse(block);
        if (parsed)
            fencedDefs.push(...parsed);
    }
    if (fencedDefs.length > 0)
        return fencedDefs;
    const arrayCandidate = trimmed.match(/\[[\s\S]*\]/);
    if (arrayCandidate) {
        const parsed = tryParse(arrayCandidate[0]);
        ...
        for (let i = 0; i < text.length; i++) {
            ...
            if (ch === '{') {
                ...
                if (depth === 0 && start !== -1) {
                    try {
                        const obj = JSON.parse(text.slice(start, i + 1)) as Record<string, unknown>;
                        if (looksLikeCraftingRecipe(obj))
                            boundaries.push({ start, end: i + 1, craftingRecipe: obj });
                    }
                    catch { /* ignore */ }
                    start = -1;
                }
...
export function parseGeneratedGachaPacksResponse(text: string): unknown[] {
    ...
    if (!trimmed)
        return [];
    const looksLikeGachaPack = (obj: Record<string, unknown>) => typeof obj.code_name === 'string' &&
    ...
    const tryParse = (input: string): unknown[] | null => {
        try {
            ...
            if (parsed && typeof parsed === 'object') {
                const record = parsed as Record<string, unknown>;
                if (Array.isArray(record.generated_packs))
                    return record.generated_packs;
                if (Array.isArray(record.packs))
                    return record.packs;
                if (looksLikeGachaPack(record))
                    return [record];
            }
        }
        catch { /* ignore */ }
    ...
    for (const block of fencedBlocks) {
        const parsed = tryParse(block);
        if (parsed)
            fencedPacks.push(...parsed);
    }
    if (fencedPacks.length > 0)
        return fencedPacks;
    const extracted = extractTopLevelJsonObjects(trimmed);
    if (extracted.length > 0) {
        ...
        for (let i = 0; i < text.length; i++) {
            ...
            if (ch === '{') {
                ...
                if (depth === 0 && start !== -1) {
                    try {
                        const obj = JSON.parse(text.slice(start, i + 1)) as Record<string, unknown>;
                        if (looksLikeGachaPack(obj))
                            boundaries.push({ start, end: i + 1, gachaPack: obj });
                    }
                    catch { /* ignore */ }
                    start = -1;
                }
...
export function parseGeneratedEquipmentSlotsResponse(text: string): unknown[] {
    ...
    if (!trimmed)
        return [];
    const looksLikeEquipmentSlot = (obj: Record<string, unknown>) => typeof obj.slot_key === 'string' && Array.isArray(obj.allowed_categories);
    const tryParse = (input: string): unknown[] | null => {
        try {
            const parsed: unknown = JSON.parse(input);
            if (Array.isArray(parsed)) {
            ...
            if (parsed && typeof parsed === 'object') {
                const record = parsed as Record<string, unknown>;
                if (Array.isArray(record.generated_slots))
                    return record.generated_slots;
                if (Array.isArray(record.slots))
                    return record.slots;
                if (looksLikeEquipmentSlot(record))
                    return [record];
            }
        }
        catch { /* ignore */ }
    ...
    for (const block of fencedBlocks) {
        const parsed = tryParse(block);
        if (parsed)
            fencedSlots.push(...parsed);
    }
    if (fencedSlots.length > 0)
        return fencedSlots;
    const extracted = extractTopLevelJsonObjects(trimmed);
    if (extracted.length > 0) {
        ...
        for (let i = 0; i < text.length; i++) {
            ...
            if (ch === '{') {
                ...
                if (depth === 0 && start !== -1) {
                    try {
                        const obj = JSON.parse(text.slice(start, i + 1)) as Record<string, unknown>;
                        if (looksLikeEquipmentSlot(obj))
                            boundaries.push({ start, end: i + 1, equipmentSlot: obj });
                    }
                    catch { /* ignore */ }
                    start = -1;
                }
