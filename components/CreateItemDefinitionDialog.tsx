...
function KVEditor({
  ...
  return (
    <div id="create-item-kv-editor-root" className="space-y-1">
      <Label id="create-item-kv-editor-label" className="text-xs text-muted-foreground">{label}</Label>
      {entries.map((e, i) => (
        <div id={`create-item-kv-row-${i}`} key={i} className="flex gap-1 items-center">
          <Input
            id={`create-item-kv-key-${i}`}
...
export function CreateItemDefinitionDialog({
  open,
  gameId,
  studioId,
  onCreated,
  ...
  useEffect(() => {
    if (!categoriesProp) fetchItemCategories().then(setLocalCategories).catch(() => {})
    if (!raritiesProp) fetchItemRarities().then(setLocalRarities).catch(() => {})
  ...
  useEffect(() => {
    if (open && !prevOpenRef.current && initialValues) {
      ...
      if (v.item_code) { setItemCode(v.item_code); setAutoSlug(false) }
      else if (v.name) { setItemCode(toSlugUnderscore(v.name)); setAutoSlug(true) }
      if (v.category) setCategory(v.category)
      if (v.rarity) setRarity(v.rarity)
      if (v.is_stackable !== undefined) setIsStackable(v.is_stackable)
      if (v.max_stack_size !== undefined) setMaxStack(v.max_stack_size)
      ...
      <SheetContent id="create-item-def-sheet" side="right" className="sm:max-w-[560px] flex flex-col p-0">
        ...
        <div id="create-item-def-body" className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          <div id="create-item-def-name-section" className="space-y-1">
            ...
            <Input
              id="create-item-def-name-input"
              placeholder="e.g. Iron Sword"
              value={name}
              onChange={(e) => {
                const v = e.target.value
                setName(v)
                if (autoSlug) setItemCode(toSlugUnderscore(v))
              }}
            />
          ...
          <div id="create-item-def-code-section" className="space-y-1">
            ...
            <div id="create-item-def-code-row" className="flex gap-2">
              ...
              />
              <Button
                id="create-item-def-code-slug-btn"
                type="button"
            ...
            <div id="create-item-def-category-section" className="space-y-1">
              ...
              <Select value={category} onValueChange={(v) => setCategory(v as ItemCategory)}>
                ...
                <SelectContent id="create-item-def-category-content">
                  {categories.map((c) => (
                    <SelectItem id={`create-item-def-cat-${c}`} key={c} value={c}>{prettyCategory(c)}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div id="create-item-def-rarity-section" className="space-y-1">
              ...
              <Select value={rarity} onValueChange={(v) => setRarity(v as ItemRarity)}>
                ...
                <SelectContent id="create-item-def-rarity-content">
                  {rarities.map((r) => (
                    <SelectItem id={`create-item-def-rarity-${r}`} key={r} value={r}><RarityBadge rarity={r} /></SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          <div id="create-item-def-grid-section" className="space-y-1.5">
            <div id="create-item-def-grid-row" className="grid grid-cols-2 gap-3">
              <div id="create-item-def-gridw-section" className="space-y-1">
                ...
                <Input
                  id="create-item-def-gridw-input"
                  type="number"
                  min={0}
                  value={gridW}
              ...
              <div id="create-item-def-gridh-section" className="space-y-1">
                ...
                <Input
                  id="create-item-def-gridh-input"
                  type="number"
                  min={0}
                  value={gridH}
          ...
          <div id="create-item-def-stackable-section" className="space-y-1.5">
            <div id="create-item-def-stackable-row" className="flex items-center gap-3">
              <Switch
                id="create-item-def-stackable-switch"
                checked={isStackable}
              ...
              {isStackable && (
                <div id="create-item-def-maxstack-wrap" className="relative">
                  <Input
                    id="create-item-def-maxstack-input"
                    type="number"
                    min={1}
                    value={maxStack}
                  ...
                  {maxStack && (
                    <button
                      id="create-item-def-maxstack-clear"
                      type="button"
            ...
            {errors.maxStack && (
              <p id="create-item-def-maxstack-error" className="text-xs text-destructive pl-1">{errors.maxStack}</p>
            )}
          </div>
          ...
          <div id="create-item-def-client-writable-section" className="space-y-2">
            <div id="create-item-def-client-writable-row" className="flex items-center gap-3">
              <Switch
                id="create-item-def-client-writable-switch"
            ...
            {clientWritable && (
              <p id="create-item-def-client-writable-hint" className="text-xs text-muted-foreground pl-1">
          ...
          <div id="create-item-def-update-qty-section" className="space-y-2">
            <div id="create-item-def-update-qty-row" className="flex items-center gap-3">
              <Switch
                id="create-item-def-update-qty-switch"
            ...
            {allowClientUpdateQty && (
              <p id="create-item-def-update-qty-hint" className="text-xs text-muted-foreground pl-1">
          ...
          {/* Generator Config */}
          {category === 'generator' && (
            <div id="create-item-def-gen-section" className="space-y-4">
              ...
              <div id="create-item-def-gen-collect-card" className="space-y-4 rounded-lg border p-5">
                <div id="create-item-def-gen-collect-dest-section" className="space-y-1.5">
                  ...
                  {genCollectDestination === 'inventory' && (
                    <p id="create-item-def-gen-inventory-hint" className="text-xs text-muted-foreground">{t('items.generatorInventoryHint')}</p>
                  )}
                </div>

                {genCollectDestination === 'mailbox' && (
                  <div id="create-item-def-gen-mailbox-section" className="space-y-3">
                    <div id="create-item-def-gen-mailbox-title-section" className="space-y-1.5">
                      ...
                      <Input
                        id="create-item-def-gen-mailbox-title"
                    ...
                    <div id="create-item-def-gen-mailbox-body-section" className="space-y-1.5">
                      ...
                      <Textarea
                        id="create-item-def-gen-mailbox-body"
              ...
              <div id="create-item-def-gen-interval-card" className="space-y-4 rounded-lg border p-5">
                <div id="create-item-def-gen-interval-row" className="grid grid-cols-2 gap-4">
                  <div id="create-item-def-gen-interval-section" className="space-y-1.5">
                    ...
                    <Input
                      id="create-item-def-gen-interval"
                      type="number"
                      min={1}
                      value={genInterval}
                ...
                {(() => {
                  ...
                  if (interval > 0 && ticks > 0) {
                    ...
                    const timeStr = hours > 0
                      ...
                      : `${mins}m`
                    return (
                      <div id="create-item-def-gen-offline-calc" className="rounded-md bg-muted/50 border border-dashed px-3 py-2 text-xs text-muted-foreground space-y-0.5">
              ...
              <div id="create-item-def-gen-pool-card" className="space-y-3 rounded-lg border p-5">
                <div id="create-item-def-gen-pool-header" className="flex items-center justify-between">
                  ...
                  <Button
                    id="create-item-def-gen-pool-add-btn"
                    type="button"
                    variant="outline"
                    size="sm"
                ...
                {genOutputPool.map((entry, idx) => {
                  ...
                  return (
                    <div id={`create-item-def-gen-pool-entry-${idx}`} key={idx} className="rounded-lg border bg-muted/20 p-4 space-y-3 relative">
                      <div id={`create-item-def-gen-pool-entry-header-${idx}`} className="flex items-center justify-between">
                        ...
                        {genOutputPool.length > 1 && (
                          <Button
                            id={`create-item-def-gen-pool-del-${idx}`}
                            type="button"
                            variant="ghost"
                            size="icon"
                      ...
                      <div id={`create-item-def-gen-pool-item-section-${idx}`} className="space-y-1.5">
                        ...
                        >
                          <PopoverTrigger asChild>
                            <Button
                              id={`create-item-def-gen-pool-item-btn-${idx}`}
                              variant="outline"
                              role="combobox"
                            ...
                            >
                              {entry.item_definition_id ? (
                                <span id={`create-item-def-gen-pool-item-val-${idx}`} className="truncate">
                                ...
                                <span id={`create-item-def-gen-pool-item-placeholder-${idx}`} className="text-muted-foreground">Select item…</span>
                              )}
                          ...
                          <PopoverContent id={`create-item-def-gen-pool-popover-${idx}`} className="w-[--radix-popover-trigger-width] p-0" align="start">
                            <Command shouldFilter={false}>
                              <CommandInput
                                id={`create-item-def-gen-pool-search-${idx}`}
                              ...
                              <CommandList id={`create-item-def-gen-pool-list-${idx}`}>
                                ...
                                <CommandGroup id={`create-item-def-gen-pool-group-${idx}`}>
                                  {genAllItems
                                    ...
                                    .map((d) => (
                                      <CommandItem
                                        id={`create-item-def-gen-pool-option-${idx}-${d.id}`}
                                        key={d.id}
                                        value={d.id}
                                        onSelect={() => {
                                      ...
                                      >
                                        ...
                                        {d.item_code && (
                                          <span id={`create-item-def-gen-pool-code-${idx}-${d.id}`} className="ml-2 text-xs text-muted-foreground font-mono">{d.item_code}</span>
                                        )}
                                      </CommandItem>
                                    ))}
                                </CommandGroup>
