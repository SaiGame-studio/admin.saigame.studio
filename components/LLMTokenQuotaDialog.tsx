...
function TokenPoolRow({ id, label, remaining, used, reserved, total, onTopUp }: PoolRowProps) {
  ...
  return (
    <div id={`llm-token-pool-${id}`} className="rounded-lg border px-4 py-3 space-y-2">
      {/* Header */}
...
export function LLMTokenQuotaDialog({ game }: Props) {
  ...
  const load = async () => {
    ...
    try {
      ...
      const msg = (err as { detail?: string })?.detail ?? String(err)
      setError(msg)
    } finally {
      setLoading(false)
    }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent id={`llm-quota-dialog-${game.id}`} className="sm:max-w-md">
        ...
        <div id={`llm-quota-body-${game.id}`} className="py-1 space-y-3">
          {loading && !balance && (
            <div id={`llm-quota-skeleton-${game.id}`} className="space-y-3">
          ...
          {error && !loading && (
            <div id={`llm-quota-error-${game.id}`} className="rounded-md bg-destructive/10 border border-destructive/30 px-4 py-3 text-sm text-destructive">
              {error}
            </div>
          )}
