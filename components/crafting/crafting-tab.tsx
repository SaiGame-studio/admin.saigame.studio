...
import type { CraftingRecipe, CreateCraftingRecipeRequest, UpdateCraftingRecipeRequest, CraftingRecipeInput, CraftingRecipeOutput, } from "@/types/crafting";
import { lsPendingCraftingRecipeCreate, lsPendingCraftingRecipeEdit, } from "@/components/llm-conversations/conversation-panel-utils";
function ItemSelector({ value, onChange, items, loading, placeholder }: {
    value: string;
      ...
      <PopoverContent className="w-[300px] p-0" align="start">
        <Command>
          ...
          <CommandList>
            ...
            {loading ? (<div className="p-4 flex justify-center"><Loader2 className="h-4 w-4 animate-spin text-muted-foreground"/></div>) : (<CommandGroup>
                {items.map(it => (<CommandItem key={it.id} value={`${it.name} ${it.id}`} onSelect={() => {
                    onChange(it.id);
                    setOpen(false);
                }} className="text-xs">
...
function buildCraftingFormFromDraft(draft: Record<string, unknown>, items: ItemDefinition[]): {
...
function buildCraftingFormFromRecipe(recipe: CraftingRecipe, items: ItemDefinition[], draft?: Record<string, unknown> | null): {
    form: CreateCraftingRecipeRequest;
    meta: {
        key: string;
        value: string;
...
export function CraftingTab({ gameId, studioId }: {
    gameId: string;
    studioId: string;
}) {
    ...
    const [expandedRecipe, setExpandedRecipe] = useState<string | null>(() => searchParams.get("expanded"));
    ...
    const [formMetaEntries, setFormMetaEntries] = useState<{
        key: string;
        value: string;
    }[]>([]);
    const fetchRecipes = useCallback(async () => {
        ...
        setError(null);
        try {
            const res = await listCraftingRecipes({ gameId }, {
                page,
                page_size: pageSize,
            });
    ...
    useEffect(() => {
        if (searchParams.get("create") !== "1")
            return;
        if (searchParams.get("tab") !== "crafting")
            return;
        ...
        try {
            ...
            if (pending.recipe && typeof pending.recipe === "object") {
                ...
                setPendingCreateContext(pending.turnId && pending.responseIdx != null && pending.craftingRecipeIdx != null
                    ? {
    ...
    useEffect(() => {
        if (searchParams.get("editFromLLM") !== "1")
            return;
        if (searchParams.get("tab") !== "crafting")
            return;
        ...
        try {
            ...
            if (pending.existingRecipe && typeof pending.existingRecipe === "object") {
                ...
                setPendingEditContext(pending.turnId && pending.responseIdx != null && pending.craftingRecipeIdx != null
                    ? {
                        turnId: pending.turnId,
                        responseIdx: Number(pending.responseIdx),
                        craftingRecipeIdx: Number(pending.craftingRecipeIdx),
    ...
    useEffect(() => {
        if (!createOpen || !pendingCreateDraft || allItems.length === 0)
            return;
    ...
    useEffect(() => {
        if (!editOpen || !editingRecipe || allItems.length === 0 || editFormHydratedRef.current)
            return;
    ...
    useEffect(() => {
        ...
        if (key === lastRecipeKeyRef.current)
            return;
        if (!key.trim()) {
    ...
    async function searchCraftingRecipeByKey(rawKey: string, options?: {
        updateUrl?: boolean;
    }) {
    ...
    return (<div className="space-y-4">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="flex flex-wrap items-end justify-end gap-2">
          <div className="flex items-center gap-1.5">
            <div className="relative w-[220px] sm:w-[260px]">
              ...
              <Input value={recipeKeyQuery} onChange={(e) => {
        ...
        }} placeholder={t('crafting.searchByKey')} className="h-8 pl-7 pr-7 text-xs"/>
              {recipeKeyQuery.trim() && (<button type="button" onClick={() => void handleClearRecipeKeySearch()} className="absolute right-1.5 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors" aria-label={t('crafting.clearButton')} title={t('crafting.clearButton')}>
                  <X className="h-3 w-3"/>
                </button>)}
            </div>
      ...
      {/* RECIPE LIST */}
      {recipeKeyLoading ? (<Card className="border-dashed">
        ...
        </Card>) : recipeKeyError ? (<Card className="border-destructive">
          <CardContent className="pt-6 text-destructive text-sm">{recipeKeyError}</CardContent>
        </Card>) : loading && !isRecipeKeySearchActive ? (<div className="space-y-3">
        ...
        </Card>) : visibleRecipes.length === 0 ? (<Card className="border-dashed">
          <CardContent className="py-16 flex flex-col items-center gap-3 text-center">
            ...
            {!isRecipeKeySearchActive && (<Button onClick={handleCreateOpen}><Plus className="h-4 w-4 mr-2"/>{t('crafting.createFirstRecipe')}</Button>)}
          </CardContent>
        </Card>) : (<div className="space-y-3">
          {visibleRecipes.map((recipe) => {
                const isExpanded = expandedRecipe === recipe.id;
                const detail = detailCache[recipe.id];
                return (<Card key={recipe.id} className={`transition-all ${!recipe.is_active ? "opacity-70" : ""}`}>
                {/* Header Row */}
                <div className="cursor-pointer select-none border-b border-transparent" onClick={() => toggleExpand(recipe.id)}>
                  <CardHeader className="py-3 px-4">
                    <div className="flex items-center gap-3">
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 text-muted-foreground hover:text-primary" title={t('crafting.viewCraftHistory')} onClick={e => { e.stopPropagation(); router.push(`/games/${gameId}/crafting/recipes/${recipe.id}/history`); }}>
                        <History className="h-4 w-4"/>
                      </Button>

                      <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 text-muted-foreground hover:text-destructive" onClick={e => { e.stopPropagation(); setDeleteRecipe(recipe); }}>
                ...
                {isExpanded && (<>
                    ...
                    <CardContent className="group/detail p-4 bg-muted/10">
                      {detailLoading === recipe.id ? (<div className="flex items-center justify-center py-8 text-muted-foreground gap-2">
                          <Loader2 className="h-4 w-4 animate-spin"/>
                          <span className="text-sm">{t('crafting.loadingDetails')}</span>
                        </div>) : detailError[recipe.id] ? (<div className="text-center font-medium text-sm text-destructive py-8">
                          {detailError[recipe.id]}
                        </div>) : detail ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-4">
                              ...
                              <div className="text-sm bg-background border rounded-md p-3 space-y-2">
                                <div className="grid grid-cols-[110px_1fr] gap-x-2 gap-y-2 items-center">
                                  ...
                                  <div className="group flex items-center gap-1 min-w-0">
                                    {editingField === "name" ? (<>
                                        <Input value={editValue} onChange={e => setEditValue(e.target.value)} className="h-7 text-xs py-0" disabled={fieldSaving} autoFocus onKeyDown={e => {
                                    if (e.key === "Enter")
                                        handleFieldSave(detail.id);
                                    if (e.key === "Escape")
                                        setEditingField(null);
                                }}/>
                                  ...
                                  <div className="group flex items-center gap-1 min-w-0">
                                    {editingField === "category" ? (<>
                                        <Input value={editValue} onChange={e => setEditValue(e.target.value)} className="h-7 text-xs py-0 w-28" disabled={fieldSaving} autoFocus onKeyDown={e => {
                                    if (e.key === "Enter")
                                        handleFieldSave(detail.id);
                                    if (e.key === "Escape")
                                        setEditingField(null);
                                  ...
                                  <div className="group flex items-center gap-1 min-w-0">
                                    {editingField === "success_rate" ? (<>
                                        <Input type="number" value={editValue} onChange={e => setEditValue(e.target.value)} className="h-7 text-xs py-0 w-28 font-mono" disabled={fieldSaving} autoFocus onKeyDown={e => {
                                    if (e.key === "Enter")
                                        handleFieldSave(detail.id);
                                    if (e.key === "Escape")
                                        setEditingField(null);
                                  ...
                                  <div className="group flex items-center gap-1 min-w-0">
                                    {editingField === "description" ? (<>
                                        <Input value={editValue} onChange={e => setEditValue(e.target.value)} className="h-7 text-xs py-0" disabled={fieldSaving} autoFocus placeholder={t('crafting.addDescriptionPlaceholder')} onKeyDown={e => {
                                    if (e.key === "Enter")
                                        handleFieldSave(detail.id);
                                    if (e.key === "Escape")
                                        setEditingField(null);
                            ...
                            <div>
                              ...
                              {editingMeta ? (<div className="space-y-2 border rounded-md p-2 bg-background">
                                  {draftMeta.map((entry, idx) => (<div key={idx} className="flex gap-2">
                                ...
                                </div>) : detail.metadata && Object.keys(detail.metadata).length > 0 ? (<div className="border rounded-md overflow-hidden bg-background">
                                  {Object.entries(detail.metadata).map(([k, v]) => (<div key={k} className="grid grid-cols-[1fr_2fr] gap-2 px-3 py-1.5 text-xs border-b last:border-b-0 hover:bg-muted/30">
                                      <span className="font-mono text-muted-foreground truncate">{k}</span>
                                      <span className="font-mono truncate">{String(v)}</span>
                                    </div>))}
                                </div>) : (<p className="text-xs text-muted-foreground italic">{t('crafting.noMetadata')}</p>)}
                            </div>
                          </div>
                          ...
                          <div className="space-y-4">
                            <div>
                              ...
                              {editingInputs ? (<div className="space-y-2 border border-rose-500/20 rounded-md p-2 bg-background">
                                  {draftInputs.map((inp, idx) => (<div key={idx} className="flex gap-2 items-center bg-muted/30 p-1.5 rounded-md">
                                      ...
                                      <div className="flex-1">
                                        ...
                                        const n = [...draftInputs];
                                        n[idx] = { ...n[idx], item_definition_id: v };
                                        setDraftInputs(n);
                                ...
                                </div>) : (<div className="border border-rose-500/20 rounded-md overflow-hidden bg-background">
                                <Table>
                                  <TableHeader>
                                  ...
                                  <TableBody>
                                    ...
                                    {(!detail.inputs || detail.inputs.length === 0) && (<TableRow>
                            ...
                            <div>
                              ...
                              {editingOutputs ? (<div className="space-y-2 border border-emerald-500/20 rounded-md p-2 bg-background">
                                  {draftOutputs.map((out, idx) => (<div key={idx} className="flex gap-2 items-center bg-muted/30 p-1.5 rounded-md flex-wrap">
                                      <Select value={out.output_type} onValueChange={v => {
                                        const n = [...draftOutputs];
                                        n[idx] = { ...n[idx], output_type: v };
                                        setDraftOutputs(n);
                                    }}>
                                      ...
                                      <div className="flex items-center gap-1 shrink-0">
                                        ...
                                        const n = [...draftOutputs];
                                        n[idx] = { ...n[idx], quantity_min: Number(e.target.value) };
                                        setDraftOutputs(n);
                                    }}/>
                                        ...
                                        const n = [...draftOutputs];
                                        n[idx] = { ...n[idx], quantity_max: Number(e.target.value) };
                                        setDraftOutputs(n);
                                    }}/>
                                      </div>
                                      <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 text-muted-foreground hover:text-destructive" onClick={() => {
                                ...
                                </div>) : (<div className="border border-emerald-500/20 rounded-md overflow-hidden bg-background">
                                <Table>
                                  <TableHeader>
                                    <TableRow className="bg-muted/40 hover:bg-muted/40">
                                  ...
                                  <TableBody>
                                    ...
                                    {(!detail.outputs || detail.outputs.length === 0) && (<TableRow>
                                        <TableCell colSpan={3} className="text-center text-muted-foreground text-xs py-3 italic">{t('common.none')}</TableCell>
                                      </TableRow>)}
                                  </TableBody>
                                </Table>
          ...
          {/* Pagination */}
          {totalPages > 1 && (<div className="flex items-center justify-between mt-4 text-sm text-muted-foreground px-2">
          ...
          <div className="px-6 py-4 flex-1 overflow-y-auto space-y-6 bg-muted/5">
            ...
            if (autoSlug) {
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2">
                    <Input value={form.recipe_key} onChange={e => {
            if (editOpen)
                return;
            setAutoSlug(false);
            setForm({ ...form, recipe_key: e.target.value });
        }} readOnly={editOpen} placeholder={t('crafting.recipeKeyPlaceholder')} className={`font-mono ${editOpen ? "bg-muted/40" : ""}`}/>
                    {!editOpen && (<Button type="button" variant={autoSlug ? "default" : "outline"} size="icon" className="shrink-0" title={autoSlug ? t('crafting.autoSlugOn') : t('crafting.autoSlugOff')} onClick={() => {
                setAutoSlug(true);
                setForm({
                    ...form,
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {form.outputs.map((out, idx) => (<div key={idx} className="flex gap-2 items-center bg-background p-2 rounded-md border shadow-sm">
                    <Select value={out.output_type} onValueChange={v => {
                const newOuts = [...form.outputs];
                newOuts[idx].output_type = v;
                ...
                setForm({ ...form, outputs: newOuts });
                    ...
                    <div className="flex items-center gap-1">
                      <Input type="number" min={1} value={out.quantity_min} className="w-14 h-8 text-xs text-center px-1" title={t('crafting.minQty')} onChange={e => {
                const newOuts = [...form.outputs];
                newOuts[idx].quantity_min = Number(e.target.value);
                setForm({ ...form, outputs: newOuts });
            }}/>
                      <span className="text-muted-foreground text-[10px]">-</span>
                      <Input type="number" min={1} value={out.quantity_max} className="w-14 h-8 text-xs text-center px-1" title={t('crafting.maxQty')} onChange={e => {
                const newOuts = [...form.outputs];
                newOuts[idx].quantity_max = Number(e.target.value);
                setForm({ ...form, outputs: newOuts });
            }}/>
                    </div>
      ...
      <AlertDialog open={!!deleteRecipe} onOpenChange={open => {
            if (!open && !deleteRecipeSaving)
                setDeleteRecipe(null);
        }}>
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90" disabled={deleteRecipeSaving} onClick={e => {
            e.preventDefault();
            if (deleteRecipe)
