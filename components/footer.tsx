...
import { Check, ChevronsUpDown, Clock, Package, Pencil } from "lucide-react";
...
export function Footer() {
    ...
    async function saveTimezone() {
        ...
        try {
            await updateUserTimezone(timezoneValue);
            await refreshUser();
    ...
    return (<footer id="app-footer" className="shrink-0 border-t py-2 px-4 text-xs text-muted-foreground flex items-center justify-between gap-3">
      ...
      {timezone && (<span id="app-footer-timezone" className="app-footer-timezone flex items-center gap-1 opacity-60" title={`Dates and times are shown in: ${timezone}`}>
          ...
          {timezoneEditing ? (<span id="app-footer-timezone-editor" className="app-footer-timezone-editor flex items-center gap-1">
              <Popover open={timezoneOpen} onOpenChange={setTimezoneOpen}>
                <PopoverTrigger asChild>
                  <Button id="app-footer-timezone-select" variant="ghost" role="combobox" className="app-footer-timezone-select h-6 gap-1 px-1 text-xs font-normal">
                  ...
                  <Command id="app-footer-timezone-command" className="app-footer-timezone-command">
                    ...
                    <CommandList id="app-footer-timezone-options" className="app-footer-timezone-options max-h-60">
                      ...
                      <CommandGroup id="app-footer-timezone-options-group" className="app-footer-timezone-options-group">
                        {ALL_TIMEZONES.map(tz => (<CommandItem id={`app-footer-timezone-option-${tz.toLowerCase().replaceAll("/", "-").replaceAll("_", "-")}`} className="app-footer-timezone-option" key={tz} value={tz} onSelect={value => { setTimezoneValue(value); setTimezoneOpen(false); }}>
              ...
              <Button id="app-footer-timezone-save-button" size="sm" className="app-footer-timezone-save-button h-6 px-1.5 text-xs" disabled={timezoneSaving} onClick={saveTimezone}>{timezoneSaving ? "Saving…" : "Save"}</Button>
              <Button id="app-footer-timezone-cancel-button" size="sm" variant="ghost" className="app-footer-timezone-cancel-button h-6 px-1.5 text-xs" onClick={() => { setTimezoneValue(timezone); setTimezoneEditing(false); }}>Cancel</Button>
            </span>) : (<>
              <span id="app-footer-timezone-value" className="app-footer-timezone-value">{timezone}</span>
              <Button id="app-footer-timezone-edit-button" size="icon" variant="ghost" className="app-footer-timezone-edit-button h-5 w-5" disabled={!user} onClick={() => { setTimezoneValue(timezone); setTimezoneEditing(true); }}>
                <Pencil id="app-footer-timezone-edit-icon" className="app-footer-timezone-edit-icon h-2 w-2"/>
              </Button>
            </>)}
        </span>)}
