    ...
    return (<div className="space-y-6">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div className="min-w-0">
          ...
          {!isSuperAdmin && (<div className="flex items-start gap-2 mt-2 rounded-md border border-yellow-500/30 bg-yellow-500/10 px-3 py-2 text-sm text-yellow-600 dark:text-yellow-400">
        ...
        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
          ...
          <SheetContent side="right" className="w-full sm:max-w-md">
            ...
            <CreateCodeForm onCreated={() => { setSheetOpen(false); fetchCodes(); }}/>
          </SheetContent>
        </Sheet>
        </div>
      </div>
      ...
      {loading ? (<div className="space-y-3">
        ...
        </div>) : codes.length === 0 ? (<div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border/60 py-16 text-muted-foreground">
        ...
        </div>) : (<div className="rounded-2xl border border-border/60 overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow>
              ...
              {codes.map((c) => (<TableRow key={c.id}>
                  ...
                  <TableCell>
                    <span className="tabular-nums text-sm whitespace-nowrap">
                      {c.current_usage}
                      {c.usage_limit > 0 && (<span className="text-muted-foreground"> / {c.usage_limit}</span>)}
                    </span>
                  </TableCell>
                  <TableCell>
                    <Switch checked={c.is_active} disabled={togglingIds.has(c.id)} onCheckedChange={() => handleToggleActive(c)}/>
                  </TableCell>
                  <TableCell>
...
function CreateCodeForm({ onCreated }: {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            if (campaign.trim())
                metadata.campaign = campaign.trim();
    ...
    return (<form onSubmit={handleSubmit} className="space-y-4">
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <Input id="rc-code" placeholder="e.g. SAI-INVITE-ABC-123-XYZ" value={code} onChange={(e) => setCode(e.target.value)} required className="flex-1"/>
          <Button type="button" variant="outline" size="icon" className="shrink-0" title="Generate random code" onClick={() => setCode(generateRandomCode())}>
      ...
      <div className="space-y-2">
        ...
        <Input id="rc-limit" type="number" min={0} value={usageLimit} onChange={(e) => setUsageLimit(e.target.value)}/>
      </div>
      <div className="space-y-2">
        <Label htmlFor="rc-campaign">Campaign (optional)</Label>
        <Input id="rc-campaign" placeholder="e.g. early_access" value={campaign} onChange={(e) => setCampaign(e.target.value)}/>
      </div>
