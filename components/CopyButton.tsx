...
export function CopyButton({ text, className, size = "h-3.5 w-3.5" }: CopyButtonProps) {
    ...
    return (<button onClick={handleCopy} className={`ml-1 inline-flex items-center text-muted-foreground hover:text-foreground transition-colors${className ? ` ${className}` : ""}`} title="Copy" type="button">
      {copied
