...
export function MermaidDiagram({ chart, className }: MermaidDiagramProps) {
    ...
    if (!svg && !error)
        return (<div className="h-32 flex items-center justify-center text-xs text-muted-foreground animate-pulse">
      Rendering diagram…
    </div>);
    if (error)
        return (<div className="text-xs text-destructive font-mono p-2 border rounded bg-destructive/5 whitespace-pre-wrap">{error}</div>);
    return (<div className={className} dangerouslySetInnerHTML={{ __html: svg }} suppressHydrationWarning/>);
}
