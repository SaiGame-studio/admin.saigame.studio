...
import { BadgeCheck, Building2, CheckCircle2, CreditCard, Gem, Loader2, MessageCircle, } from "lucide-react";
...
function getLocalizedSgemPrice(pkg: SgemPackage, locale: string, method?: PaymentMethod | null): {
    amount: number;
    currency: string;
} {
...
export function BuySGemTab() {
    ...
    return (<div id="buy-sgem-tab-root" className="mt-6 space-y-8">
      ...
      <section id="sgem-packages-section" className="space-y-3">
        ...
        {packagesLoading ? (<div id="sgem-packages-loading" className="flex items-center justify-center py-10">
            <Loader2 className="h-6 w-6 animate-spin text-muted-foreground"/>
          </div>) : packagesError ? (<Card id="sgem-packages-error-card">
            <CardContent id="sgem-packages-error-content" className="flex flex-col items-center gap-3 py-8 text-center">
              <p className="text-sm text-destructive">{t("payment.sgemPackagesError")}</p>
              <Button id="sgem-packages-retry-btn" variant="outline" size="sm" onClick={fetchPackages}>
                {t("payment.tryAgain")}
              </Button>
            </CardContent>
          </Card>) : packages.length === 0 ? (<Card id="sgem-packages-empty-card">
            <CardContent id="sgem-packages-empty-content" className="py-8 text-center text-sm text-muted-foreground">
              {t("payment.noSgemPackages")}
            </CardContent>
          </Card>) : (<div id="sgem-packages-grid" className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {packages.map((pkg) => {
                ...
                return (<Card key={pkg.id} id={`sgem-package-card-${pkg.id}`} className={`relative cursor-pointer transition-all hover:border-primary/60 ${isSelected ? "border-primary ring-2 ring-primary/30" : ""}`} onClick={() => {
                  ...
                  <CardHeader id={`sgem-package-header-${pkg.id}`} className="pb-2 pt-5">
                    ...
                    <CardDescription id={`sgem-package-desc-${pkg.id}`} className="text-xs line-clamp-2 h-[2.5rem]">
                      {pkg.description}
                    </CardDescription>
                  </CardHeader>
                  <CardContent id={`sgem-package-content-${pkg.id}`} className="pt-0 space-y-1">
                    <div id={`sgem-package-price-row-${pkg.id}`} className="flex items-end justify-between gap-2">
                      <p id={`sgem-package-amount-${pkg.id}`} className="text-2xl font-bold tabular-nums">
                        {pkg.sgem_amount.toLocaleString()}
                        <span className="text-sm font-normal text-muted-foreground ml-1">💎</span>
                      </p>
                      <p id={`sgem-package-price-${pkg.id}`} className="text-base font-semibold text-primary whitespace-nowrap pb-0.5">
      ...
      <div id="sgem-checkout-grid" className="grid gap-6 lg:grid-cols-2 lg:items-start">
        ...
        <section id="sgem-methods-section" className="space-y-3">
          ...
          {methodsLoading ? (<div id="sgem-methods-loading" className="flex items-center justify-center py-10">
              <Loader2 className="h-6 w-6 animate-spin text-muted-foreground"/>
            </div>) : methodsError ? (<Card id="sgem-methods-error-card">
              <CardContent id="sgem-methods-error-content" className="flex flex-col items-center gap-3 py-8 text-center">
                <p className="text-sm text-destructive">{t("payment.methodsError")}</p>
                <Button id="sgem-methods-retry-btn" variant="outline" size="sm" onClick={fetchMethods}>
                  {t("payment.tryAgain")}
                </Button>
              </CardContent>
            </Card>) : methods.length === 0 ? (<Card id="sgem-methods-empty-card">
              <CardContent id="sgem-methods-empty-content" className="py-8 text-center text-sm text-muted-foreground">
                {t("payment.noMethods")}
              </CardContent>
            </Card>) : (<div id="sgem-methods-list" className="grid gap-3">
              {methods.map((method) => {
                const isSelected = selectedMethod?.id === method.id;
                return (<Card key={method.id} id={`sgem-method-card-${method.id}`} className={`cursor-pointer transition-all hover:border-primary/60 ${isSelected ? "border-primary ring-2 ring-primary/30" : ""}`} onClick={() => {
                        if (isSelected) {
                    ...
                    <CardContent id={`sgem-method-content-${method.id}`} className="flex items-center gap-3 p-3 sm:gap-4 sm:p-4">
                      <div id={`sgem-method-icon-wrap-${method.id}`} className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl border bg-background shadow-sm text-foreground">
                      ...
                      <div id={`sgem-method-info-${method.id}`} className="flex-1 min-w-0">
                        ...
                        {method.description && (<p id={`sgem-method-desc-${method.id}`} className="text-xs text-muted-foreground mt-0.5 line-clamp-2">
                            {method.description}
                          </p>)}
                      </div>
                      {isSelected && (<div id={`sgem-method-selected-${method.id}`} className="rounded-full bg-primary p-0.5 text-primary-foreground shadow-md shrink-0">
                          <CheckCircle2 className="h-5 w-5"/>
                        </div>)}
                    </CardContent>
                  </Card>);
          ...
          <Card id="sgem-order-card">
            <CardContent id="sgem-order-content" className="p-4 space-y-4 sm:p-5">
              ...
              <div id="sgem-order-package-row" className="space-y-1">
                <p id="sgem-order-package-label" className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                  {t("payment.sgemPackages")}
                </p>
                {selectedPackage ? (<div id="sgem-order-package-selected" className="flex items-start justify-between gap-2">
                ...
                return formatSgemPrice(p.amount, p.currency);
                  ...
                  </div>) : (<p id="sgem-order-package-empty" className="text-sm text-muted-foreground italic">
              ...
              <div id="sgem-order-method-row" className="space-y-1">
                <p id="sgem-order-method-label" className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                  {t("payment.paymentMethods")}
                </p>
                {selectedMethod ? (<div id="sgem-order-method-selected" className="flex items-center gap-3">
                    <div id="sgem-order-method-icon" className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg border bg-muted text-foreground">
                  ...
                  </div>) : (<p id="sgem-order-method-empty" className="text-sm text-muted-foreground italic">
              ...
              <Button id="sgem-checkout-btn" className="w-full" disabled={!selectedPackage || !selectedMethod || checkingOut} onClick={handleCheckout}>
