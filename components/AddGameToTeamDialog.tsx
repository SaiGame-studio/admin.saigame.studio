...
export function AddGameToTeamDialog({ teamId, studioId, existingGameIds, onGamesAdded }: AddGameToTeamDialogProps) {
    ...
    function toggleGame(gameId: string) {
        setSelectedGameIds(prev => prev.includes(gameId)
        ...
        <SheetContent side="right" className="w-full sm:max-w-lg flex flex-col">
          <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
            ...
            <div className="py-4 flex-1 overflow-y-auto">
              {gamesLoading ? (<div className="flex items-center justify-center py-8">
                  <Loader2 className="h-6 w-6 animate-spin text-muted-foreground"/>
                </div>) : availableGames.length > 0 ? (<div className="space-y-3">
                  {availableGames.map((game) => (<div key={game.id} className="flex items-start space-x-3 p-3 border rounded-md hover:bg-accent">
                      <Checkbox id={`game-${game.id}`} checked={selectedGameIds.includes(game.id)} onCheckedChange={() => toggleGame(game.id)} disabled={loading}/>
                      <div className="flex-1">
                        <Label htmlFor={`game-${game.id}`} className="font-medium cursor-pointer">
                          {game.name}
                        </Label>
                        {game.description && (<p className="text-xs text-muted-foreground mt-1">
                            {game.description}
                          </p>)}
                        <div className="flex gap-2 mt-1">
                          {game.slug && (<span className="text-xs text-muted-foreground font-mono">
                              {game.slug}
                            </span>)}
                          {game.status && (<span className="text-xs text-muted-foreground">
                              • {game.status}
                            </span>)}
                        </div>
                      </div>
            ...
            <SheetFooter className="pt-4">
              ...
              <Button type="submit" disabled={loading || selectedGameIds.length === 0}>
