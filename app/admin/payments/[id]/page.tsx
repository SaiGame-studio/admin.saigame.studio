...
import { getGiftCode, updateGiftCode, deleteGiftCode, listGiftCodeRedemptions, type GiftCode, type GiftCodeRedemption, } from "@/lib/admin-api";
...
function getStatus(gc: GiftCode) {
    ...
    if (activeTs && activeTs > now)
        return { label: "Scheduled", className: "bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300" };
    ...
    if (gc.max_uses !== -1 && gc.used_count >= gc.max_uses)
        return { label: "Exhausted", className: "bg-orange-100 text-orange-700 dark:bg-orange-900 dark:text-orange-300" };
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_1.2fr] gap-6 items-start">
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-2 gap-4 sm:grid-cols-3">
              ...
              {gc.created_by && (<div className="col-span-2 sm:col-span-3">
              ...
              <form onSubmit={handleSave} className="space-y-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2">
                    <Input id="activeAt" type="datetime-local" value={activeAt} onChange={(e) => setActiveAt(e.target.value)} className="flex-1"/>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <Checkbox id="neverExpires" checked={neverExpires} onCheckedChange={(v) => setNeverExpires(!!v)}/>
                    <Label htmlFor="neverExpires" className="cursor-pointer font-normal">{t('adminGiftCodes.neverExpires')}</Label>
                  </div>
                  {!neverExpires && (<Input id="expiresAt" type="datetime-local" value={expiresAt} onChange={(e) => setExpiresAt(e.target.value)}/>)}
                </div>
          ...
          <div className="space-y-3">
            ...
            <Card>
              <CardContent className="p-0">
                {redemptionsLoading ? (<div className="flex items-center justify-center py-10">
                    <Loader2 className="h-5 w-5 animate-spin text-muted-foreground"/>
                  </div>) : redemptions.length === 0 ? (<p className="py-10 text-center text-sm text-muted-foreground">{t('adminGiftCodes.noRedemptions')}</p>) : (<Table>
                    <TableHeader>
                      <TableRow>
                    ...
                    <TableBody>
                      {redemptions.map((r, i) => (<TableRow key={r.user_id ?? i}>
                          <TableCell>
            ...
            {redemptionsTotal > LIMIT && (<div className="flex items-center justify-between">
                <Button variant="outline" size="sm" disabled={redemptionsOffset === 0 || redemptionsLoading} onClick={() => fetchRedemptions(Math.max(0, redemptionsOffset - LIMIT))}>
                  {t('common.previous')}
                </Button>
                <span className="text-sm text-muted-foreground">
                  {redemptionsOffset + 1}–{Math.min(redemptionsOffset + LIMIT, redemptionsTotal)} of {redemptionsTotal}
                </span>
                <Button variant="outline" size="sm" disabled={redemptionsOffset + LIMIT >= redemptionsTotal || redemptionsLoading} onClick={() => fetchRedemptions(redemptionsOffset + LIMIT)}>
                  {t('common.next')}
                </Button>
              </div>)}
          </div>
