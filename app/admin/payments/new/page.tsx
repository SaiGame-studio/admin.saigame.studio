            ...
            <CardContent className="space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  <Input id="code" placeholder={t('adminGiftCodes.codeHint')} value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} required spellCheck={false} autoComplete="off" className="font-mono"/>
              ...
              <div className="space-y-1.5">
                ...
                <Input id="coins" type="number" min={1} placeholder="e.g. 100" value={coinsAmount} onChange={(e) => setCoinsAmount(e.target.value)} required/>
              ...
              <div className="space-y-2">
                ...
                <RadioGroup value={maxUsesMode} onValueChange={(v) => setMaxUsesMode(v as MaxUsesMode)} className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {maxUsesMode === "limited" && (<Input type="number" min={2} className="h-8 w-24" value={limitedUses} onChange={(e) => setLimitedUses(e.target.value)}/>)}
                  </div>
                  <div className="flex items-center gap-2">
              ...
              <div className="space-y-1.5">
                ...
                <Textarea id="description" placeholder="Admin memo for this gift code…" value={description} onChange={(e) => setDescription(e.target.value)} required rows={3}/>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  ...
                  <Button type="button" variant="outline" size="sm" className="shrink-0" onClick={() => setActiveAt(new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 16))}>
                    Now
                  </Button>
                </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  <Input id="expiresAt" type="datetime-local" value={expiresAt} onChange={(e) => setExpiresAt(e.target.value)} className="flex-1"/>
                  <Button type="button" variant="outline" size="sm" className="shrink-0" onClick={() => setExpiresAt(new Date(Date.now() + 7 * 24 * 60 * 60 * 1000 - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 16))}>
                    +7 days
                  </Button>
                </div>
