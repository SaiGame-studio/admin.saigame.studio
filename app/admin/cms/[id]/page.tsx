...
function preprocessImgSize(md: string): string {
    return md.replace(/!\[([^\]]*)\]\((\S+?)\s+=(\d+)?[x×](\d+)?\)/g, (_match, alt, url, w, h) => {
...
import { ArrowLeft, Star, Clock, FileText, Tag, ShieldAlert, Save, Loader2, Eye, Pencil, Bold, Italic, Heading, List, ListOrdered, Link as LinkIcon, Image, Columns2, Table2, Code, Quote, Minus, X, Plus, RefreshCw, Wand2, } from "lucide-react";
...
function CmsDetailInner() {
    ...
    const [syncConfirm, setSyncConfirm] = useState<{
        ...
        key: string;
        value: string;
        targets: ContentTranslationSummary[];
    } | null>(null);
    ...
    useEffect(() => {
        function flattenTree(cats: ContentCategory[], depth = 0): {
            value: string;
            label: string;
            depth: number;
        }[] {
    ...
    useEffect(() => {
        ...
        if (id && capabilities.is_super_admin)
            load();
    ...
    const handleSave = async () => {
        const parsedMetadata: Record<string, unknown> = Object.fromEntries(metadataPairs.filter(p => p.key.trim()).map(p => [p.key.trim(), p.value]));
        setSaving(true);
        try {
    ...
    return (<div className="container mx-auto py-6">
      ...
      {content && (() => {
            ...
            <Tabs value={content.language}>
              <TabsList>
                {SUPPORTED_LANGS.map(lang => {
                    ...
                    return (<TabsTrigger key={lang} value={lang} disabled={isTranslating} className="uppercase text-xs px-3" onClick={async () => {
                            if (trans) {
                                if (trans.id === id)
                                    return;
                                if (dirty) {
                                ...
                                const doTranslate = async () => {
                                    ...
                                    try {
                                        ...
                                        if (newTrans)
                                            router.push(`/admin/cms/${newTrans.id}`);
                                    }
                                    catch (err) {
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <Switch checked={content.status === "published"} disabled={publishing} onCheckedChange={handleTogglePublish}/>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
            ...
            <div className="lg:col-span-3 space-y-2">
              <div className="flex gap-2 items-end">
                ...
                markDirty();
            }} placeholder="Title" className="text-lg font-semibold h-10"/>
                </div>
                <div className="w-48 space-y-1">
                  <Label className="text-xs text-muted-foreground flex items-center gap-1"><Tag className="h-3 w-3"/>Category</Label>
                  <select className="h-10 w-full rounded-md border border-input bg-background px-2 text-sm" value={categoryId ?? ""} onChange={(e) => {
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  <Input value={slug} onChange={(e) => { setAutoSlug(false); setSlug(e.target.value); markDirty(); }} placeholder="slug-url" className="text-sm font-mono pr-9"/>
                  <button type="button" onClick={() => setAutoSlug(!autoSlug)} className={cn("absolute right-0 top-0 h-full px-2.5 rounded-r-md border-l transition-colors", autoSlug ? "text-primary bg-primary/10" : "text-muted-foreground hover:text-foreground")} title={autoSlug ? "Auto-slug enabled (click to disable)" : "Auto-slug disabled (click to enable)"}>
              ...
              <div className="space-y-1">
                ...
                <Input value={description} onChange={(e) => { setDescription(e.target.value); markDirty(); }} placeholder="Description" className="text-sm"/>
              </div>
              ...
              <div className="flex items-center gap-1 flex-wrap border rounded-md p-1 bg-muted/30">
                ...
                <Button variant={previewMode ? "default" : "ghost"} size="sm" className="h-7 gap-1.5 text-xs" onClick={() => setPreviewMode(!previewMode)}>
                  {previewMode ? <Pencil className="h-3 w-3"/> : <Eye className="h-3 w-3"/>}
                  {previewMode ? "Edit" : "Preview"}
                </Button>
                <Button variant="ghost" size="sm" className="h-7 gap-1.5 text-xs" onClick={handleSave} disabled={saving || !dirty}>
              ...
              {previewMode ? (<Card>
                  <CardContent className={cn("pt-4 prose prose-sm max-w-none", resolvedTheme?.includes("dark") && "prose-invert")}>
                ...
                </Card>) : (<CodeMirror value={body} onChange={(val) => { setBody(val); markDirty(); }} theme={resolvedTheme?.includes("dark") ? vscodeDark : vscodeLight} extensions={[
                    markdown({ codeLanguages: languages }),
                    EditorView.lineWrapping,
                ]} onCreateEditor={(view) => setEditorView(view)} basicSetup={{
                    lineNumbers: true,
                    foldGutter: true,
                    ...
                    tabSize: 2,
                }} style={{ fontSize: "14px" }} className="rounded-lg border border-zinc-700 [&_.cm-editor]:outline-none [&_.cm-editor]:h-[80vh] [&_.cm-scroller]:overflow-auto [&_.cm-scroller]:h-full"/>)}
            </div>
            ...
            <div className="space-y-4">
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  <div className="space-y-1">
                    <Label className="text-xs text-muted-foreground">Title</Label>
                    <Input value={seoTitle} onChange={(e) => { setSeoTitle(e.target.value); markDirty(); }} placeholder="SEO title" className="h-7 text-xs"/>
                  </div>
                  <div className="space-y-1">
                    <Label className="text-xs text-muted-foreground">Description</Label>
                    <Input value={seoDescription} onChange={(e) => { setSeoDescription(e.target.value); markDirty(); }} placeholder="SEO description" className="h-7 text-xs"/>
                  </div>
                  <div className="space-y-1">
                    <Label className="text-xs text-muted-foreground">Keywords</Label>
                    <div className="flex flex-wrap gap-1 mb-1">
                      {seoKeywords.map((kw) => (<Badge key={kw} variant="secondary" className="text-xs gap-1 pr-1">
                          {kw}
                    ...
                    <div className="flex gap-1">
                      <Input value={seoKeywordInput} onChange={(e) => setSeoKeywordInput(e.target.value)} onKeyDown={(e) => {
                if (e.key === "Enter") {
                    e.preventDefault();
                    handleAddSeoKeyword();
                }
              ...
              <Card>
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    ...
                    {translations.filter(t => !t.is_root).length > 0 && metadataPairs.some(p => p.key.trim()) && (<Button variant="ghost" size="sm" className="h-6 text-xs gap-1 px-2" onClick={() => {
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {metadataPairs.map((pair, i) => (<div key={i} className="flex items-center gap-1.5">
                        <Input value={pair.key} onChange={(e) => {
                    const next = [...metadataPairs];
                    next[i] = { ...next[i], key: e.target.value };
                    setMetadataPairs(next);
                    markDirty();
                }} placeholder="key" className="h-7 text-xs w-1/2"/>
                        <Input value={pair.value} onChange={(e) => {
                    const next = [...metadataPairs];
                    next[i] = { ...next[i], value: e.target.value };
                    setMetadataPairs(next);
                    markDirty();
                }} placeholder="value" className="h-7 text-xs flex-1"/>
                        {translations.filter(t => !t.is_root).length > 0 && pair.key.trim() && (<Button variant="ghost" size="icon" className="h-7 w-7 shrink-0" title={`Sync "${pair.key}" to all translations`} disabled={!!syncingMeta[i]} onClick={() => {
                        const targets = translations.filter(t => !t.is_root);
                        if (!targets.length || !pair.key.trim())
                            return;
                        ...
                        <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0" onClick={() => { setMetadataPairs(metadataPairs.filter((_, j) => j !== i)); markDirty(); }}>
                          <X className="h-3.5 w-3.5"/>
                        </Button>
                      </div>))}
                    <Button variant="outline" size="sm" className="h-7 w-full text-xs gap-1" onClick={() => { setMetadataPairs([...metadataPairs, { key: "", value: "" }]); markDirty(); }}>
      ...
      {!loading && !error && content && (<button className="fixed bottom-6 right-6 h-10 w-10 rounded-full bg-primary text-primary-foreground shadow-lg flex items-center justify-center hover:bg-primary/90 transition-colors z-50" title="Jump to editor" onClick={() => {
      ...
      {/* Nav Confirm Dialog */}
      <Dialog open={!!navConfirm} onOpenChange={(open) => {
            if (!open)
                setNavConfirm(null);
        }}>
        <DialogContent>
      ...
      <Dialog open={!!syncConfirm} onOpenChange={(open) => {
            if (!open)
                setSyncConfirm(null);
        }}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {syncConfirm?.key === "__all__" ? (<>Sync <strong>all metadata fields</strong> to {syncConfirm?.targets.length} translation(s):{" "}
                {syncConfirm?.targets.map(t => t.language.toUpperCase()).join(", ")}.<br />
                This will overwrite their entire metadata.</>) : (<>Sync field <span className="font-mono font-semibold">&ldquo;{syncConfirm?.key}&rdquo;</span> to{" "}
          ...
          <DialogFooter>
            ...
            <Button disabled={syncConfirm ? !!syncingMeta[syncConfirm.pairIndex] : false} onClick={async () => {
            if (!syncConfirm)
                return;
