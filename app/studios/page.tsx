...
export default function StudiosPage() {
    ...
    // When referral_code_required is false, skip the activation gate entirely
    const canCreate = referralCodeRequired === false || isActivated;
    ...
    <div className="container mx-auto px-4 py-4 sm:px-6 sm:py-6">
      <div className="flex flex-col gap-4 mb-6 sm:flex-row sm:justify-between sm:items-center">
        ...
        <div className="flex flex-col items-stretch gap-2 sm:items-end">
          {!isVerified && (<div className="flex items-center gap-1.5 text-sm">
          ...
          {isVerified && !isActivated && referralCodeRequired !== false && (<div className="flex items-center gap-1.5 text-sm">
              <ShieldOff className="h-3.5 w-3.5 text-yellow-500 shrink-0"/>
              <span className="text-yellow-500 font-medium">{t('studio.notActivated')}</span>
            </div>)}
          {maxStudios !== null && (<div className="flex items-center gap-1.5 text-sm">
          ...
          <div className="flex flex-col gap-1 sm:items-end">
            <div className="flex items-center gap-2">
              <Input value={newStudioName} onChange={e => setNewStudioName(e.target.value)} placeholder={t('studio.newName')} className="flex-1 sm:w-48 sm:flex-none" disabled={creating || atLimit || !canCreate || !isVerified} onKeyDown={e => {
            if (e.key === 'Enter' && !creating && !atLimit && canCreate && isVerified) {
                handleCreateStudio();
            }
        }}/>
              <Button onClick={handleCreateStudio} disabled={creating || atLimit || !canCreate || !isVerified} variant="default" className="shrink-0" title={!isVerified ? t('studio.verifyEmailToCreate') : !canCreate ? t('studio.activateToCreate') : atLimit ? `${t('studio.studioLimitTitle')} (${usedStudios} / ${maxStudios})` : undefined}>
                {creating ? t('common.loading') : t('studio.create')}
              </Button>
            </div>
            {!atLimit && canCreate && isVerified && (<p className="text-xs text-muted-foreground">
      ...
      {atLimit && (<Alert variant="destructive" className="mb-6">
      ...
      {createError && (<Alert variant="destructive" className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <AlertCircle className="h-4 w-4"/>
            <div>
              <AlertTitle>Error</AlertTitle>
      ...
      {loading ? (<div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          ...
          <CardContent className="flex flex-col items-center justify-center py-10">
            ...
            {!isActivated && referralCodeRequired && (<ReferralCodeInput onActivated={async () => { await refreshUser(); loadStudios(); }}/>)}
          </CardContent>
        </Card>) : (<div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {studios.map((studio) => (<Card key={studio.id} className="overflow-hidden">
              <CardHeader className="pb-3 flex flex-row items-start justify-between gap-3 space-y-0">
                <div className="flex flex-col space-y-2 min-w-0 flex-1">
                  ...
                  {studio.slug && (<div className="flex items-center gap-1">
                      <Badge variant="outline" className="text-xs font-mono truncate max-w-full">{studio.slug}</Badge>
                    </div>)}
                  {studio.description && (<p className="text-sm text-muted-foreground break-words">{studio.description}</p>)}
                  <div className="flex flex-col gap-1">
                    {studio.tier && (<div className="flex items-center gap-2">
...
function ReferralCodeInput({ onActivated }: {
    onActivated: () => void;
}) {
    const { t } = useTranslation();
    ...
    return (<div className="w-full max-w-md mt-2">
      ...
      <form onSubmit={handleActivate} className="flex gap-2">
        <div className="relative flex-1">
          ...
          <Input placeholder={t('studio.enterReferralCode')} value={code} onChange={(e) => { setCode(e.target.value); setError(null); }} className="pl-9" disabled={activating || success}/>
        </div>
      ...
      {success && (<p className="text-sm text-green-500 mt-2">{t('studio.referralActivated')}</p>)}
    </div>);
}
