...
function StatChip({
  ...
  const inner = (
    <div className={`flex flex-col gap-1.5 rounded-xl border bg-card px-3 py-3 h-full transition-colors sm:px-4${href ? " hover:bg-muted/50 hover:border-primary/40 cursor-pointer group" : ""}`}>
...
function StudioCard({ studio }: { studio: Studio }) {
  ...
  return (
    <div className="rounded-2xl border border-border/60 bg-card overflow-hidden flex flex-col">
      ...
      {!loadingGames && games.length > 0 && (
        <>
          <button
            ...
            className="w-full flex items-center justify-between px-4 py-2 border-t border-border/40 text-xs text-muted-foreground hover:bg-muted/30 transition-colors sm:px-5"
          >
              ...
              {games.map((game) => {
                ...
                return (
                  <Link key={game.id} href={`/games/${game.id}`}
                    className="flex items-center gap-2 px-4 py-2.5 hover:bg-muted/30 transition-colors group sm:gap-3 sm:px-5"
                  >
      ...
      {!loadingGames && games.length === 0 && (
        <div className="px-4 py-6 text-center border-t border-border/40 sm:px-5">
...
export default function DashboardPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-4 space-y-6 sm:px-6 sm:py-6 sm:space-y-8">
