...
import { BarChart2, ChevronDown, ChevronRight, ExternalLink, Gamepad2, Layers, Moon, Plus, RefreshCw, Sparkles, Sunrise, Sunset, Sun, Users, Zap, } from "lucide-react";
...
function UsageBar({ used, max }: {
    ...
    return (<div className="mt-1 h-1 w-full rounded-full bg-muted overflow-hidden">
      <div className={`h-full rounded-full ${barColor} transition-all`} style={{ width: `${p}%` }}/>
    </div>);
}
function StatChip({ icon, label, value, sub, href, color = "text-primary", }: {
    ...
    const inner = (<div className={`flex flex-col gap-1.5 rounded-xl border bg-card px-3 py-3 h-full transition-colors sm:px-4${href ? " hover:bg-muted/50 hover:border-primary/40 cursor-pointer group" : ""}`}>
      <div className="flex items-center gap-2 text-muted-foreground text-[10px] font-semibold uppercase tracking-wide sm:text-xs">
      ...
      {sub && <div className="text-[11px] text-muted-foreground sm:text-xs">{sub}</div>}
    </div>);
    if (href)
        return <Link href={href} className="contents">{inner}</Link>;
    return inner;
}
function GameStatusDot({ status }: {
    status?: string;
}) {
...
function StudioCard({ studio }: {
    ...
    return (<div className="rounded-2xl border border-border/60 bg-card overflow-hidden flex flex-col">
      {/* Header */}
      <div className="flex items-start justify-between gap-2 p-4 pb-3 sm:gap-3 sm:p-5 sm:pb-4">
        <div className="flex items-center gap-2 min-w-0 sm:gap-3">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary font-extrabold text-base select-none sm:h-10 sm:w-10 sm:text-lg">
            {studio.name.charAt(0).toUpperCase()}
          </div>
          <div className="min-w-0">
            <Link href={`/studios/${studio.id}`} className="text-sm font-bold hover:text-primary transition-colors truncate flex items-center gap-1 sm:text-base">
      ...
      {/* Quick stats */}
      <div className="grid grid-cols-3 gap-2 px-4 pb-3 sm:px-5 sm:pb-4">
      ...
      {!loadingGames && games.length > 0 && (<>
          <button onClick={() => setExpanded((v) => !v)} className="w-full flex items-center justify-between px-4 py-2 border-t border-border/40 text-xs text-muted-foreground hover:bg-muted/30 transition-colors sm:px-5">
            <span>{expanded ? t('home.hide') : t('home.show')} {games.length} {games.length !== 1 ? t('home.games') : t('home.game')}</span>
            {expanded ? <ChevronDown className="h-3.5 w-3.5"/> : <ChevronRight className="h-3.5 w-3.5"/>}
          </button>
          {expanded && (<div className="divide-y divide-border/30 flex-1">
              {games.map((game) => {
                    const p = pct(game.usage?.concurrent_users, game.limits?.max_concurrent_users);
                    return (<Link key={game.id} href={`/games/${game.id}`} className="flex items-center gap-2 px-4 py-2.5 hover:bg-muted/30 transition-colors group sm:gap-3 sm:px-5">
                    ...
                    {game.limits?.max_concurrent_users != null && (<span className="flex items-center gap-1.5 text-xs text-muted-foreground shrink-0">
                        <Users className="h-3 w-3"/>
                        {fmt(game.usage?.concurrent_users ?? 0)}/{fmt(game.limits.max_concurrent_users)}
                        {p != null && (<span className="w-12 h-1.5 rounded-full bg-muted overflow-hidden inline-block align-middle">
      ...
      {!loadingGames && games.length === 0 && (<div className="px-4 py-6 text-center border-t border-border/40 sm:px-5">
...
export default function DashboardPage() {
    ...
    useEffect(() => {
        const interval = setInterval(() => setLocalHour(getUserLocalHour()), 60000);
    ...
    return (<div className="container mx-auto px-4 py-4 space-y-6 sm:px-6 sm:py-6 sm:space-y-8">

      {/* ── Greeting ── */}
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div className="min-w-0">
          <p className="text-sm text-muted-foreground mb-1">{greeting.icon} {greeting.text} 👋</p>
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight break-words">{displayName}</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatChip icon={<Layers className="h-4 w-4"/>} label={t('common.studios')} value={loading ? <Skeleton className="h-7 w-10 inline-block"/> : studios.length} sub={`${activeStudios} ${t('home.active')}`} href="/studios" color="text-primary"/>
        <StatChip icon={<Gamepad2 className="h-4 w-4"/>} label={t('common.games')} value={loading ? <Skeleton className="h-7 w-10 inline-block"/> : totalGamesUsed} sub={totalGamesMax > 0 ? `of ${totalGamesMax} ${t('home.capacity')}` : t('home.acrossAllStudios')} href="/games" color="text-blue-500"/>
        <StatChip icon={<Users className="h-4 w-4"/>} label={t('home.members')} value={loading ? <Skeleton className="h-7 w-10 inline-block"/> : totalMembers} sub={t('home.acrossAllStudios')} color="text-green-500"/>
        <StatChip icon={<span className="text-base leading-none">🪙</span>} label={t('home.coinBalance')} value={loading || wallet === null
            ? <Skeleton className="h-7 w-16 inline-block"/>
            : wallet.balance.toLocaleString()} sub={t('home.inYourWallet')} href="/payment" color="text-yellow-500"/>
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {user?.capabilities?.is_super_admin && (<Link href="/admin/monitor?tab=users">
      ...
      <div>
        ...
        {loading ? (<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          </div>) : studios.length === 0 ? (<div className="rounded-2xl border border-dashed border-border/60 p-12 text-center space-y-4">
