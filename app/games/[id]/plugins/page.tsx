    ...
    ? cancelledSubs.reduce(
        (acc, { subscription, plugin }) => {
          ...
          return {
            ...
            max_entity_definitions: acc.max_entity_definitions + (plugin.entity_defs_grant ?? 0) * n,
          }
        },
        { max_concurrent_users: 0, max_profiles: 0, max_items: 0, max_shops: 0, max_quests: 0, max_node_definitions: 0, max_event_types: 0, max_boards: 0, max_entity_definitions: 0 }
      )
    : null
        ...
        <div className="relative p-5 flex flex-col gap-4">
          ...
          {game && (() => {
            const statsData = [
                  ...
                  { key: "entity_defs", label: "Entity Defs", max: game.limits?.max_entity_definitions ?? null, reduction: pendingReduction?.max_entity_definitions, used: game.usage?.entity_definitions ?? 0, icon: "🧩", grantField: (p: Plugin) => p.entity_defs_grant ?? 0 },
