...
function StatusBadge({ status }: {
    ...
    return (<Badge variant={variants[status] ?? "outline"} className="text-xs capitalize">
      {labels[status] ?? status}
    </Badge>);
}
function ReadBadge({ readAt, claimedAt }: {
    readAt: string | null;
    claimedAt: string | null;
}) {
    // claimed implies read — if either timestamp is set, the message was read
    const isRead = readAt !== null || claimedAt !== null;
    return (<Badge variant={isRead ? "outline" : "secondary"} className={`text-xs flex items-center gap-1 shrink-0 ${isRead ? "text-muted-foreground" : "border-blue-400 text-blue-600 dark:text-blue-400"}`}>
...
export default function MailboxPage({ params }: {
        ...
        try {
            ...
            for (const msg of (data.messages ?? [])) {
                for (const att of (msg.attachments ?? [])) {
                    if (att.definition_id)
                        defIds.add(att.definition_id);
                }
            }
    ...
    useEffect(() => {
        if (!form.receiver_id || !isValidUUID(form.receiver_id) || selectedPlayerName)
            return;
    ...
    return (<div className="container mx-auto py-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 items-start">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Popover open={playerSearchOpen} onOpenChange={setPlayerSearchOpen}>
                  <PopoverTrigger asChild>
                    <Button ref={playerIdRef as any} type="button" variant="outline" role="combobox" aria-expanded={playerSearchOpen} className={`w-full justify-between font-normal transition-all duration-300 ${highlightPlayerId ? "ring-2 ring-primary border-primary" : ""}`}>
            ...
            : form.receiver_id
                ...
                : <span className="text-muted-foreground">{t('mailbox.placeholderPlayerId')}</span>}
                      ...
                      <div className="flex items-center gap-1 ml-2 shrink-0">
                        {form.receiver_id && (<span role="button" className="text-muted-foreground hover:text-foreground transition-colors" onClick={(e) => {
                e.stopPropagation();
                ...
                setPlayerSearchQuery("");
                  ...
                  <PopoverContent className="p-0" style={{ width: "var(--radix-popover-trigger-width)" }} align="start">
                    <Command shouldFilter={false}>
                      <CommandInput placeholder="Search player by name..." value={playerSearchQuery} onValueChange={setPlayerSearchQuery}/>
                      <CommandList>
                        {playerSearchLoading ? (<div className="py-6 text-center text-sm text-muted-foreground">Searching...</div>) : playerSearchResults.length === 0 && playerSearchQuery.trim() ? (<CommandEmpty>No players found.</CommandEmpty>) : playerSearchResults.length === 0 ? (<div className="py-6 text-center text-sm text-muted-foreground">Type a name to search players.</div>) : (<CommandGroup>
                            {playerSearchResults.map((player) => (<CommandItem key={player.id} value={player.id} onSelect={() => handleSelectPlayer(player)}>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input id="subject" placeholder={t('mailbox.placeholderSubject')} value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} className={form.subject ? "pr-8" : ""} required/>
                  {form.subject && (<button type="button" className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors" onClick={() => setForm({ ...form, subject: "" })}>
                      <X className="h-3.5 w-3.5"/>
                    </button>)}
                </div>
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Textarea id="body" placeholder={t('mailbox.placeholderBody')} value={form.body} onChange={(e) => setForm({ ...form, body: e.target.value })} rows={4} className={form.body ? "pr-8" : ""} required/>
                {form.body && (<button type="button" className="absolute right-2 top-2 text-muted-foreground hover:text-foreground transition-colors" onClick={() => setForm({ ...form, body: "" })}>
                    <X className="h-3.5 w-3.5"/>
                  </button>)}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input id="expires_in_days" type="number" min="1" max="365" value={form.expires_in_days} onChange={(e) => setForm({ ...form, expires_in_days: parseInt(e.target.value) })} required/>
              </div>
              <div className="space-y-2">
                <Label htmlFor="idempotency_key">{t('mailbox.labelIdempotencyKey')}</Label>
                <div className="relative">
                  <Input id="idempotency_key" placeholder={t('mailbox.placeholderIdempotencyKey')} value={form.idempotency_key} onChange={(e) => setForm({ ...form, idempotency_key: e.target.value })} className={form.idempotency_key ? "pr-8" : ""}/>
                  {form.idempotency_key && (<button type="button" className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors" onClick={() => setForm({ ...form, idempotency_key: "" })}>
                      <X className="h-3.5 w-3.5"/>
                    </button>)}
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              {form.attachments.map((attachment, index) => (<div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 border rounded-lg">
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Popover open={openItemDropdown === index} onOpenChange={(open) => setOpenItemDropdown(open ? index : null)}>
                      <PopoverTrigger asChild>
                        <Button variant="outline" role="combobox" aria-expanded={openItemDropdown === index} className="w-full justify-between">
                ...
                : t('mailbox.selectItem')}
                      ...
                      <PopoverContent className="w-[300px] p-0">
                        <Command>
                          <CommandInput placeholder={t('mailbox.searchItems')} onValueChange={(value) => {
                ...
                loadItemProfiles(value);
                          ...
                          <CommandList>
                            ...
                            <CommandGroup>
                              {itemProfiles.map((item) => (<CommandItem key={item.id} onSelect={(currentValue) => {
                    updateAttachment(index, "definition_id", currentValue === attachment.definition_id ? "" : item.id);
                    setOpenItemDropdown(null);
                }}>
                                  <Check className={`mr-2 h-4 w-4 ${attachment.definition_id === item.id ? "opacity-100" : "opacity-0"}`}/>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input type="number" min="1" value={attachment.quantity} onChange={(e) => updateAttachment(index, "quantity", parseInt(e.target.value))}/>
                  </div>
                  
                  <div className="flex items-end">
                    <Button type="button" variant="destructive" size="sm" onClick={() => removeAttachment(index)}>
                      {t('mailbox.remove')}
                    </Button>
                  </div>
                </div>))}
        ...
        {game && (<div className="flex items-center justify-between rounded-lg border px-4 py-3 bg-muted/30">
            <div>
          ...
          <CardHeader>
            ...
            : form.receiver_id && isValidUUID(form.receiver_id)
                ...
                : (<span className="inline-flex items-center gap-1">
                        ...
                        <button type="button" onClick={handleFocusPlayerId} title="Enter Player ID" className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 p-0.5 rounded hover:bg-muted text-muted-foreground hover:text-foreground inline-flex items-center">
                          <CornerDownLeft className="h-3.5 w-3.5"/>
                        </button>
                      </span>)}
                </CardDescription>
              </div>
              {form.receiver_id && isValidUUID(form.receiver_id) && (<Button variant="ghost" size="icon" onClick={() => fetchPlayerMailbox(form.receiver_id)} disabled={mailboxLoading} title={t('common.refresh')}>
                  <RefreshCw className={`h-4 w-4 ${mailboxLoading ? "animate-spin" : ""}`}/>
                </Button>)}
            </div>
          </CardHeader>
          <CardContent>
            {mailboxError && (<div className="p-3 bg-destructive/10 border border-destructive/30 rounded-md mb-4">
                <p className="text-destructive text-sm">{mailboxError}</p>
              </div>)}

            {mailboxLoading && !mailboxData && (<div className="flex items-center justify-center py-12 text-muted-foreground">
                <RefreshCw className="h-5 w-5 animate-spin mr-2"/>
                {t('mailbox.inboxLoadingMailbox')}
              </div>)}

            {!mailboxLoading && !mailboxData && !mailboxError && (<div className="flex flex-col items-center justify-center py-12 text-muted-foreground gap-2">
                <Inbox className="h-10 w-10 opacity-30"/>
                <p className="text-sm">{t('mailbox.inboxNoMailbox')}</p>
              </div>)}

            {mailboxData && mailboxData.messages.length === 0 && (<div className="flex flex-col items-center justify-center py-12 text-muted-foreground gap-2">
                <Inbox className="h-10 w-10 opacity-30"/>
                <p className="text-sm">{t('mailbox.inboxNoMessages')}</p>
              </div>)}

            {mailboxData && mailboxData.messages.length > 0 && (<div className="space-y-2 max-h-[680px] overflow-y-auto pr-1">
                ...
                return (<div key={msg.id} className="border rounded-lg text-sm overflow-hidden">
                      {/* Collapsed row — always visible */}
                      <button type="button" onClick={toggle} className="w-full flex items-center gap-2 px-3 py-2.5 hover:bg-muted/50 transition-colors text-left">
                        ...
                        {msg.attachments && msg.attachments.length > 0 && (<span className="inline-flex items-center gap-0.5 text-xs text-muted-foreground shrink-0">
                      ...
                      {/* Expanded detail */}
                      {isExpanded && (<div className="px-4 pb-4 pt-1 space-y-2 border-t">
                          ...
                          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                            ...
                            {msg.expires_at && (<span>{t('mailbox.detailExpires')}: <span className="font-medium text-foreground">{formatDate(msg.expires_at)}</span></span>)}
                            <span>Read: <span className="font-medium text-foreground">{msg.read_at ? formatDate(msg.read_at) : "—"}</span></span>
                            {msg.claimed_at && (<span>{t('mailbox.detailClaimed')}: <span className="font-medium text-foreground">{formatDate(msg.claimed_at)}</span></span>)}
                          </div>

                          {msg.attachments && msg.attachments.length > 0 && (<div className="pt-1">
                              <p className="text-xs font-medium text-muted-foreground flex items-center gap-1 mb-1">
                                <Package className="h-3 w-3"/>
                                {t('mailbox.detailAttachments')} ({msg.attachments.length})
                              </p>
                              <div className="flex flex-wrap gap-2">
                                {msg.attachments.map((att, i) => (<span key={i} className="inline-flex items-center gap-1 bg-muted rounded px-2 py-0.5 text-xs">
                                    <span className="capitalize">{att.type}</span>
                                    {att.definition_id && (<span className="text-muted-foreground truncate max-w-[160px]" title={att.definition_id}>
                                    ...
                                    {att.type === "item" && att.definition_id && (<a href={`/games/${gameId}/items/${att.definition_id}`} className="text-muted-foreground hover:text-foreground transition-colors" title="Open item page" onClick={(e) => e.stopPropagation()}>
