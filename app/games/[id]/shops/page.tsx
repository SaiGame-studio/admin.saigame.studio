...
import { Store, Plus, ArrowLeft, AlertCircle, Calendar, ShoppingBag, PackageSearch, Loader2, Eye, RefreshCw, Hammer, Wand2, ChevronsUpDown, Check, } from "lucide-react";
...
export default function GameShopsPage({ embedded = false, active = true }: GameShopsPageProps) {
    ...
    function validateForm(): string | null {
        if (!form.shop_key.trim())
            return "Shop key is required.";
        ...
        if (!form.currency_item_def_id?.trim())
            return "Currency Item Def ID is required.";
        if (form.shop_type === "event") {
            if (!form.starts_at)
                return "Start date is required for event shops.";
            if (!form.ends_at)
                return "End date is required for event shops.";
            if (new Date(form.ends_at) <= new Date(form.starts_at!))
                return "End date must be after start date.";
        }
        if (form.currency_item_def_id &&
            !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(form.currency_item_def_id))
    ...
    return (<div id="game-shops-page" className={embedded ? "space-y-4" : "container mx-auto py-6"}>
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap mb-4">
        ...
        <div className="flex gap-2 items-center flex-wrap">
          <div className="flex items-center gap-2 mr-2">
            <Checkbox id="active_only" checked={activeOnly} onCheckedChange={(v) => setActiveOnly(!!v)}/>
          ...
          <Button onClick={openCreate} disabled={game?.limits?.max_shops != null &&
            (game?.usage?.shops ?? 0) >= game.limits.max_shops} title={game?.limits?.max_shops != null &&
      ...
      {loading ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        </div>) : error && shops.length === 0 ? null : shops.length === 0 ? (<Card className="text-center p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {shops.map((shop) => (<Card key={shop.id}>
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <Switch checked={shop.is_active} onCheckedChange={(checked) => setPendingToggle({ shopId: shop.id, newActive: checked })} title={shop.is_active ? t('shop.deactivateShop') : t('shop.activateShop')}/>
                ...
                <CardContent className="space-y-3">
                  ...
                  {shop.description && (<p className="text-sm text-muted-foreground line-clamp-2 h-10 overflow-hidden">
                      {shop.description}
                    </p>)}
                  {!shop.description && <div className="h-10"/>}
                  {shop.shop_type === "event" && (shop.starts_at || shop.ends_at) ? (<div className="flex items-center flex-wrap gap-x-2 gap-y-0.5 text-xs text-muted-foreground">
                    ...
                    </div>) : (<div className="flex items-center gap-1 text-xs text-muted-foreground italic">
                      <Calendar className="h-3 w-3 shrink-0"/>
      ...
      {/* ── Toggle Active Confirm ──────────────────────────────── */}
      <AlertDialog open={!!pendingToggle} onOpenChange={(open) => {
            if (!open)
                setPendingToggle(null);
        }}>
        <AlertDialogContent>
        ...
        <SheetContent side="right" className="sm:max-w-[580px] flex flex-col p-0">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
            <div className="space-y-1.5">
              ...
              <Input id="shop_name" placeholder={t('shop.shopNamePlaceholder')} value={form.name} onChange={(e) => {
            const name = e.target.value;
            setField("name", name);
            if (autoSlug)
                setField("shop_key", genSlug(name));
        }}/>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-1.5">
                <Input id="shop_key" placeholder={t('shop.shopKeyPlaceholder')} value={form.shop_key} onChange={(e) => {
            setAutoSlug(false);
            setField("shop_key", e.target.value.toLowerCase());
        }} className="font-mono"/>
                <button type="button" title={autoSlug ? t('shop.autoSlugOn') : t('shop.autoSlugOff')} onClick={() => {
            const next = !autoSlug;
            setAutoSlug(next);
            if (next)
                setField("shop_key", genSlug(form.name));
            ...
            <div className="space-y-1.5">
              ...
              <Select value={form.shop_type} onValueChange={(v) => setField("shop_type", v as ShopType)}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {(Object.keys(SHOP_TYPE_LABELS) as ShopType[]).map((st) => (<SelectItem key={st} value={st}>
                      {SHOP_TYPE_LABELS[st]}
                    </SelectItem>))}
                </SelectContent>
              </Select>
            ...
            {needsDates && (<div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-1.5">
                    <Input id="starts_at" type="date" value={form.starts_at?.split("T")[0] ?? ""} onChange={(e) => {
                const time = form.starts_at?.split("T")[1] ?? "00:00";
                setField("starts_at", e.target.value ? `${e.target.value}T${time}` : "");
            }} className="flex-1"/>
                    <Input type="time" lang="en-GB" value={form.starts_at?.split("T")[1] ?? ""} onChange={(e) => {
                ...
                setField("starts_at", e.target.value ? `${date}T${e.target.value}` : "");
                  ...
                  <div className="flex gap-1.5">
                    <Button type="button" variant="outline" size="sm" className="h-7 px-2 text-xs" onClick={() => setField("starts_at", toDatetimeLocal(new Date()))}>
                      {t('shop.now')}
                    </Button>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-1.5">
                    <Input id="ends_at" type="date" value={form.ends_at?.split("T")[0] ?? ""} onChange={(e) => {
                const time = form.ends_at?.split("T")[1] ?? "23:59";
                setField("ends_at", e.target.value ? `${e.target.value}T${time}` : "");
            }} className="flex-1"/>
                    <Input type="time" lang="en-GB" value={form.ends_at?.split("T")[1] ?? ""} onChange={(e) => {
            ...
            <div className="space-y-1.5">
              ...
              <Popover open={currencyOpen} onOpenChange={setCurrencyOpen} modal={false}>
                <PopoverTrigger asChild>
                  <Button variant="outline" role="combobox" aria-expanded={currencyOpen} className="w-full justify-between font-normal" disabled={itemDefsLoading}>
                    {itemDefsLoading ? (<span className="flex items-center gap-2 text-muted-foreground">
                        <Loader2 className="h-3 w-3 animate-spin"/> Loading…
                      </span>) : form.currency_item_def_id ? (<span className="flex items-center gap-2">
                        <span>
                          {itemDefs.find((d) => d.id === form.currency_item_def_id)?.name ?? (<span className="font-mono text-xs">{form.currency_item_def_id}</span>)}
                        </span>
                        {itemDefs.find((d) => d.id === form.currency_item_def_id) && (<span className="text-xs text-muted-foreground font-mono">
                            {itemDefs.find((d) => d.id === form.currency_item_def_id)!.item_code}
                          </span>)}
                      </span>) : (<span className="text-muted-foreground">{t('shop.selectCurrencyItem')}</span>)}
                    <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50"/>
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-[--radix-popover-trigger-width] p-0" align="start" style={{ zIndex: 9999 }}>
                  <Command shouldFilter={false}>
                    <CommandInput placeholder={t('shop.searchByNameOrCode')} value={currencySearch} onValueChange={setCurrencySearch}/>
                    <CommandList>
                      <CommandEmpty>{t('shop.noItemFound')}</CommandEmpty>
                      <CommandGroup>
                        {itemDefs
            .filter((d) => (showAllItemTypes || d.category === "currency") &&
            ...
            .map((d) => (<CommandItem key={d.id} value={d.id} onSelect={() => {
              ...
              <div className="flex items-center gap-2 pt-0.5">
                <Checkbox id="show_all_types" checked={showAllItemTypes} onCheckedChange={(v) => setShowAllItemTypes(!!v)}/>
                <label htmlFor="show_all_types" className="text-xs text-muted-foreground cursor-pointer select-none">
            ...
            <div className="flex items-center gap-3">
              <Switch id="is_active" checked={form.is_active} onCheckedChange={(v) => setField("is_active", v)}/>
