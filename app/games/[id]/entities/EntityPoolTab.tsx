...
import { lsPendingEntityPoolEdit } from "@/components/llm-conversations/conversation-panel-utils";
...
export function EntityPoolTab({ gameId }: {
    gameId: string;
}) {
    ...
    function setExpandedId(id: string | null) {
        ...
        if (id)
            params.set("poolExpanded", id);
        else
            params.delete("poolExpanded");
        router.replace(`?${params.toString()}`, { scroll: false });
    }
    const [pools, setPools] = useState<EntityPool[]>([]);
    const [loading, setLoading] = useState(true);
    const [refreshing, setRefreshing] = useState(false);
    const [searchQuery, setSearchQuery] = useState(() => searchParams.get("poolKey") ?? "");
    ...
    const [editForm, setEditForm] = useState<CreatePoolFormState>(emptyCreateForm());
    const [editEntries, setEditEntries] = useState<CreatePoolEntryRow[]>([]);
    ...
    const [editMetaKey, setEditMetaKey] = useState<string | "__new__" | null>(null);
    const [editMetaFieldKey, setEditMetaFieldKey] = useState("");
    ...
    useEscapeLayer(editOpen, () => setEditOpen(false), 1);
    ...
    function newEntryRow(partial?: Partial<CreatePoolEntryRow>): CreatePoolEntryRow {
        return {
            ...
            existingEntryId: partial?.existingEntryId,
    ...
    function sanitizeJsonValue(value: unknown, seen = new WeakSet<object>()): unknown {
        ...
        if (Array.isArray(value)) {
            return value.map((item) => sanitizeJsonValue(item, seen)).filter((item) => item !== undefined);
        }
        if (valueType !== "object")
            return undefined;
    ...
    function startEditMetaEdit(key: string, currentValue: unknown) {
        setEditMetaKey(key);
        setEditMetaFieldKey(key);
        setEditMetaFieldValue(String(currentValue ?? ""));
    }
    function startEditAddMeta() {
        setEditMetaKey("__new__");
        setEditMetaFieldKey("");
        setEditMetaFieldValue("");
    }
    function cancelEditMetaEdit() {
        setEditMetaKey(null);
        setEditMetaFieldKey("");
        setEditMetaFieldValue("");
    }
    function saveEditMeta() {
        const key = editMetaFieldKey.trim();
        if (!key)
            return;
        const raw = editMetaFieldValue;
        const num = Number(raw);
        const val = raw.trim() !== "" && !isNaN(num) ? num : raw;
        setEditForm((prev) => {
            const next = { ...(prev.metadata ?? {}) };
            if (editMetaKey !== "__new__" && editMetaKey && editMetaKey !== key) {
                delete next[editMetaKey];
            }
            next[key] = val;
    ...
    function deleteEditMeta(key: string) {
        setEditForm((prev) => {
            const next = { ...(prev.metadata ?? {}) };
            delete next[key];
            return { ...prev, metadata: next };
    ...
    function deleteCreateMeta(key: string) {
        ...
        if (createMetaKey === key)
            cancelCreateMetaEdit();
    }
    function updateEditEntry(id: string, patch: Partial<CreatePoolEntryRow>) {
        setEditEntries(prev => prev.map(row => row.id === id ? { ...row, ...patch } : row));
    }
    function addEditEntry() {
        setEditEntries(prev => {
            if (prev.length >= MAX_POOL_CREATE_ENTRIES)
                return prev;
            return [...prev, newEntryRow()];
        });
    }
    function removeEditEntry(id: string) {
        setEditEntries(prev => prev.filter(row => row.id !== id));
    }
    function poolEntryToRow(entry: EntityPoolEntry): CreatePoolEntryRow {
        return newEntryRow({
            existingEntryId: entry.id,
            entity_definition_id: entry.entity_definition_id,
            entity_definition_name: entry.entity_name,
            entity_definition_key: entry.entity_key,
            weight: String(entry.weight),
        });
    }
    function draftMetadataFromSource(source?: Record<string, unknown>) {
        const rawMetadata = source?.metadata;
        const sanitized = sanitizeJsonObject(rawMetadata);
    ...
    async function resolveCreateEntryDraft(entry: Record<string, unknown>) {
        ...
        const candidateKey = rawId.startsWith("__REF:") ? rawId.slice("__REF:".length).trim()
            : rawKey || rawId;
        if (candidateKey) {
            try {
    ...
    async function openCreateWithForm(draft?: Record<string, unknown>, context?: {
        turnId: string;
        responseIdx: number;
        entityPoolIdx: number;
        convId: string;
        ...
        const metadata = rawMetadata && typeof rawMetadata === 'object' && !Array.isArray(rawMetadata)
            ? sanitizeJsonObject(rawMetadata)
        ...
        const nextEntries = draft && Array.isArray(draft.entries)
            ? await Promise.all(draft.entries
    ...
    async function openEditWithForm(poolId: string, draft?: Record<string, unknown>, context?: {
        ...
        const existingPool = pools.find((p) => p.id === poolId) ?? await getEntityPool(gameId, poolId);
        const existingEntryIdsByDefinitionId = new Map<string, string>();
        for (const entry of existingPool.entries ?? []) {
            const key = String(entry.entity_definition_id ?? '').trim();
            if (key && !existingEntryIdsByDefinitionId.has(key)) {
                existingEntryIdsByDefinitionId.set(key, entry.id);
            }
        }
        const sourceMetadata = draftMetadataFromSource(draft);
        const fallbackMetadata = existingPool.metadata ?? {};
        const metadata = Object.keys(sourceMetadata).length > 0 ? sourceMetadata : fallbackMetadata;
        const description = typeof draft?.description === 'string' ? draft.description
            : typeof existingPool.description === 'string' ? existingPool.description
                : "";
        const nextForm = {
            pool_key: existingPool.pool_key,
            name: typeof draft?.name === 'string' && draft.name.trim() ? draft.name : existingPool.name,
            description,
            is_active: typeof draft?.is_active === 'boolean' ? draft.is_active : existingPool.is_active,
            metadata,
        };
        const nextEntries = Array.isArray(draft?.entries) && draft.entries.length > 0
            ? await Promise.all(draft.entries
                .filter((entry) => entry && typeof entry === 'object' && !Array.isArray(entry))
                .slice(0, MAX_POOL_CREATE_ENTRIES)
                .map(async (entry) => {
                const row = await resolveCreateEntryDraft(entry as Record<string, unknown>);
                const existingEntryId = existingEntryIdsByDefinitionId.get(row.entity_definition_id.trim());
                return existingEntryId ? { ...row, existingEntryId } : row;
            }))
            : ((existingPool.entries ?? []).slice(0, MAX_POOL_CREATE_ENTRIES).map((entry) => poolEntryToRow(entry)));
        setSheetMode("edit");
        setEditingPoolId(poolId);
        setEditForm(nextForm);
        cancelEditMetaEdit();
        setEditEntries(nextEntries.length > 0 ? nextEntries : [newEntryRow()]);
        setAutoSlug(false);
        setCreateEntityConvContext(context);
        setCreateOpen(false);
        setEditOpen(true);
    ...
    useEffect(() => {
        const urlPoolKey = searchParams.get("poolKey") ?? "";
        setSearchQuery((prev) => (prev === urlPoolKey ? prev : urlPoolKey));
    }, [searchParams]);
    useEffect(() => {
    ...
    useEffect(() => {
        function consumePendingEntityPoolEdit() {
            const pendingRaw = safeGetItem(lsPendingEntityPoolEdit(gameId));
            if (!pendingRaw)
                return;
            try {
                const detail = JSON.parse(pendingRaw) as Record<string, unknown>;
                safeRemoveItem(lsPendingEntityPoolEdit(gameId));
                const poolId = typeof detail.existingPoolId === 'string'
                    ? detail.existingPoolId
                    : (typeof detail.poolId === 'string' ? detail.poolId : '');
                if (!poolId)
                    return;
                void openEditWithForm(poolId, detail, {
                    turnId: typeof detail.turnId === 'string' ? detail.turnId : '',
                    responseIdx: typeof detail.responseIdx === 'number' ? detail.responseIdx : Number(detail.responseIdx ?? 0),
                    entityPoolIdx: typeof detail.entityPoolIdx === 'number' ? detail.entityPoolIdx : Number(detail.entityPoolIdx ?? 0),
                    convId: typeof detail.convId === 'string' ? detail.convId : '',
        ...
        function handleOpenEditEntityPool(e: Event) {
            const detail = (e as CustomEvent).detail as Record<string, unknown> | undefined;
            if (!detail)
                return;
            const poolId = typeof detail.existingPoolId === 'string'
        ...
        window.addEventListener('ss:open-edit-entity-pool', handleOpenEditEntityPool as EventListener);
        if (searchParams.get("tab") === "pools" && searchParams.get("editPool")) {
            consumePendingEntityPoolEdit();
        }
        return () => {
            window.removeEventListener('ss:open-edit-entity-pool', handleOpenEditEntityPool as EventListener);
        };
    }, [gameId, searchParams, pools]);
    ...
    const handleCreatePool = async () => {
        if (!createForm.pool_key.trim() || !createForm.name.trim())
            return;
        ...
        try {
            ...
            const sanitizedMetadata = sanitizeJsonObject(createForm.metadata);
            if (Object.keys(sanitizedMetadata).length > 0 || createForm.is_active === false) {
            ...
            toast({ title: t('common.added'), description: t('entity.poolCreated').replace('{name}', created.name) });
            resetPoolSheet();
        }
        catch (err) {
    ...
    const handleEditPool = async () => {
        if (!editingPoolId)
            return;
        if (!editForm.pool_key.trim() || !editForm.name.trim())
            return;
        ...
        setCreating(true);
        try {
            const existingPool = await getEntityPool(gameId, editingPoolId);
            const existingEntries = Array.isArray(existingPool.entries) ? existingPool.entries : [];
            const pendingDeletes = new Set(existingEntries.map((entry) => entry.id));
            await updateEntityPool(gameId, editingPoolId, {
                name: editForm.name.trim(),
                description: editForm.description.trim() || undefined,
                ...(Object.keys(sanitizeJsonObject(editForm.metadata)).length > 0 ? { metadata: editForm.metadata } : {}),
                ...(editForm.is_active === false ? { is_active: false } : { is_active: true }),
            });
            for (const entry of editEntries) {
                ...
                if (entry.existingEntryId) {
                    const existingEntry = existingEntries.find((row) => row.id === entry.existingEntryId);
                    if (existingEntry) {
                        if (existingEntry.entity_definition_id === entity_definition_id) {
                            pendingDeletes.delete(existingEntry.id);
                            await updateEntityPoolEntry(gameId, editingPoolId, existingEntry.id, { weight });
                            continue;
                        }
                        pendingDeletes.delete(existingEntry.id);
                        await deleteEntityPoolEntry(gameId, editingPoolId, existingEntry.id);
                    }
                }
                await createEntityPoolEntry(gameId, editingPoolId, { entity_definition_id, weight });
            }
            for (const entryId of pendingDeletes) {
                await deleteEntityPoolEntry(gameId, editingPoolId, entryId);
            }
            const refreshed = await getEntityPool(gameId, editingPoolId);
            setPools(prev => prev.map(pool => pool.id === editingPoolId ? refreshed : pool));
            if (createEntityConvContext?.convId && createEntityConvContext.turnId && createEntityConvContext.responseIdx != null && createEntityConvContext.entityPoolIdx != null) {
                const poolKey = `${createEntityConvContext.turnId}:${createEntityConvContext.responseIdx}:${createEntityConvContext.entityPoolIdx}`;
                const convId = createEntityConvContext.convId;
                const existingRaw = safeGetItem(lsEntityPoolLinks(convId));
                ...
                safeSetItem(lsEntityPoolLinks(convId), JSON.stringify({ ...mapped, [poolKey]: refreshed.id }));
                const existingNamesRaw = safeGetItem(lsEntityPoolNames(convId));
                let namesMap: Record<string, string> = {};
                if (existingNamesRaw) {
                    try {
                ...
                safeSetItem(lsEntityPoolNames(convId), JSON.stringify({ ...namesMap, [refreshed.id]: refreshed.name }));
                void linkConversationContent(gameId, convId, 'entity_pool', refreshed.id)
                    .then(() => {
                    window.dispatchEvent(new CustomEvent('ss:conv-external-created', {
                        detail: { convId, gameId },
                    }));
                    window.dispatchEvent(new CustomEvent('ss:conv-content-linked', {
                        detail: { convId, gameId, contentType: 'entity_pool', contentId: refreshed.id, contentName: refreshed.name },
                    }));
                })
                    .catch(() => { });
            }
            toast({ title: t('common.saved'), description: t('entity.poolUpdated').replace('{name}', refreshed.name) });
            resetPoolSheet();
        }
        catch (err) {
            const msg = err instanceof ApiError ? err.message : t('entity.failedSave');
    ...
    return (<div className="space-y-4 animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="relative">
            ...
            <Input placeholder={t('entity.poolSearchPlaceholder')} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 h-9 w-64 text-sm bg-muted/50 border-none focus-visible:ring-1"/>
            {searchQuery && (<button className="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors" onClick={() => setSearchQuery("")}>
                <X className="h-4 w-4"/>
              </button>)}
          </div>
          ...
          <Button size="sm" className="h-9 gap-2" onClick={() => openCreateWithForm()} disabled={pools.length >= MAX_POOLS_PER_GAME} title={pools.length >= MAX_POOLS_PER_GAME ? t('entity.poolLimitReached').replace('{max}', String(MAX_POOLS_PER_GAME)) : undefined}>
      ...
      <Card className="border-none shadow-md bg-card/50 backdrop-blur-sm overflow-hidden">
        <CardContent className="p-0">
          {loading ? (<div className="p-6 space-y-4">
            ...
            </div>) : filteredPools.length === 0 ? (<div className="py-20 text-center text-muted-foreground">
              ...
              <Table>
                ...
                return (<Fragment key={pool.id}>
                        <TableRow className={`group cursor-pointer transition-colors border-muted/30 ${isExpanded ? "bg-muted/40" : "hover:bg-muted/20"}`} onClick={(e) => toggleExpand(pool.id, e)}>
                          ...
                          <TableCell className="text-center">
                            <Switch checked={pool.is_active} onClick={(e) => handleToggleActive(pool, e)}/>
                          </TableCell>
                        ...
                        {isExpanded && (<TableRow className="bg-muted/10 hover:bg-muted/10 border-none">
      ...
      <Sheet open={createOpen} onOpenChange={(open) => {
        ...
        <SheetContent className="flex h-full flex-col overflow-hidden" style={{ width: "700px", maxWidth: "90vw" }}>
          <SheetHeader>
            <SheetTitle>{sheetMode === "edit" ? t('entity.editPool') : t('entity.newPool')}</SheetTitle>
            <SheetDescription>
              {sheetMode === "edit" ? t('entity.poolKeyReadonly') : t('entity.poolEntriesHint')}
            </SheetDescription>
          </SheetHeader>
          <ScrollArea className="flex-1 pr-4">
            ...
            <div className="space-y-2">
              ...
              <Input placeholder={t('entity.poolNamePlaceholder')} value={createForm.name} maxLength={MAX_POOL_NAME_LENGTH} onChange={(e) => {
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Input placeholder={t('entity.poolKeyPlaceholder')} value={createForm.pool_key} onChange={(e) => {
            setAutoSlug(false);
            setCreateForm(f => ({ ...f, pool_key: e.target.value }));
        }} disabled={creating || sheetMode === "edit"} className="font-mono"/>
                <Button type="button" variant={autoSlug ? "default" : "outline"} size="icon" className="shrink-0" title={autoSlug ? t('entity.autoSlugOn') : t('entity.autoSlugOff')} disabled={creating || sheetMode === "edit"} onClick={() => {
            const next = !autoSlug;
            setAutoSlug(next);
            if (next) {
            ...
            <div className="space-y-2">
              ...
              <Textarea placeholder={t('entity.optionalDesc')} value={createForm.description} maxLength={MAX_POOL_DESC_LENGTH} onChange={(e) => setCreateForm(f => ({ ...f, description: e.target.value.slice(0, MAX_POOL_DESC_LENGTH) }))} disabled={creating} rows={3}/>
            ...
            <div className="flex items-center justify-between gap-3 rounded-md border border-muted/40 bg-muted/20 px-3 py-2">
              ...
              <Switch checked={createForm.is_active} onCheckedChange={(checked) => setCreateForm((f) => ({ ...f, is_active: checked }))} disabled={creating}/>
            </div>
            <div className="space-y-2">
              <Label>{t('entity.fieldMetadata')}</Label>
              <div className="space-y-0.5">
                {Object.entries(createForm.metadata ?? {}).map(([k, v]) => (<div key={k} className="group/meta">
                    {createMetaKey === k ? (<div className="flex items-center gap-1.5 py-0.5">
                      ...
                      </div>) : (<div className="flex items-center gap-1.5 py-0.5 px-1 rounded hover:bg-muted/50">
                ...
                {createMetaKey === "__new__" ? (<div className="flex items-center gap-1.5 py-0.5 mt-1">
                    ...
                    <Button size="icon" variant="ghost" className="h-6 w-6 shrink-0" onClick={cancelCreateMetaEdit} disabled={creating}><X className="w-3.5 h-3.5"/></Button>
                  </div>) : (<Button variant="ghost" size="sm" className="h-6 text-xs gap-1 px-2 mt-1" onClick={startCreateAddMeta} disabled={creating || createMetaKey !== null}>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-3">
                {createEntries.length === 0 ? (<div className="rounded-md border border-dashed border-muted-foreground/30 bg-muted/20 px-3 py-4 text-sm text-muted-foreground">
                    {t('entity.noEntriesInPool')}
                  </div>) : (createEntries.map((row, idx) => (<CreatePoolEntryRow key={row.id} gameId={gameId} row={row} index={idx} disabled={creating} canRemove={createEntries.length > 1} onChange={(patch) => updateCreateEntry(row.id, patch)} onRemove={() => removeCreateEntry(row.id)}/>)))}
              </div>
              <Button type="button" variant="outline" size="sm" className="w-full gap-2" onClick={addCreateEntry} disabled={creating || createEntries.length >= MAX_POOL_CREATE_ENTRIES} title={createEntries.length >= MAX_POOL_CREATE_ENTRIES ? t('entity.entriesLimitReached').replace('{max}', String(MAX_POOL_CREATE_ENTRIES)) : undefined}>
                <Plus className="h-4 w-4"/>
                {t('entity.addEntry')}
              </Button>
            </div>
      ...
      <Sheet open={editOpen} onOpenChange={(open) => {
        ...
        <SheetContent className="flex h-full flex-col overflow-hidden" style={{ width: "700px", maxWidth: "90vw" }}>
          <SheetHeader>
            <SheetTitle>{t('entity.editPool')}</SheetTitle>
            <SheetDescription>{t('entity.poolKeyReadonly')}</SheetDescription>
          </SheetHeader>
          <ScrollArea className="flex-1 pr-4">
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <Label>{t('entity.poolName')} <span className="text-destructive">*</span></Label>
                <Input placeholder={t('entity.poolNamePlaceholder')} value={editForm.name} maxLength={MAX_POOL_NAME_LENGTH} onChange={(e) => {
            const v = e.target.value.slice(0, MAX_POOL_NAME_LENGTH);
            setEditForm(f => ({ ...f, name: v }));
        }} disabled={creating}/>
                <p className={`text-xs text-right ${editForm.name.length >= MAX_POOL_NAME_LENGTH ? "text-destructive" : "text-muted-foreground"}`}>
                  {editForm.name.length}/{MAX_POOL_NAME_LENGTH}
                </p>
              </div>
              <div className="space-y-2">
                <Label>{t('entity.poolKey')} <span className="text-destructive">*</span></Label>
                <Input placeholder={t('entity.poolKeyPlaceholder')} value={editForm.pool_key} disabled className="font-mono"/>
              </div>
              <div className="space-y-2">
                <Label>{t('entity.fieldDescription')}</Label>
                <Textarea placeholder={t('entity.optionalDesc')} value={editForm.description} maxLength={MAX_POOL_DESC_LENGTH} onChange={(e) => setEditForm(f => ({ ...f, description: e.target.value.slice(0, MAX_POOL_DESC_LENGTH) }))} disabled={creating} rows={3}/>
                <p className={`text-xs text-right ${editForm.description.length >= MAX_POOL_DESC_LENGTH ? "text-destructive" : "text-muted-foreground"}`}>
                  {editForm.description.length}/{MAX_POOL_DESC_LENGTH}
                </p>
              </div>
              <div className="flex items-center justify-between gap-3 rounded-md border border-muted/40 bg-muted/20 px-3 py-2">
                <div className="space-y-0.5">
                ...
                <Switch checked={editForm.is_active} onCheckedChange={(checked) => setEditForm((f) => ({ ...f, is_active: checked }))} disabled={creating}/>
              </div>
              <div className="space-y-2">
                <Label>{t('entity.fieldMetadata')}</Label>
                <div className="space-y-0.5">
                  {Object.entries(editForm.metadata ?? {}).map(([k, v]) => (<div key={k} className="group/meta">
                      {editMetaKey === k ? (<div className="flex items-center gap-1.5 py-0.5">
                          <Input value={editMetaFieldKey} onChange={(e) => setEditMetaFieldKey(e.target.value)} placeholder={t('entity.metaKeyPlaceholder')} className="h-7 text-xs w-32 font-mono" disabled={creating}/>
                          <span className="text-muted-foreground text-xs">:</span>
                          <Input value={editMetaFieldValue} onChange={(e) => setEditMetaFieldValue(e.target.value)} placeholder={t('entity.metaValuePlaceholder')} className="h-7 text-xs flex-1 font-mono" disabled={creating}/>
                          <Button size="icon" variant="ghost" className="h-6 w-6 shrink-0" onClick={saveEditMeta} disabled={creating}>
                            <Save className="w-3.5 h-3.5"/>
                          </Button>
                          <Button size="icon" variant="ghost" className="h-6 w-6 shrink-0" onClick={cancelEditMetaEdit} disabled={creating}><X className="w-3.5 h-3.5"/></Button>
                        </div>) : (<div className="flex items-center gap-1.5 py-0.5 px-1 rounded hover:bg-muted/50">
                          <span className="text-xs font-mono text-muted-foreground w-32 truncate shrink-0">{k}</span>
                          <span className="text-xs text-muted-foreground">:</span>
                          <span className="text-xs font-mono flex-1">{String(v)}</span>
                          <Button size="icon" variant="ghost" className="h-5 w-5 shrink-0 opacity-0 group-hover/meta:opacity-100 transition-opacity text-muted-foreground hover:text-destructive" onClick={() => deleteEditMeta(k)} disabled={creating}><X className="w-3 h-3"/></Button>
                        </div>)}
                    </div>))}
                  {editMetaKey === "__new__" ? (<div className="flex items-center gap-1.5 py-0.5 mt-1">
                      <Input value={editMetaFieldKey} onChange={(e) => setEditMetaFieldKey(e.target.value)} placeholder={t('entity.metaKeyPlaceholder')} className="h-7 text-xs w-32 font-mono" disabled={creating} autoFocus/>
                      <span className="text-muted-foreground text-xs">:</span>
                      <Input value={editMetaFieldValue} onChange={(e) => setEditMetaFieldValue(e.target.value)} placeholder={t('entity.metaValuePlaceholder')} className="h-7 text-xs flex-1 font-mono" disabled={creating}/>
                      <Button size="icon" variant="ghost" className="h-6 w-6 shrink-0" onClick={saveEditMeta} disabled={creating}>
                        <Save className="w-3.5 h-3.5"/>
                      ...
                      <Button size="icon" variant="ghost" className="h-6 w-6 shrink-0" onClick={cancelEditMetaEdit} disabled={creating}><X className="w-3.5 h-3.5"/></Button>
                    </div>) : (<Button variant="ghost" size="sm" className="h-6 text-xs gap-1 px-2 mt-1" onClick={startEditAddMeta} disabled={creating || editMetaKey !== null}>
                      <Plus className="w-3 h-3"/> {t('entity.addField')}
                    </Button>)}
                </div>
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <Label>{t('entity.poolEntries')}</Label>
                  <span className="text-xs text-muted-foreground">
                    {editEntries.length}/{MAX_POOL_CREATE_ENTRIES}
                  </span>
                </div>
                <p className="text-xs text-muted-foreground">
                  {t('entity.poolEntriesHint')}
                ...
                <div className="space-y-3">
                  {editEntries.length === 0 ? (<div className="rounded-md border border-dashed border-muted-foreground/30 bg-muted/20 px-3 py-4 text-sm text-muted-foreground">
                      {t('entity.noEntriesInPool')}
                    </div>) : (editEntries.map((row, idx) => (<CreatePoolEntryRow key={row.id} gameId={gameId} row={row} index={idx} disabled={creating} canRemove={editEntries.length > 1} onChange={(patch) => updateEditEntry(row.id, patch)} onRemove={() => removeEditEntry(row.id)}/>)))}
                </div>
                <Button type="button" variant="outline" size="sm" className="w-full gap-2" onClick={addEditEntry} disabled={creating || editEntries.length >= MAX_POOL_CREATE_ENTRIES} title={editEntries.length >= MAX_POOL_CREATE_ENTRIES ? t('entity.entriesLimitReached').replace('{max}', String(MAX_POOL_CREATE_ENTRIES)) : undefined}>
                  <Plus className="h-4 w-4"/>
                  {t('entity.addEntry')}
                </Button>
              </div>
          ...
          <SheetFooter>
            ...
            <Button onClick={handleEditPool} disabled={creating
            || !editForm.name.trim()
            || editEntries.length === 0
            || editEntries.some((entry) => !entry.entity_definition_id.trim() || !entry.weight.trim())}>
                {creating && <Loader2 className="h-4 w-4 mr-2 animate-spin"/>}
              Update
            </Button>
          </SheetFooter>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => {
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDeletePool} disabled={deleting} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
...
function CreatePoolEntryRow({ gameId, row, index, disabled, canRemove, onChange, onRemove, }: {
    gameId: string;
    ...
    return (<div className="rounded-md border border-muted/40 bg-background/60 p-2">
      <div className="flex items-center gap-2">
        <Popover open={open} onOpenChange={setOpen} modal>
          <PopoverTrigger asChild>
            <Button type="button" variant="outline" role="combobox" aria-expanded={open} className="flex-1 min-w-0 justify-between font-normal h-8 px-2 text-xs" disabled={disabled}>
              {selectedLabel ? (<span className="truncate text-left">{selectedLabel}</span>) : (<span className="text-muted-foreground truncate text-left">{t('entity.searchEntityPlaceholder')}</span>)}
              <ChevronsUpDown className="ml-2 h-3.5 w-3.5 shrink-0 opacity-50"/>
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-[--radix-popover-trigger-width] p-0" align="start" style={{ zIndex: 9999 }}>
            <Command shouldFilter={false}>
              <CommandInput placeholder={t('entity.searchEntityDropdown')} value={searchInput} onValueChange={setSearchInput}/>
              <CommandList>
                {searchLoading ? (<div className="flex items-center justify-center py-4">
                    <Loader2 className="h-4 w-4 animate-spin text-muted-foreground"/>
                  </div>) : (<>
                    <CommandEmpty>{t('entity.noEntityFound')}</CommandEmpty>
                    <CommandGroup>
                      {searchResults.map((def) => (<CommandItem key={def.id} value={def.id} onSelect={() => {
                    onChange({
        ...
        <Input type="number" min={MIN_ENTRY_WEIGHT} max={MAX_ENTRY_WEIGHT} value={row.weight} onChange={(e) => onChange({ weight: e.target.value })} className="h-8 w-24 text-xs font-mono" disabled={disabled}/>
        {canRemove ? (<Button type="button" variant="ghost" size="icon" className="h-8 w-8 shrink-0" onClick={onRemove} disabled={disabled}>
            <X className="h-4 w-4"/>
          </Button>) : (<div className="h-8 w-8 shrink-0"/>)}
      </div>
    </div>);
}
...
function PoolExpandedContent({ pool, gameId, onSaved, }: {
    pool: EntityPool;
    gameId: string;
    ...
    useEffect(() => {
        ...
        listEntityDefinitions(gameId, {})
            .then((results) => {
            if (!cancelled)
                setDefSearchResults(results.slice(0, 10));
        })
            .catch(() => {
            if (!cancelled)
                setDefSearchResults([]);
    ...
    function handleDefSearch(value: string) {
        ...
        if (defSearchRef.current)
            clearTimeout(defSearchRef.current);
    ...
    return (<div className="group/expand px-12 py-4 animate-in slide-in-from-top-2 duration-300 space-y-4">
        ...
        <div>
          ...
          <dd>
            ...
            if (k === "drop_pack_ids") {
                ...
                return (<div key={k} className="flex items-center gap-1.5 py-0.5 px-1 rounded">
                      <span className="text-xs font-mono text-muted-foreground w-32 truncate shrink-0">{k}</span>
                      <span className="text-xs text-muted-foreground">:</span>
                      <span className="text-xs font-mono flex-1 text-muted-foreground">{count} pack{count !== 1 ? "s" : ""}</span>
                    </div>);
            }
            return (<div key={k} className="group/meta">
                    {editingMetaKey === k ? (<div className="flex items-center gap-1.5 py-0.5">
                      ...
                      </div>) : (<div className="flex items-center gap-1.5 py-0.5 px-1 rounded hover:bg-muted/50 cursor-pointer" onClick={() => startEditMeta(k, String(v))}>
                        <span className="text-xs font-mono text-muted-foreground w-32 truncate shrink-0">{k}</span>
                        <span className="text-xs text-muted-foreground">:</span>
                        <span className="text-xs font-mono flex-1">{String(v)}</span>
              ...
              {editingMetaKey === "__new__" ? (<div className="flex items-center gap-1.5 py-0.5 mt-1">
                  ...
                  <Button size="icon" variant="ghost" className="h-6 w-6 shrink-0" onClick={cancelEditMeta} disabled={saving}><X className="w-3.5 h-3.5"/></Button>
                </div>) : (<Button variant="ghost" size="sm" className="h-6 text-xs gap-1 px-2 mt-1" onClick={startAddMeta} disabled={saving || editingMetaKey !== null}>
                  <Plus className="w-3 h-3"/> {t('entity.addField')}
                </Button>)}
            </div>
          </dd>
        ...
        <div>
          ...
          <dd>
            ...
            return (<div key={packId} id={`pool-drop-pack-item-${pool.id}-${packId}`} className="pool-drop-pack-item flex items-center gap-1.5 py-0.5 px-1 rounded bg-muted/40">
              ...
              {gachaPacks.length > 0 && (<Select onValueChange={addDropPack} disabled={saving}>
                  <SelectTrigger className="h-7 text-xs mt-1">
                    <SelectValue placeholder={t('entity.linkGachaPackPlaceholder')}/>
                  </SelectTrigger>
                  <SelectContent>
                    {gachaPacks.filter((p) => !getDropPackIds().includes(p.id)).map((p) => (<SelectItem key={p.id} value={p.id} className="text-xs">
                        <span>{p.name}</span>
      ...
      <div>
        <div className="flex items-center gap-2 mb-2">
          ...
          <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => setShowAddEntry(!showAddEntry)} disabled={entries.length >= MAX_ENTRIES_PER_POOL} title={entries.length >= MAX_ENTRIES_PER_POOL ? t('entity.entriesLimitReached').replace('{max}', String(MAX_ENTRIES_PER_POOL)) : undefined}>
            <Plus className="h-3.5 w-3.5"/>
          </Button>
        </div>
        {showAddEntry && (<div className="flex items-end gap-2 mb-3 p-3 rounded-lg border border-muted/30 bg-background/40">
            <div className="w-1/2 min-w-[200px] space-y-1">
              ...
              <Popover open={defPopoverOpen} onOpenChange={setDefPopoverOpen}>
                <PopoverTrigger asChild>
                  <Button variant="outline" role="combobox" aria-expanded={defPopoverOpen} className="w-full h-8 justify-between text-sm font-normal" disabled={addingEntry}>
                    {addDefLabel ? (<span className="truncate">{addDefLabel}</span>) : (<span className="text-muted-foreground">{t('entity.searchEntityPlaceholder')}</span>)}
                    <ChevronsUpDown className="ml-2 h-3.5 w-3.5 shrink-0 opacity-50"/>
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-[--radix-popover-trigger-width] p-0" align="start" style={{ zIndex: 9999 }}>
                  <Command shouldFilter={false}>
                    <CommandInput placeholder={t('entity.searchEntityDropdown')} value={defSearchInput} onValueChange={handleDefSearch}/>
                    <CommandList>
                      {defSearchLoading ? (<div className="flex items-center justify-center py-4">
                          <Loader2 className="h-4 w-4 animate-spin text-muted-foreground"/>
                        </div>) : (<>
                          <CommandEmpty>{t('entity.noEntityFound')}</CommandEmpty>
                          <CommandGroup>
                    ...
                    return sorted.map((def) => {
                        ...
                        return (<CommandItem key={def.id} value={def.id} onSelect={() => selectDef(def)} className={inPool ? "opacity-50" : ""}>
                                    <Check className={`mr-2 h-4 w-4 shrink-0 ${addDefId === def.id ? "opacity-100" : "opacity-0"}`}/>
                                    <span className="flex-1 truncate">{def.name}</span>
                                    {inPool && (<Badge className="ml-2 text-[10px] px-1.5 py-0 h-auto bg-amber-500/15 text-amber-600 border-amber-500/30">
                                        {t('entity.inPool')}
                                      </Badge>)}
                                    <span className="ml-2 text-xs text-muted-foreground font-mono">{def.entity_key}</span>
                                  </CommandItem>);
            ...
            <div className="w-24 space-y-1">
              ...
              <Input type="number" min={MIN_ENTRY_WEIGHT} max={MAX_ENTRY_WEIGHT} value={addWeight} onChange={(e) => setAddWeight(e.target.value)} className="h-8 text-sm font-mono" disabled={addingEntry}/>
            </div>
        ...
        {loadingDetail ? (<div className="space-y-2">
            {[1, 2].map(i => <Skeleton key={i} className="h-8 w-full rounded"/>)}
          </div>) : entries.length === 0 ? (<p className="text-sm text-muted-foreground italic">{t('entity.noEntriesInPool')}</p>) : (<div className="overflow-x-auto rounded-lg border border-muted/30">
            <Table>
              <TableHeader>
              ...
              <TableBody>
                ...
                return (<TableRow key={entry.id} className="border-muted/20 hover:bg-muted/20">
                    <TableCell className="py-1.5">
                      <Link href={`/games/${gameId}/entities?tab=entities&search=${entry.entity_definition_id}`} className="inline-flex items-center gap-1 text-sm font-medium text-primary hover:underline" onClick={(e) => e.stopPropagation()}>
                        {entry.entity_name}
                    ...
                    <TableCell className="py-1.5 text-right">
                      {editingWeightId === entry.id ? (<div className="flex items-center justify-end gap-1">
                          <Input type="number" min={MIN_ENTRY_WEIGHT} max={MAX_ENTRY_WEIGHT} value={editWeightValue} onChange={(e) => setEditWeightValue(e.target.value)} className="h-6 w-20 text-sm font-mono text-right" disabled={savingWeight} autoFocus onKeyDown={(e) => {
                            if (e.key === "Enter")
                                saveWeight(entry.id);
                            if (e.key === "Escape")
                                cancelEditWeight();
                        ...
                        </div>) : (<span className="inline-flex items-center gap-1 cursor-pointer group/weight" onClick={(e) => { e.stopPropagation(); startEditWeight(entry); }}>
                    ...
                    <TableCell className="py-1.5">
                      {entry.stats && Object.keys(entry.stats).length > 0 ? (<div className="flex flex-wrap gap-1">
                          {Object.entries(entry.stats).map(([k, v]) => (<Badge key={k} variant="outline" className="text-[10px] font-mono h-auto py-0 px-1.5 bg-muted/30 border-muted/50">
                              {k}: {String(v)}
                            </Badge>))}
                        </div>) : (<span className="text-muted-foreground text-xs">—</span>)}
                    </TableCell>
                    <TableCell className="py-1.5">
                      <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive hover:bg-destructive/10 opacity-0 group-hover/expand:opacity-100 transition-opacity" disabled={deletingEntryId === entry.id} onClick={(e) => { e.stopPropagation(); handleDeleteEntry(entry.id); }}>
