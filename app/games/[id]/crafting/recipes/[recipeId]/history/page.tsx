...
import {
  ArrowLeft, RefreshCw, Loader2, History, CheckCircle2, XCircle, PackageCheck,
...
function StatusBadge({ success }: { success: boolean }) {
  if (success) {
    return (
      <Badge className="text-xs gap-1 bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800">
...
function TransactionRow({ tx, gameId }: { tx: CraftingHistoryTransaction; gameId: string }) {
    ...
    <>
      ...
      >
        ...
        <TableCell>
          <div className="flex items-center gap-1.5">
            <Link
              href={`/games/${gameId}/players/${tx.user_id}`}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="space-y-1">
                  {tx.materials_snapshot.length === 0 ? (
                    <p className="text-xs text-muted-foreground">No materials</p>
                  ) : tx.materials_snapshot.map((m, i) => (
                    <div key={i} className="flex items-center justify-between text-xs bg-background border rounded px-2 py-1">
                      <div className="flex items-center gap-1.5">
                        <Link
                          href={`/games/${gameId}/items/${m.item_definition_id}`}
                        ...
                        {m.was_consumed && (
                          <Badge variant="secondary" className="text-[10px] px-1 py-0">consumed</Badge>
                        )}
                      </div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {tx.outputs_snapshot.length === 0 ? (
                    <p className="text-xs text-muted-foreground">No outputs</p>
                  ) : tx.outputs_snapshot.map((o, i) => (
                    <div key={i} className="flex items-center justify-between text-xs bg-background border rounded px-2 py-1">
                      <Link
                        href={`/games/${gameId}/items/${o.item_definition_id}`}
                        className="hover:underline text-primary"
                        onClick={e => e.stopPropagation()}
                      >
...
export default function RecipeHistoryPage() {
  const params = useParams() as { id: string; recipeId: string }
  ...
  const fetchHistory = useCallback(async (p: number, status: typeof statusFilter) => {
    ...
    setError(null)
    try {
      const data = await listCraftingRecipeHistory(
        { gameId },
        recipeId,
        {
          page: p,
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <Table>
              ...
              <TableBody>
                {transactions.map(tx => (
                  <TransactionRow key={tx.id} tx={tx} gameId={gameId} />
                ))}
              </TableBody>
            </Table>
          )}
