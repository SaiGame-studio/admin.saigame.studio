...
import { getContainerItems, getGameProgressDetail, PlayerContainer, PlayerItem, } from "@/lib/game-user-api";
...
const CELL_PX = 64;
// ─── Grid View ────────────────────────────────────────────────────────────────
function GridView({ container, items, gameId, onRefresh, loading }: {
    ...
    useEffect(() => {
        ...
        items.forEach((item) => {
            ...
            if (Array.isArray(ids))
                ids.forEach((id: string) => allPackIds.add(id));
        });
    ...
    return (<div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        {onRefresh && (<Button variant="outline" size="icon" onClick={onRefresh} disabled={loading} title="Refresh" className="shrink-0">
      ...
      <div className="overflow-auto pb-2 w-[90%] max-h-[50vh]">
        <div className="relative border border-border rounded-md bg-muted/10 p-px" style={{
          ...
          {items.map((item) => {
            ...
            return (<div key={item.id} className={`absolute rounded border-2 flex flex-col items-center justify-center p-1 cursor-pointer transition-shadow select-none ${s.cell} ${isPinned ? "ring-2 ring-amber-400 ring-offset-background ring-offset-1 z-30 shadow-xl" : isHov ? "ring-2 ring-primary ring-offset-background ring-offset-1 z-20 shadow-lg" : "z-10"}`} style={{
                    left: item.grid_x * (CELL_PX + 1),
                    top: item.grid_y * (CELL_PX + 1),
                    width: w * CELL_PX + (w - 1),
                    height: h * CELL_PX + (h - 1),
                }} onMouseEnter={() => setHovered(item.id)} onMouseLeave={() => setHovered(null)} onClick={() => setPinned(prev => prev === item.id ? null : item.id)}>
                ...
                {item.quantity > 1 && (<span className="text-[9px] text-muted-foreground font-mono mt-0.5">×{item.quantity}</span>)}
                {item.level > 1 && (<span className="text-[9px] text-amber-400 font-mono">Lv{item.level}</span>)}
              </div>);
        })}
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {Object.entries(RARITY_STYLE).map(([rarity, s]) => (<span key={rarity} className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold border capitalize ${s.badge}`}>
            {rarity}
          </span>))}
      </div>
      ...
      {(pinned ?? hovered) && (() => {
            ...
            return (<Card className={pinned ? "border-amber-400/50" : ""}>
            <CardHeader className="py-3">
              <CardTitle className="text-sm flex items-center gap-2">
                {item.definition?.name ?? "Item"}
                {rarity && (<span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold border capitalize ${(RARITY_STYLE[rarity] ?? RARITY_STYLE.common).badge}`}>
                    {rarity}
                  </span>)}
                {pinned === item.id && (<span className="ml-auto text-xs text-amber-400 font-normal">&#x1F4CC; Pinned — click item to unpin</span>)}
              </CardTitle>
            </CardHeader>
            <CardContent className="py-0 pb-4 space-y-4 text-sm">
              ...
              <div>
                ...
                {/* Custom Properties */}
                {item.custom_properties && Object.keys(item.custom_properties).length > 0 && (<div className="mt-2">
              ...
              {item.definition && (<div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div>
                      ...
                      <div className="flex items-center gap-1">
                        <a href={`/games/${gameId}/items/${item.definition.id}`} className="inline-flex items-center gap-1 text-xs font-medium hover:text-primary transition-colors" title={item.definition.id} onClick={(e) => e.stopPropagation()}>
                  ...
                  {/* Base Stats */}
                  {item.definition.base_stats && Object.keys(item.definition.base_stats).length > 0 && (<div className="mt-2">
                      <p className="text-muted-foreground text-xs mb-1">Base Stats</p>
                      <div className="flex flex-wrap gap-1.5">
                        {Object.entries(item.definition.base_stats).map(([k, v]) => (<span key={k} className="text-xs bg-muted rounded px-2 py-0.5 font-mono">{k}: {v}</span>))}
                      </div>
                    </div>)}
                  ...
                  {item.definition.metadata && Object.keys(item.definition.metadata).length > 0 && (() => {
                        ...
                        const restMeta = Object.fromEntries(Object.entries(item.definition.metadata).filter(([k]) => k !== "gacha_pack_ids"));
                        return (<>
                        {/* Gacha Packs */}
                        {gachaPackIds.length > 0 && (<div className="mt-2">
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {gachaPackIds.map((packId) => {
                                    ...
                                    return (<a key={packId} href={`/games/${gameId}/items?tab=gacha&editPack=${packId}`} className="inline-flex items-center gap-1 text-xs bg-muted hover:bg-muted/80 rounded px-2 py-1 font-medium hover:text-primary transition-colors" title={packId} onClick={(e) => e.stopPropagation()}>
                        ...
                        {/* Rest of Metadata */}
                        {Object.keys(restMeta).length > 0 && (<div className="mt-2">
...
// ─── Page ─────────────────────────────────────────────────────────────────────
export default function ContainerItemsPage({ params: paramsPromise, }: {
    params: Promise<{
        id: string;
        progressId: string;
        containerId: string;
    ...
    const itemCategories = useMemo(() => {
        ...
        items.forEach((i) => {
            if (i.definition?.category)
                s.add(i.definition.category);
        });
    ...
    const itemRarities = useMemo(() => {
        const s = new Set<string>();
        items.forEach((i) => {
            if (i.definition?.rarity)
                s.add(i.definition.rarity);
        });
        return Array.from(s).sort();
    }, [items]);
    const filteredItems = useMemo(() => {
        ...
        if (q)
            result = result.filter((item) => (item.definition?.name ?? "").toLowerCase().includes(q) ||
                (item.definition?.item_code ?? "").toLowerCase().includes(q) ||
                item.item_definition_id.toLowerCase().includes(q));
        if (filterCategory)
            result = result.filter((item) => item.definition?.category === filterCategory);
        if (filterRarity)
            result = result.filter((item) => item.definition?.rarity === filterRarity);
        return result;
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            if (!game)
                setGame(gameRes);
            if (detailRes)
                setPlayerName(detailRes.user_display_name ?? "");
    ...
    return (<div className="container mx-auto py-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div className="flex items-start gap-3">
          <Button variant="outline" size="icon" className="mt-1 shrink-0" onClick={() => router.push(`/games/${gameId}/players/${progressId}?tab=containers`)}>
          ...
          <div className="flex-1 min-w-0">
            ...
            {container?.definition && (<div className="flex flex-wrap gap-2 mt-1.5 text-xs text-muted-foreground">
      ...
      <PlayerSectionNav gameId={gameId} progressId={progressId} activeTab="containers-detail"/>
      ...
      {loading ? (<div className="space-y-3">
        ...
        </Card>) : (<Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-4">
          <TabsList>
            <TabsTrigger value="list" className="gap-1.5">
              <List className="h-3.5 w-3.5"/>
              List
              {items.length > 0 && (<span className="ml-1 rounded-full bg-muted px-1.5 py-0.5 text-xs">{items.length}</span>)}
            </TabsTrigger>
            <TabsTrigger value="grid" className="gap-1.5" disabled={!gridCols || !gridRows}>
              <Grid2x2 className="h-3.5 w-3.5"/>
              Grid
              {gridCols && gridRows && (<span className="ml-1 rounded-full bg-muted px-1.5 py-0.5 text-xs">{gridCols}×{gridRows}</span>)}
            </TabsTrigger>
          </TabsList>

          {/* ── List Tab ── */}
          <TabsContent value="list" className="mt-0">
            {items.length === 0 ? (<div className="p-12 text-center text-muted-foreground">
                ...
                <div className="flex items-center justify-between gap-2 flex-wrap mb-3">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <div className="relative">
                      ...
                      <input type="text" placeholder="Search by name…" value={filterText} onChange={(e) => setFilterText(e.target.value)} className="h-8 w-44 rounded-md border border-input bg-background pl-8 pr-7 text-sm outline-none focus:ring-1 focus:ring-ring"/>
                      {filterText && (<button className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground" onClick={() => setFilterText("")}>
                          <X className="h-3.5 w-3.5"/>
                        </button>)}
                    </div>
                    {itemCategories.length > 0 && (<select className="h-8 rounded-md border border-input bg-background px-2 text-sm capitalize" value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)}>
                        <option value="">All Categories</option>
                        {itemCategories.map((c) => (<option key={c} value={c} className="capitalize">{c}</option>))}
                      </select>)}
                    {itemRarities.length > 0 && (<select className="h-8 rounded-md border border-input bg-background px-2 text-sm capitalize" value={filterRarity} onChange={(e) => setFilterRarity(e.target.value)}>
                        <option value="">All Rarities</option>
                        {itemRarities.map((r) => (<option key={r} value={r} className="capitalize">{r}</option>))}
                      </select>)}
                    {(filterText || filterCategory || filterRarity) && (<button className="text-xs text-muted-foreground hover:text-foreground underline-offset-2 hover:underline" onClick={() => { setFilterText(""); setFilterCategory(""); setFilterRarity(""); }}>
                        Clear
                      </button>)}
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filteredItems.length === 0 ? (<TableRow>
                        ...
                        </TableRow>) : filteredItems.map((item) => {
                    const rarity = item.definition?.rarity;
                    ...
                    return (<TableRow key={item.id}>
                            <TableCell>
                              <div className="font-medium whitespace-nowrap flex items-center gap-1">
                                {item.definition?.name ?? item.item_definition_id.slice(0, 8)}
                                <a href={`/games/${gameId}/items/${item.item_definition_id}`} className="text-muted-foreground hover:text-foreground transition-colors shrink-0" title="Open item definition" onClick={(e) => e.stopPropagation()}>
                                  <ExternalLink className="h-3.5 w-3.5"/>
                                </a>
                              </div>
                              {item.definition?.item_code && (<div className="text-xs text-muted-foreground font-mono">{item.definition.item_code}</div>)}
                            </TableCell>
                            <TableCell>
                              {rarity ? (<span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold border capitalize ${badgeStyle}`}>
                                  {rarity}
                                </span>) : "—"}
                            </TableCell>
                            ...
                            <TableCell className="text-xs">
                              {item.definition?.base_stats && Object.keys(item.definition.base_stats).length > 0 ? (<div className="flex flex-wrap gap-1">
                                  {Object.entries(item.definition.base_stats).map(([k, v]) => (<span key={k} className="bg-muted rounded px-1.5 py-0.5 font-mono whitespace-nowrap">{k}: {v}</span>))}
                                </div>) : "—"}
                            </TableCell>
          ...
          <TabsContent value="grid" className="mt-0">
            {container && gridCols && gridRows ? (items.length === 0 ? (<div className="p-12 text-center text-muted-foreground">
                  <Package className="h-12 w-12 mx-auto mb-4 opacity-30"/>
                  <p className="text-lg font-medium">Empty container</p>
                  <p className="text-sm mt-1">No items placed in this container.</p>
                </div>) : (<GridView container={container} items={items} gameId={gameId} onRefresh={loadData} loading={loading}/>)) : (<div className="p-12 text-center text-muted-foreground text-sm">
