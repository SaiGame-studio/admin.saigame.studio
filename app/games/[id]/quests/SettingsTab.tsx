...
export function SettingsTab({ game, onGameUpdate }: Props) {
  ...
  const textDirty =
    delivery === "mailbox" && (title !== savedTitle || body !== savedBody)

  const handleDeliveryChange = async (value: string) => {
    if (value !== "mailbox" && value !== "direct") return
    ...
    <div className="space-y-6 max-w-3xl">
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          >
            <Label
              htmlFor="delivery-mailbox"
              className="flex items-start gap-3 rounded-md border p-4 cursor-pointer hover:bg-muted/50"
            >
