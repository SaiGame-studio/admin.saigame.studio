...
import { ReactFlow, ReactFlowProvider, useReactFlow, Background, Controls, ControlButton, MiniMap, useNodesState, useEdgesState, addEdge, MarkerType, Handle, Position, type Node, type Edge, type NodeTypes, type NodeProps, type Connection, } from "@xyflow/react";
...
import { getChainLayout, saveChainLayout, type QuestChainMember, type QuestDefinition, type ChainLayoutNodePosition, } from "@/lib/quest-api";
// ─── Types ────────────────────────────────────────────────────────────────────
type QuestNodeData = {
    label: string;
    questType: string;
...
function getLayoutedElements(nodes: Node<QuestNodeData>[], edges: Edge[], direction: "TB" | "LR" = "LR") {
...
function QuestNode({ data }: NodeProps<Node<QuestNodeData>>) {
    ...
    return (<div className="relative rounded-lg border bg-card text-card-foreground shadow-sm w-[240px] select-none">
      ...
      <div className="p-3 space-y-1.5">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {data.isActive ? (<Badge className="text-[10px] px-1.5 py-0 h-4 bg-green-600">{t('quest.activeStatus')}</Badge>) : (<Badge variant="secondary" className="text-[10px] px-1.5 py-0 h-4">{t('quest.inactiveStatus')}</Badge>)}
        </div>
      </div>
...
function ChainFlowViewContent({ gameId, chainId, members, questDefsMap, availableQuests, onQuickAdd, onEditMember, onRemoveMember, onConnectQuests, onDisconnectQuests, onRefresh, }: ChainFlowViewProps) {
    ...
    const pendingDropPosRef = useRef<Record<string, {
        x: number;
        y: number;
    }>>({});
        ...
        async function applyLayout() {
            if (isNewChain) {
                ...
                for (const node of rawNodes)
                    delete pendingDropPosRef.current[node.id];
                setNodes(positioned);
                setEdges(rawEdges);
            }
        }
    ...
    const persistLayout = useCallback((currentNodes: Node<QuestNodeData>[]) => {
        ...
        try {
            localStorage.setItem(`chain-layout:${chainId}`, JSON.stringify(positions));
        }
    ...
    const filteredAvailable = useMemo(() => {
        ...
        return availableQuests.filter((d) => d.name.toLowerCase().includes(q) ||
            d.id.toLowerCase().includes(q) ||
            (d.quest_type && d.quest_type.toLowerCase().includes(q)));
    }, [availableQuests, sidebarSearch]);
    const handleQuickAdd = useCallback(async (questId: string) => {
    ...
    return (<div className="flex gap-0 h-[500px] w-full rounded-md border bg-background/50 overflow-hidden">
      ...
      <div className="relative flex-1 min-w-0">
        ...
        <div className="absolute top-2 right-2 z-10 flex items-center gap-1">
          ...
          <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={handleConnect} onEdgesDelete={handleEdgesDelete} onNodeDragStop={handleNodeDragStop} onDragOver={onDragOver} onDrop={onDrop} nodeTypes={nodeTypes} nodesDraggable={!locked} nodesConnectable={!locked} elementsSelectable={!locked} fitView fitViewOptions={{ padding: 0.3 }} proOptions={{ hideAttribution: true }} minZoom={0.3} maxZoom={2} deleteKeyCode={locked ? [] : ["Backspace", "Delete"]} defaultEdgeOptions={{
            ...
            {members.length === 0 && (<div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
            ...
            <Controls showInteractive={false}>
              <ControlButton onClick={() => setLocked((v) => !v)} title={locked ? t('quest.flow.unlockNodes') : t('quest.flow.lockNodes')}>
      ...
      {/* ── Quick-add sidebar ────────────────────────────── */}
      {sidebarOpen && (<div className="w-[260px] shrink-0 border-l flex flex-col bg-background">
          <div className="px-3 py-2 border-b space-y-2">
            ...
            <div className="relative">
              ...
              <Input placeholder={t('quest.flow.searchQuests')} value={sidebarSearch} onChange={(e) => setSidebarSearch(e.target.value)} className="h-7 pl-7 text-xs"/>
            </div>
          </div>
          <ScrollArea className="flex-1">
            {filteredAvailable.length === 0 ? (<p className="text-xs text-muted-foreground text-center py-6">
                {availableQuests.length === 0 ? t('quest.flow.allQuestsInChain') : t('quest.flow.noMatchingQuests')}
              </p>) : (<div className="p-1.5 space-y-0.5">
                {filteredAvailable.map((quest) => (<div key={quest.id} draggable={addingQuestId === null} onDragStart={(e) => {
                        e.dataTransfer.setData("application/chain-quest-id", quest.id);
                        e.dataTransfer.effectAllowed = "move";
                    }} className={cn("w-full text-left rounded-md px-2.5 py-2 hover:bg-accent/60 transition-colors group", "flex items-start gap-2", addingQuestId === null ? "cursor-grab active:cursor-grabbing" : "opacity-70", addingQuestId === quest.id && "opacity-70")}>
