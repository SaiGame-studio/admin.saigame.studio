...
export function QuestDeliveryOverride({ quest, game, onUpdated }: Props) {
  ...
  const savedMode =
    metaDelivery(quest.metadata) ?? gameDefaultMode(game)
    ...
    try {
      ...
      if (!override) {
        nextMeta = Object.fromEntries(
          Object.entries(existing).filter(([k]) => !OVERRIDE_KEYS.includes(k as typeof OVERRIDE_KEYS[number])),
        )
      } else if (mode === "direct") {
        ...
        if (title) nextMeta.mailbox_title = title
        else delete nextMeta.mailbox_title
        if (body) nextMeta.mailbox_body = body
        else delete nextMeta.mailbox_body
      }

      const updated = await updateQuestDefinition(
        game.studio_id,
        game.id,
        quest.id,
        { metadata: nextMeta },
  ...
  return (
    <div className="border rounded-md p-3 bg-background space-y-3">
      <div className="flex items-center justify-between gap-4">
        ...
        <Label
          htmlFor={`override-${quest.id}`}
          className="flex items-center gap-2 text-xs cursor-pointer"
        >
          {t('quest.delivery.overrideToggle')}
          <Switch
            id={`override-${quest.id}`}
            checked={override}
      ...
      {override && (
        <div className="space-y-3 pl-6 border-l-2 border-muted">
          <RadioGroup
            value={mode}
            onValueChange={(v) => {
              if (v === "mailbox" || v === "direct") setMode(v)
            }}
            className="flex gap-4"
          >
            <Label
              htmlFor={`mode-mailbox-${quest.id}`}
