...
export default function ItemProfileDetailPage() {
    ...
    return (<div className="container mx-auto py-6">
      ...
      <div className="mb-6">
        <ItemProfileNameEditable itemProfile={itemProfile} itemProfileId={params.itemProfileId} onItemProfileUpdate={updateItemProfileData}/>
      </div>
      ...
      <div className="bg-background border rounded-lg">
        <div className="border-b">
          <div className="flex justify-between items-center px-6">
            <nav className="flex space-x-8" aria-label="Tabs">
              <button onClick={() => changeTab("details")} className={`${activeTab === "details"
            ...
            : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors duration-200`}>
              ...
              <button onClick={() => changeTab("properties")} className={`${activeTab === "properties"
            ? "border-blue-500 text-blue-600"
            : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors duration-200`}>
                {t('properties.title')}
              </button>
              {(!hideDisabledTabs || itemProfile.type === 'inventory') && (<button onClick={() => {
                if (itemProfile.type === 'inventory') {
                    changeTab("inventory");
                }
            }} disabled={itemProfile.type !== 'inventory'} className={`${itemProfile.type !== 'inventory'
              ...
              {(!hideDisabledTabs || itemProfile.type === 'loot_box_fixed') && (<button onClick={() => {
                if (itemProfile.type === 'loot_box_fixed') {
                    changeTab("lootbox");
                }
            }} disabled={itemProfile.type !== 'loot_box_fixed'} className={`${itemProfile.type !== 'loot_box_fixed'
                ? "border-transparent text-gray-300 cursor-not-allowed line-through"
                : activeTab === "lootbox"
                    ? "border-blue-500 text-blue-600"
                    : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors duration-200`}>
              ...
              {(!hideDisabledTabs || isRngLootboxType(itemProfile)) && (<button onClick={() => {
                if (isRngLootboxType(itemProfile)) {
                    changeTab("rng-lootbox");
                }
            }} disabled={!isRngLootboxType(itemProfile)} className={`${!isRngLootboxType(itemProfile)
                ? "border-transparent text-gray-300 cursor-not-allowed line-through"
            ...
            <TooltipProvider delayDuration={200}>
              <Tooltip>
                <TooltipTrigger asChild>
                  <button onClick={() => setHideDisabledTabs(!hideDisabledTabs)} className="py-2 px-2 text-gray-500 hover:text-gray-700 hover:bg-gray-50 transition-all duration-200 border border-gray-200 rounded-md flex items-center justify-center">
        ...
        <div className="p-6">
          {activeTab === "details" && (<div className="space-y-6 group">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <ItemProfileCodeNameEditable itemProfile={itemProfile} itemProfileId={params.itemProfileId} onItemProfileUpdate={updateItemProfileData}/>
                  </div>

                  <div>
                    <ItemProfileTypeEditable itemProfile={itemProfile} itemProfileId={params.itemProfileId} onItemProfileUpdate={updateItemProfileData}/>
                  </div>

                  <div>
                    <ItemProfileStatusEditable itemProfile={itemProfile} itemProfileId={params.itemProfileId} onItemProfileUpdate={updateItemProfileData}/>
                  </div>

                  <div>
                    <ItemProfileLevelEditable itemProfile={itemProfile} itemProfileId={params.itemProfileId} onItemProfileUpdate={updateItemProfileData}/>
                  </div>

                  <div>
                    <ItemProfileStackableEditable itemProfile={itemProfile} itemProfileId={params.itemProfileId} onItemProfileUpdate={updateItemProfileData}/>
                  </div>

                  <div>
                    <ItemProfileStackLimitEditable itemProfile={itemProfile} itemProfileId={params.itemProfileId} onItemProfileUpdate={updateItemProfileData}/>
                  </div>

                  <div>
                    <ItemProfileCreateOnRegistryEditable itemProfile={itemProfile} itemProfileId={params.itemProfileId} onItemProfileUpdate={updateItemProfileData}/>
                  </div>

                  <div>
                    <ItemProfileAmountOnRegistryEditable itemProfile={itemProfile} itemProfileId={params.itemProfileId} onItemProfileUpdate={updateItemProfileData}/>
                  </div>

                  <div>{t('common.game')}: {game?.id && game?.name ? (<Link href={`/games/${game.id}`} className="inline-flex items-center gap-1 hover:text-primary font-semibold">
                      {game.name}
                      <ExternalLink className="w-4 h-4 text-muted-foreground"/>
                    </Link>) : (<span className="font-semibold">{game?.name}</span>)}</div>

                  {/* Inventory Profile Link */}
                  {itemProfile.inventory_profile_id && (<div>{t('itemProfile.belongsToInventory')}: 
                      <Link href={getInventoryTabUrl(params.id, itemProfile.inventory_profile_id)} className="inline-flex items-center gap-1 hover:text-primary font-semibold ml-1" title={inventoryProfile ? `${inventoryProfile.name} (${inventoryProfile.code_name})` : undefined}>
                        {inventoryLoading ? (<span className="text-muted-foreground">{t('common.loading')}</span>) : inventoryProfile ? (inventoryProfile.name) : inventoryError ? (<span className="text-muted-foreground text-sm" title={t('inventory.inventoryNotLoaded')}>
                            {t('inventory.title')}
                          </span>) : (<span className="text-muted-foreground font-mono text-sm">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {itemProfile.custom_data && Object.keys(itemProfile.custom_data).length > 0 ? (Object.entries(itemProfile.custom_data).map(([key, value]) => (<ItemProfileCustomDataEditable key={key} itemProfile={itemProfile} itemProfileId={params.itemProfileId} onItemProfileUpdate={updateItemProfileData} customKey={key} customValue={value}/>))) : newCustomDataForms.length === 0 ? (<p className="text-muted-foreground text-sm">{t('itemProfile.noCustomData')}</p>) : null}
                    
                    {newCustomDataForms.length < 5 && (<button className="text-sm text-primary hover:text-primary/80 transition-colors font-medium disabled:text-muted-foreground disabled:cursor-not-allowed" onClick={addNewCustomDataForm}>
                        + {t('itemProfile.newCustomData')}
                      </button>)}
                    
                    {newCustomDataForms.map(formId => (<ItemProfileNewCustomDataForm key={formId} formId={formId} itemProfile={itemProfile} itemProfileId={params.itemProfileId} onItemProfileUpdate={updateItemProfileData} onRemove={() => removeNewCustomDataForm(formId)}/>))}
                  </div>
                </div>
              </div>
            </div>)}

          {activeTab === "inventory" && (<InventoryTab itemProfile={itemProfile} gameId={params.id}/>)}

          {activeTab === "lootbox" && (<FixedLootBoxTab itemProfile={itemProfile} gameId={params.id}/>)}

          {activeTab === "rng-lootbox" && (<RngLootBoxTab itemProfile={itemProfile} gameId={params.id}/>)}

          {activeTab === "properties" && (<PropertiesTab itemProfileId={params.itemProfileId}/>)}
        </div>
      </div>
    </div>);
}
function ItemProfileNameEditable({ itemProfile, itemProfileId, onItemProfileUpdate }: {
    itemProfile: ItemProfile;
    itemProfileId: string;
    onItemProfileUpdate: (updatedData: ItemProfile) => void;
}) {
    ...
    return (<div className="flex items-center gap-2">
      ...
      {error && (<div className="text-red-500 text-xs mt-1">
...
function ItemProfileCodeNameEditable({ itemProfile, itemProfileId, onItemProfileUpdate }: {
    itemProfile: ItemProfile;
    itemProfileId: string;
    onItemProfileUpdate: (updatedData: ItemProfile) => void;
}) {
    ...
    return (<div className="flex items-center gap-2">
      {editing ? (<>
          <Input value={codeName} onChange={e => setCodeName(e.target.value)} className="w-48 h-8 px-2 text-base font-mono" disabled={loading}/>
          <Button size="icon" variant="ghost" onClick={handleSave} disabled={loading}>
            <Save className="w-4 h-4"/>
          </Button>
...
function ItemProfileTypeEditable({ itemProfile, itemProfileId, onItemProfileUpdate }: {
    itemProfile: ItemProfile;
    itemProfileId: string;
    onItemProfileUpdate: (updatedData: ItemProfile) => void;
}) {
    ...
    return (<div className="flex items-center gap-2">
      {editing ? (<>
          <Select value={type} onValueChange={setType} disabled={loading}>
            ...
            <SelectContent>
              {itemTypeOptions.map((itemType) => (<SelectItem key={itemType.value} value={itemType.value}>
                  {itemType.label}
                </SelectItem>))}
            </SelectContent>
          </Select>
...
function ItemProfileStatusEditable({ itemProfile, itemProfileId, onItemProfileUpdate }: {
    itemProfile: ItemProfile;
    itemProfileId: string;
    onItemProfileUpdate: (updatedData: ItemProfile) => void;
}) {
      ...
      <div className="flex items-center gap-2">
        {editing ? (<>
            <Select value={status} onValueChange={setStatus} disabled={loading}>
              ...
              <SelectContent>
                {editableStatusOptions.map((statusOption) => (<SelectItem key={statusOption} value={statusOption}>
            ...
            <Button size="icon" variant="ghost" className="opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => setEditing(true)}>
              <Pencil className="w-4 h-4"/>
            </Button>
          </>)}
      </div>
...
function ItemProfileLevelEditable({ itemProfile, itemProfileId, onItemProfileUpdate }: {
    itemProfile: ItemProfile;
    itemProfileId: string;
    onItemProfileUpdate: (updatedData: ItemProfile) => void;
}) {
    ...
    return (<div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        {editing ? (<>
            <div className="flex items-center gap-2">
              <Input type="number" value={levelStart} onChange={e => setLevelStart(Number(e.target.value))} className="w-20 h-8 px-2 text-sm" placeholder="Min" disabled={loading}/>
              <span>-</span>
              <Input type="number" value={levelMax} onChange={e => setLevelMax(Number(e.target.value))} className="w-20 h-8 px-2 text-sm" placeholder="Max" disabled={loading}/>
            </div>
            <Button size="icon" variant="ghost" onClick={handleSave} disabled={loading}>
              <Save className="w-4 h-4"/>
            </Button>
...
function ItemProfileStackLimitEditable({ itemProfile, itemProfileId, onItemProfileUpdate }: {
    itemProfile: ItemProfile;
    itemProfileId: string;
    onItemProfileUpdate: (updatedData: ItemProfile) => void;
}) {
    ...
    return (<div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        {editing ? (<>
            <Input type="number" value={stackLimit} onChange={e => setStackLimit(Number(e.target.value))} className="w-24 h-8 px-2 text-sm" disabled={loading}/>
            <Button size="icon" variant="ghost" onClick={handleSave} disabled={loading}>
              <Save className="w-4 h-4"/>
            </Button>
...
function ItemProfileStackableEditable({ itemProfile, itemProfileId, onItemProfileUpdate }: {
    itemProfile: ItemProfile;
    itemProfileId: string;
    onItemProfileUpdate: (updatedData: ItemProfile) => void;
}) {
    ...
    return (<div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        {editing ? (<>
            <Checkbox checked={stackable} onCheckedChange={(checked) => setStackable(checked === true)} disabled={loading}/>
...
function ItemProfileCreateOnRegistryEditable({ itemProfile, itemProfileId, onItemProfileUpdate }: {
    itemProfile: ItemProfile;
    itemProfileId: string;
    onItemProfileUpdate: (updatedData: ItemProfile) => void;
}) {
    ...
    return (<div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        {editing ? (<>
            <Checkbox checked={createOnRegistry} onCheckedChange={(checked) => setCreateOnRegistry(checked === true)} disabled={loading}/>
...
function ItemProfileAmountOnRegistryEditable({ itemProfile, itemProfileId, onItemProfileUpdate }: {
    itemProfile: ItemProfile;
    itemProfileId: string;
    onItemProfileUpdate: (updatedData: ItemProfile) => void;
}) {
    ...
    return (<div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        {editing ? (<>
            <Input type="number" value={amountOnRegistry} onChange={e => setAmountOnRegistry(Number(e.target.value))} className="w-24 h-8 px-2 text-sm" disabled={loading}/>
            <Button size="icon" variant="ghost" onClick={handleSave} disabled={loading}>
              <Save className="w-4 h-4"/>
            </Button>
...
function ItemProfileCustomDataEditable({ itemProfile, itemProfileId, onItemProfileUpdate, customKey, customValue }: {
    itemProfile: ItemProfile;
    itemProfileId: string;
    onItemProfileUpdate: (updatedData: ItemProfile) => void;
    customKey: string;
    ...
    return (<div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        {editing ? (<>
            <div className="flex items-center gap-2">
              <Input value={key} onChange={e => setKey(e.target.value)} className="w-32 h-8 px-2 text-sm font-medium" disabled={loading} placeholder={t('itemProfile.propertyName')}/>
              <span>:</span>
              <Input value={value} onChange={e => setValue(e.target.value)} className="w-48 h-8 px-2 text-sm" disabled={loading} placeholder={t('itemProfile.propertyValue')}/>
            </div>
...
function ItemProfileNewCustomDataForm({ formId, itemProfile, itemProfileId, onItemProfileUpdate, onRemove }: {
    formId: string;
    itemProfile: ItemProfile;
    itemProfileId: string;
    onItemProfileUpdate: (updatedData: ItemProfile) => void;
    ...
    return (<div className="border rounded-lg p-4 space-y-4 bg-muted/50">
      <div className="grid grid-cols-2 gap-2">
        <Input placeholder={t('itemProfile.propertyName')} value={key} onChange={e => setKey(e.target.value)} disabled={loading}/>
        <Input placeholder={t('itemProfile.propertyValue')} value={value} onChange={e => setValue(e.target.value)} disabled={loading}/>
      </div>
