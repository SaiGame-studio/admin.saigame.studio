...
export default function GameItemProfilesPage() {
    ...
    async function handleSaveStatus(profileId: string) {
        ...
        try {
            ...
            setItemProfiles(prev => prev.map(profile => profile.id === profileId ? updatedProfile : profile));
    ...
    return (<div className="container mx-auto py-6">
      ...
      <div className="mb-6">
        <div>
          ...
          <p className=" text-base">{t('itemProfile.listDesc')} {gameName && (<Link href={`/games/${params.id}`} className="text-lg font-normal text-muted-foreground inline-flex items-center gap-1 hover:text-primary">
              {gameName}
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              <input type="text" className="border rounded px-3 py-2 bg-background text-foreground pr-8" placeholder={t('itemProfile.searchPlaceholder')} value={nameFilter} onChange={e => setNameFilter(e.target.value)} style={{ minWidth: 200 }}/>
              {nameFilter && (<Button variant="ghost" size="icon" className="absolute right-1 top-1/2 transform -translate-y-1/2 h-6 w-6" onClick={clearNameFilter}>
                  <X className="w-3 h-3"/>
                </Button>)}
            </div>
              ...
              <PopoverContent className="w-[250px]">
                <div className="flex flex-col gap-2">
                  {statusOptions.map((option) => (<div key={option.value} className="flex items-center space-x-2">
                      ...
                      <label htmlFor={option.value} className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 flex items-center">
            ...
            <Popover>
              ...
              <PopoverContent className="w-[250px]">
                <div className="flex flex-col gap-2">
                  {itemTypeOptions.map((option) => (<div key={option.value} className="flex items-center space-x-2">
                      <Checkbox id={option.value} checked={typeFilter.includes(option.value)} onCheckedChange={(checked) => toggleTypeFilter(option.value)}/>
                      <label htmlFor={option.value} className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 flex items-center">
                        {option.label}
                      </label>
                    </div>))}
            ...
            <span className="text-sm text-muted-foreground">
              {(statusFilter.length > 0 || typeFilter.length > 0 || nameFilter.trim() !== "") ? (`${t('itemProfile.showing')}: ${filteredProfiles.length} / ${itemProfiles.length}`) : (`${t('itemProfile.total')}: ${itemProfiles.length}`)}
            </span>
          </div>

          {/* Create Profile Section */}
          <div className="flex gap-2 items-center">
            {itemLimitReached ? (<span className="text-sm text-destructive font-medium">
                Limit reached ({game?.usage?.items ?? itemProfiles.length}/{game?.limits?.max_items})
              </span>) : (<>
                <input ref={quickInputRef} type="text" className="border rounded px-2 py-1 bg-background text-foreground" placeholder={t('itemProfile.quickNamePlaceholder')} value={quickProfileName} onChange={e => setQuickProfileName(e.target.value)} disabled={quickProfileLoading} onKeyDown={e => {
                if (e.key === 'Enter')
                    handleQuickCreateProfile();
            }} style={{ minWidth: 160 }}/>
      ...
      {/* Active Filters */}
      {(statusFilter.length > 0 || typeFilter.length > 0 || nameFilter.trim() !== "") && (<div className="mb-4 flex items-center gap-2 flex-wrap">
          <span className="text-sm font-medium">{t('itemProfile.activeFilters')}:</span>
          
          {/* Name Filter Badge */}
          {nameFilter.trim() !== "" && (<Badge variant="secondary" className="flex items-center gap-1">
              {t('itemProfile.searchByName')}: "{nameFilter}"
              <X className="h-3 w-3 cursor-pointer hover:bg-destructive hover:text-destructive-foreground rounded-full" onClick={clearNameFilter}/>
            </Badge>)}
          ...
          {typeFilter.length > 0 && typeFilter.map((type) => {
                ...
                return (<Badge key={type} variant="secondary" className="flex items-center gap-1">
                {typeOption?.label || type}
                <X className="h-3 w-3 cursor-pointer hover:bg-destructive hover:text-destructive-foreground rounded-full" onClick={() => toggleTypeFilter(type)}/>
              </Badge>);
            })}
      ...
      {createProfileError && (<Alert variant="destructive" className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <AlertDescription>
                ...
                {Array.isArray(createProfileError?.hints) && createProfileError.hints.length > 0 && (<ul className="mt-2 list-disc list-inside text-base text-destructive">
                    {createProfileError.hints.map((hint, idx) => (<li key={idx}>{hint}</li>))}
                  </ul>)}
              </AlertDescription>
            </div>
          </div>
      ...
      {itemProfiles.length === 0 ? (<Card>
          ...
          {filteredProfiles.map((profile) => (<Card key={profile.id} className="overflow-hidden group">
                  ...
                  <div className="flex flex-col gap-1 text-sm">
                    <div className="flex items-center gap-2">
                      {editingStatus[profile.id] ? (<>
                          ...
                          <Select value={statusValues[profile.id] || profile.status} onValueChange={value => setStatusValues(prev => ({ ...prev, [profile.id]: value }))} disabled={statusLoading[profile.id]}>
                            <SelectTrigger className="w-40 h-6 text-xs">
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                              {editableStatuses.map((status) => (<SelectItem key={status} value={status}>
                                  {status.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
                                </SelectItem>))}
                            </SelectContent>
                          </Select>
                          <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => handleSaveStatus(profile.id)} disabled={statusLoading[profile.id]}>
                            <Save className="w-3 h-3"/>
                          </Button>
                          <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => handleCancelEditStatus(profile.id)} disabled={statusLoading[profile.id]}>
                            <X className="w-3 h-3"/>
                          </Button>
                        </>) : (<>
                          <div className="flex items-center gap-2">
                        ...
                        color: (() => {
                            ...
                            <Button size="icon" variant="ghost" className="h-6 w-6 opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => handleEditStatus(profile.id)}>
