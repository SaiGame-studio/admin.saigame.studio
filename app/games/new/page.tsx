...
function NewGameForm() {
    ...
    <div className="container mx-auto py-6">
      ...
      {(selectedStudio || studioDetailLoading) && studioId && (<div className="max-w-2xl mx-auto mb-6 rounded-md border p-3 bg-muted/30 space-y-2">
          <div className="flex items-center justify-between text-sm">
            <span className="flex items-center gap-1.5 text-muted-foreground">
              <Gamepad2 className="h-3.5 w-3.5"/>
              Games usage
            </span>
            {studioDetailLoading ? (<Skeleton className="h-5 w-16"/>) : selectedStudio?.limits?.max_games != null ? (<Badge variant={studioLimitReached ? "destructive" : (selectedStudio.usage?.games ?? 0) / selectedStudio.limits.max_games >= 0.8 ? "secondary" : "outline"}>
                {selectedStudio.usage?.games ?? 0} / {selectedStudio.limits.max_games}
              </Badge>) : (<Badge variant="outline">{selectedStudio?.usage?.games ?? 0} / ∞</Badge>)}
          </div>
          {studioDetailLoading ? (<Skeleton className="h-1.5 w-full"/>) : selectedStudio?.limits?.max_games != null && (<Progress value={Math.min(((selectedStudio.usage?.games ?? 0) / selectedStudio.limits.max_games) * 100, 100)} className={`h-1.5 ${studioLimitReached ? "[&>div]:bg-destructive" : (selectedStudio.usage?.games ?? 0) / selectedStudio.limits.max_games >= 0.8 ? "[&>div]:bg-yellow-500" : ""}`}/>)}
          {/* Coin cost hint */}
          {!studioDetailLoading && (<p className="text-xs text-muted-foreground">
      ...
      <Card className="max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit}>
          <CardContent className="space-y-4">
            ...
            return (<SelectItem key={studio.id} value={studio.id} disabled={atLimit}>
            ...
            <div className="space-y-2">
              ...
              <Input id="name" placeholder={t('game.enterName')} value={name} onChange={(e) => setName(e.target.value)} required/>
            </div>
          </CardContent>
...
export default function NewGamePage() {
    return (<Suspense fallback={<div className="container mx-auto py-6">
