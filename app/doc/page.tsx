...
import { BookOpen, LayoutDashboard, Brush, Gamepad2, Users, Server, Puzzle, ShoppingBag, Package, Gift, Wallet, Cog, Shield, ChevronRight, ExternalLink, AlertCircle, CheckCircle2, Info, Zap, Star, Activity, Hash, Terminal, UserCircle, KeyRound, Layers, Swords, BarChart2, } from "lucide-react";
...
// Helpers
// ---------------------------------------------------------------------------
function Heading1({ id, children }: {
    id: string;
    children: React.ReactNode;
}) {
    return (<h1 id={id} className="text-2xl font-bold mt-8 mb-3 scroll-mt-20 flex items-center gap-2 group">
...
function Heading2({ id, children }: {
    id: string;
    children: React.ReactNode;
}) {
    return (<h2 id={id} className="text-lg font-semibold mt-6 mb-2 scroll-mt-20 flex items-center gap-2 group">
      <a href={`#${id}`} className="opacity-0 group-hover:opacity-40 transition-opacity">
...
function Callout({ type = "info", children, }: {
    ...
    return (<div className={cn("flex gap-3 rounded-lg border p-4 text-sm my-4", styles[type])}>
      {icons[type]}
      <div>{children}</div>
    </div>);
}
function StepList({ steps }: {
    ...
    return (<ol className="space-y-2 my-4">
      {steps.map((step, i) => (<li key={i} className="flex items-start gap-3 text-sm">
...
function PropTable({ rows }: {
    ...
    return (<div className="overflow-x-auto my-4 rounded-lg border">
      <table className="w-full text-sm">
        ...
        <tbody>
          {rows.map((r, i) => (<tr key={i} className="border-t hover:bg-muted/30">
...
export default function DocPage() {
    ...
    return (<div className="flex gap-0 min-h-screen">
      ...
      <aside className="hidden xl:flex flex-col w-64 flex-shrink-0 sticky top-0 h-screen overflow-y-auto border-r bg-muted/20 py-6">
        ...
        <nav className="px-3 space-y-0.5">
          {SECTIONS.map((section) => (<div key={section.id}>
              <a href={`#${section.id}`} onClick={() => setActiveId(section.id)} className={cn("flex items-center gap-2 px-3 py-1.5 rounded-md text-sm transition-colors", activeId === section.id
                ? "bg-primary/10 text-primary font-medium"
                : "text-muted-foreground hover:text-foreground hover:bg-muted/50")}>
                {section.icon}
                {section.label}
              </a>
              {section.sub?.map((sub) => (<a key={sub.id} href={`#${sub.id}`} onClick={() => setActiveId(sub.id)} className={cn("flex items-center gap-2 pl-9 pr-3 py-1 rounded-md text-xs transition-colors", activeId === sub.id
      ...
      <main ref={contentRef} className="flex-1 max-w-4xl mx-auto px-6 pb-24 pt-6">
        ...
        ].map((c) => (<Card key={c.title} className="bg-card hover:bg-muted/30 transition-colors">
        ...
        <div className="overflow-x-auto my-4 rounded-lg border">
          <table className="w-full text-sm">
            <thead className="bg-muted/50">
              <tr>
                {["Plugin", "CCU", "Profiles", "Items", "Shops", "Gacha", "Cost / Stack", "Max Stacks"].map((h) => (<th key={h} className="text-left px-3 py-2 font-semibold text-muted-foreground text-xs whitespace-nowrap">{h}</th>))}
              </tr>
            </thead>
            <tbody>
              {[
        ...
        ].map((row) => (<tr key={row.name} className="border-t hover:bg-muted/30">
        ...
        ].map((s) => (<Card key={s.title} className="bg-card">
              <CardHeader className="pb-2 pt-4">
                <CardTitle className="text-sm flex items-center gap-2">
                  {s.icon} {s.title}
                </CardTitle>
